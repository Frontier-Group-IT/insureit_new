...
- **2026-09-24 — Customer App renewal single-card policy list:** branch `fix/customer-policy-renewal-single-card`; Customer App → My Policies now hides explicitly superseded/cancelled internal policies and collapses multiple remaining policy records for the same vehicle to the newest/current renewal. This prevents an expired policy and its renewed replacement from both appearing at the same time, and filter counts use the same collapsed visible set. Historical policy rows remain in the database. No schema, migration, RLS, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**
