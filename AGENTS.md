...
- **2026-09-22 — Partner APK builds made manual-only:** branch `fix/disable-partner-apk-auto-trigger`; removes the `push` trigger from the legacy Partner 0.1.0 APK workflow. It can now run only via manual `workflow_dispatch` and additionally requires the explicit approval token `BUILD_PARTNER_0_1_0`. The existing Partner 0.2.0 APK workflow was already manual-only with its own approval token. **IMPLEMENTED; PR/CI/merge pending. Do not create any Partner APK unless the user explicitly asks for a new APK.**
