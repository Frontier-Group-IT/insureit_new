...
- **2026-09-25 — Supabase consumption reduction plan:** branch `docs/supabase-consumption-reduction-plan-20260925`; extended `docs/SUPABASE_LOG_INGESTION_INVESTIGATION_2026_09_25.md` with a staged, reversible plan. Order: measurement harness → permission-map consolidation → security-preserving middleware auth/profile de-amplification → voice campaign retry/attempt governance and scalable querying → claims polling fallback → Customer App context de-duplication → page-specific query fan-out. Target is 50%+ reduction in daily Edge-log payload with no weakening of auth, RLS, role/employee overrides, or business controls. **PLAN ONLY; no production behavior changed.**
...
- **2026-09-25 — Large Voice Campaign XLSX export hardening:** branch `fix/voice-campaign-large-export`; fixes production HTTP 500/truncation for scaled voice campaigns by paging campaign members + call attempts and chunking opportunity/attempt-event UUID lookups. Existing 33-column XLSX contract, masked mobiles, IT-Super-User authorization and no-transcript privacy boundary remain unchanged. Added canonical `voice-campaign-export:regression`. **IMPLEMENTED; PR/CI/merge/deployment/runtime re-test pending.** See `docs/SARVAM_VOICE_CAMPAIGNS_2026_09_21.md`.

- **2026-09-25 — Customer Start Claim manufacturer logo:** branch `ui/customer-start-claim-manufacturer-logo`; Start Claim vehicle-number selector now resolves the selected vehicle's manufacturer via the shared `getVehicleBrandLogoSource()` catalog and displays that logo in the left-side icon shell, falling back to the existing linked-vehicle icon when no mapped manufacturer asset is available. Added mobile CI regression coverage. No database/schema/RLS/native dependency/runtime/APK/AAB change. **IMPLEMENTED; PR/CI pending.**

- **2026-09-25 — Reports smooth primary-tab navigation:** branch `perf/reports-smooth-tab-navigation-v3`; moves the primary Overview / Business / Portfolio / Operations routes into a shared Next.js route group with a persistent Reports/AppShell header, keeps the shared tab row mounted while only the report content segment changes, and portals each page's existing filters/export controls into the persistent header. Existing report queries, calculations, permissions, filters, exports, schema/RLS, and `prefetch={false}` policy remain unchanged to avoid speculative loading of heavy report routes. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-25 — Partner More header matched to Home + Profile heading:** branch `refine/partner-more-home-header-profile`; More keeps its existing banner artwork, profile card, WORK / INSIGHTS / GROW & LEARN / ACCOUNT sections, routes and logout behavior, while the hero now uses the Home-style shield + `insureit`/`Partner` branding, 33px activity/clock control, 35px initials profile avatar, matching compact typography, and a new `Profile` heading in the banner. The same treatment is mirrored in `scripts/partner/compat/partner-more-reference-0-1.tsx`, with Partner `0.1.0 (5)` OTA workflow assertions updated. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**

- **2026-09-25 — Partner Claims heading moved above search:** branch `refine/partner-claims-heading-above-search`; removed the Claims hero subheading entirely and moved the `Claims` heading to the lower-left of the banner directly above the overlapping search box. Banner artwork, Home-matched branding/activity/profile controls, search/filter UI, KPI cards, claim list/data/navigation, and business logic remain unchanged. The same change is mirrored in the installed Partner `0.1.0 (5)` compatibility Claims layout, with OTA workflow guards ensuring the subtitle does not return. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**

- **2026-09-25 — Partner Claims header matched to Home:** branch `refine/partner-claims-home-header-match`; Claims keeps its existing banner artwork, search, KPI cards, filters, claim list/data/navigation, but the header branding now uses the Home-style shield + `insureit`/`Partner` treatment, Home-matched 33px clock control, 35px profile avatar, spacing, and compact header typography. The same changes are mirrored in `scripts/partner/compat/partner-claims-reference-0-1.tsx`, with OTA workflow assertions updated for installed Partner `0.1.0 (5)`. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**

- **2026-09-25 — Customer App catalog logo fix:** branch `fix/customer-catalog-logo-mapping`; added missing bundled Tata and Toyota vehicle-brand PNG assets to the mobile app from the repo's existing canonical web logo assets, mapped `Tata`, `Tata Motors`, `Toyota`, and Toyota Kirloskar variants in `catalog-logos.ts`, and hardened HDFC ERGO legal-name aliases. Added CI regression to prevent these mappings/assets from disappearing. No database/schema/RLS/native dependency/APK/AAB change. **IMPLEMENTED; PR/CI pending.**

- **2026-09-25 — Reports header title/divider refinement:** branch `refine/reports-header-title-divider`; reduces the shared Reports heading size, adds a subtle vertical divider after the heading, and increases spacing before the Overview / Business / Portfolio / Operations navigation. Mobile removes the divider to avoid cramped stacking. No report logic, filters, exports, schema, RLS, permissions, APK/AAB, or native-runtime changes. **IMPLEMENTED; PR/CI/merge/deployment pending.**\n\n- **2026-09-25 — Shared single-row Reports header:** branch `refine/reports-shared-single-row-header`; removes the per-page “Updated” time and consolidates the Reports title, Overview / Business / Portfolio / Operations navigation, page-specific report controls, and Export action into one compact row across Overview and all shared ReportPageShell pages. Existing dropdown/filter/export behavior, active-tab state, report data logic, schema, RLS, permissions, APK/AAB, and native-runtime behavior remain unchanged. Responsive layouts wrap controls on narrower screens. **IMPLEMENTED; PR/CI/merge/deployment pending.**\n\n- **2026-09-25 — Customer App secure multi-account switching (final security hardening):** rebuilt from current `main`; secure saved sessions remain in `expo-secure-store`; account switching verifies exact Supabase user identity; failed Add Account attempts restore the prior session; failed routing after a successful target `setSession` now also restores the exact prior authenticated session before leaving the old screen active; normal/offline sign-out and inactive-account startup remove only the affected identity; per-user customer context is preserved; standard Home, portfolio Home, and shared Customer headers expose the switcher. No schema/RLS/new native dependency/runtime/APK/AAB change. **IMPLEMENTED; FINAL CI PENDING. REVERSIBLE. NO APK/AAB CREATED.**
...
- **2026-09-25 — Reports dropdown neutral selected/focus state:** branch `fix/reports-neutral-dropdown-selection`; removed the blue/purple selected/focus treatment from Reports header dropdowns while preserving all dropdown behavior. Operations `All vehicles` and Horizon selectors, plus shared report period/horizon selectors, now use neutral white/light-gray selected styling with no purple global focus ring; Overview custom dropdown selected rows use neutral gray instead of blue. Active Reports navigation tab underlines remain blue. No report queries, calculations, filters, routing, export, schema, RLS, permissions, mobile, APK/AAB, or native-runtime changes. **PR #2397 MERGED as `b7da00d17f02c776d17050d59afabd76c5fb4628`; `Verify web portal` workflow #4642 passed; deployment not verified.**

- **2026-09-25 — Operations chart dropdowns made functional:** on branch `fix/reports-portfolio-chart-dropdowns`; the two Operations chart controls that were previously static buttons are now real client-side dropdowns with selected state, outside-click/Escape close behavior, chevron state, and immediate chart switching using existing scoped Operations data. Compliance Status supports By Document / By Exception Type; Document Expiry Aging supports By Expiry Bucket / By Document. Existing 90-day horizon, All vehicles exception selector, Export, KPI logic, customer documents, register, schema, RLS, permission, APK/AAB, and native-runtime behavior remain unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**\n\n- **2026-09-25 — Portfolio chart dropdowns made functional:** branch `fix/reports-portfolio-chart-dropdowns`; the three Portfolio chart controls that were previously static buttons are now real client-side dropdowns with selected state, outside-click/Escape close behavior, and live chart switching using existing scoped report data. Renewal Pipeline supports By Renewal Bucket / By Insurer / By RM; Claims Aging supports By Aging Bucket / By Claim Status / By Insurer; Portfolio Exposure by Insurer supports Top 5 / Top 10 / All Insurers. Wider result sets remain readable through horizontal chart overflow. Existing Horizon, Business, Filters, Export, KPI, register, schema, RLS, permission, APK/AAB, and native-runtime behavior is unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**\n\n- **2026-09-25 — Voice campaign secondary mobile + controlled retry:** branch `fix/voice-campaign-secondary-mobile-retry-main2`; standard and Tata campaign uploads now use `Second number`/alternate mobile when the primary contact is missing or invalid. Campaign detail exposes bulk and row-level manual retry only for latest `busy` / `no_answer` results, records subsequent attempt number in `retry_attempt`, and preserves global kill switch, calling window, terminal/DNC and one-active-attempt guards. Database review of `TATA 100` and `tata after 100` confirmed there were no second attempts before this change; nine follow-up dispositions in `tata after 100` had no `follow_up_at`, so no scheduled second call existed. **IMPLEMENTED; PR/CI/merge pending.**


- **2026-09-25 — Reports Operations reference dashboard redesign:** branch `redesign/reports-operations-reference-dashboard`; replaces the prior table-heavy Operations page with the approved reference-style dashboard: six compact KPIs, Compliance Status grouped-bar chart, Document Expiry Aging chart, Compliance by Document horizontal bars, Vehicle Exception Mix, compact Customer Documents metrics, and a compact Vehicle Exception Register. Existing Horizon / All vehicles / Export controls are preserved and existing Operations filtering semantics remain unchanged. Operations reporting now supports configurable page size so the dashboard can aggregate scoped vehicle/document aging and exception counts without schema changes. No schema, migration, RLS, permission, APK/AAB, or native-runtime changes. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-25 — Reports Portfolio reference dashboard redesign:** branch `redesign/reports-portfolio-reference-dashboard`; replaces the prior renewal-table-heavy Portfolio page with the approved reference-style dashboard: six compact KPIs (Active Policies, Renewal Premium at Risk, Renewals 30d, Open Claims, Claim Exposure, Avg Claim Age), Renewal Pipeline dual-bar chart, Claims Aging dual-bar chart, Claim Status distribution, Portfolio Exposure by Insurer, and a tabbed Renewals/Claims register. Existing Portfolio horizon/business/filter/export controls are preserved. Claims reporting now accepts a configurable page size so the Portfolio dashboard can aggregate claim aging/exposure from scoped report data without schema changes. No schema, migration, RLS, permission, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**\n\n- **2026-09-25 — Customer App default initials avatar:** branch `ui/customer-profile-initials-avatar`; when no uploaded customer profile photo exists, Profile now shows a navy circular initials avatar derived from the customer name (for example Ragini Gupta → RG) instead of the bundled illustrated person. The existing camera overlay, image picker/upload, signed-photo display, and replacement behavior remain unchanged. No schema/RLS/native dependency/runtime/APK/AAB change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**

- **2026-09-25 — Customer App profile photo picker:** branch `feature/customer-profile-photo-picker`; Customer Profile adds a camera overlay on the avatar. Tapping it opens the existing Expo image-library picker with square crop, uploads the selected image into the existing private `customer-documents` storage path, persists it as a hidden `Profile Photo` customer document, displays it through a signed URL, and keeps profile photos out of the Documents & KYC count/list. Replacing the photo cleans up the previous stored profile-photo record best-effort. No schema/RLS/native dependency/runtime/APK/AAB change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**
...
- **2026-09-24 — Vehicle transfer post-success return UX:** PR #2370, merge commit `0a7c1c1e5f8c1624f04e4c5f559d639a443a6963`; successful full vehicle transfers now return to `/vehicles/[id]/edit?success=vehicle_transferred` instead of the read-only details page, and the Edit Vehicle page shows the green transfer-success confirmation above the form. `Verify web portal` workflow #4609 passed on the feature head, including the full-transfer regression, typecheck, lint and production build. No transfer RPC/schema/migration/RLS/permission/mobile/APK/AAB/native-runtime change. **MERGED; deployment not separately verified.**

- **2026-09-24 — Customer Bajaj insurer icon render fix:** branch `fix/customer-bajaj-insurer-icon`; Customer App insurer-logo resolver now uses the proven mobile Bajaj PNG asset for `Bajaj General Insurance Limited` / Bajaj Allianz aliases, fixing the missing Bajaj icon on policy screens. No policy logic, API/RPC, schema, RLS, native runtime, APK/AAB, or deployment change. **IMPLEMENTED; PR/CI/merge/OTA pending.**
...
- **2026-09-24 — Customer App renewed-policy dedupe:** branch `fix/customer-policy-renewal-dedupe`; Customer App → My Policies now shows only the latest/current policy per vehicle instead of showing both an expired policy and its renewed replacement. The list first excludes explicitly superseded/cancelled/rejected/void internal policies, then collapses remaining internal + external policy rows by vehicle using the newest coverage start/end dates, preferring the verified INSUREIT/SIBL record on exact ties. Filter counts and search operate on the same visible-policy set, so hidden historical policies no longer inflate All/Expired counts. No history is deleted and no schema/RLS/migration/native-runtime/APK change. **IMPLEMENTED; PR/CI/merge/OTA pending.**

- **2026-09-24 — Vehicle Edit transfer-button visibility fix:** branch `fix/vehicle-transfer-button-edit-page`; adds the existing full-transfer workflow directly to the normal `/vehicles/[id]/edit` action bar, beside Cancel/Save, for Manager/Admin/Super Admin/IT Super User with effective edit permission. The button routes to the already-deployed atomic `/vehicles/[id]/transfer` flow; no transfer/database logic, migration, RLS, mobile, APK/AAB, or native-runtime behavior changes. Existing full-transfer regression now guards the Edit Vehicle entry point. **MERGED in PR #2362; `Verify web portal` workflow #4593 passed, including the full vehicle-transfer regression, typecheck, lint and production build. Deployment not separately verified.**

- **2026-09-24 — Customer App Policy Detail header action + compact linked vehicle:** branch `ui/customer-policy-detail-linked-vehicle`; moves the existing `Add renewed policy` action from the bottom of the policy card to the far right of the `Policy details` page header, preserving the same renewal eligibility/navigation. Linked Vehicle is simplified into one fully clickable card showing the resolved vehicle manufacturer logo at left and vehicle number beneath `Linked vehicle`; manufacturer/model text, separate Vehicle Number row, Vehicle Type row, and `View vehicle details` row are removed. No policy logic, vehicle logic, API/RPC, schema, RLS, native config, runtime-version, APK/AAB, or deployment change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**


- **2026-09-24 — Business report navigation/table scroll cleanup:** branch `fix/business-report-remove-internal-scrolls`; removed horizontal scrolling from the shared Reports primary tabs and Business subnavigation rows so Overview / Business / Portfolio / Operations and Performance / Distribution / Finance stay fully visible without scrollbar controls. On `/reports/business`, RM Performance and Intermediaries Business no longer use fixed-height vertical scroll containers; both sections expand to render every aggregated row already returned by the report dataset. Business Mix and Insurer retain their intentional internal scrolling. No report formulas, aggregation logic, filters, schema, RLS, permissions, mobile, APK/AAB, or native-runtime changes. **PR #2357 MERGED as `06d0cbf4ed9497fc3cf66a2aece0422b503eb224`; `Verify web portal` workflow #4588 passed; deployment not verified.**
...
- **2026-09-24 — Full vehicle customer transfer workflow:** branch `feature/vehicle-full-customer-transfer-final`; Vehicle Details adds a controlled Transfer Vehicle flow restricted to Manager/Admin/Super Admin/IT Super User. A vehicle cannot move alone: the new service-role-only transactional RPC moves the vehicle plus all customer-scoped managed/external policies, claims, claim documents, claim notifications/support references, customer documents/activity, commission/referral rows, service enquiries and dormant vehicle-customer link state together, while preserving policy snapshots/documents and recording ownership + audit history. Policy Onboarding no longer permits its older partial ownership-transfer path and routes authorized users to the full Vehicle Transfer workflow instead. Migration `20260924163000_vehicle_full_customer_transfer.sql` and rollback/workflow/regression are included. **IMPLEMENTED; PR #2352; final CI/merge/migration application/deployment pending.**
...
- **2026-09-24 — Reports shared reference header:** PR #2340, merge commit `665d0a94022bf607cdd3f51cc4a76666368719db`; standardized report pages on the compact reference header pattern with `Reports` + India-time update stamp, plain primary tabs (Overview / Business / Portfolio / Operations), compact Business subtabs (Performance / Distribution / Finance), and flat filter/action treatment. `ReportPageShell` applies this header across report child pages; Accounts was moved onto the same shell; the existing Overview page keeps its approved reference header. `Verify web portal` workflow #4562 passed, including regressions, typecheck, lint and production build. No report calculations, schema, RLS, mobile, APK/AAB, or native-runtime changes. **MERGED; deployment not verified.**


- **2026-09-24 — Business report reference-layout redesign:** PR #2338, merge commit `dc827094e14ba5801f5aacd905cfca3689a13c57`; `/reports/business` is consolidated into a compact commercial dashboard matching the approved reference direction: the previous summary KPI row, premium trend and policy business register are removed; Commercial Flow is the first segmented row; Business Mix defaults to Business Type and supports scrollable Business Type / Insurer / RM views; Insurer, RM Performance and Intermediaries Business show the scoped commercial dataset in fixed-height scrollable tables with Net Premium, Pay-in, Payout and amount-first Retention where requested. `Verify web portal` workflow #4558 passed, including regressions, typecheck, lint and production build. No schema, migration, RLS, mobile, APK/AAB, or native-runtime change. **MERGED; deployment not verified.**
- **2026-09-24 — Reports Business Mix policy counts + 60-day renewal opportunity:** PR #2333, merge commit `c887024e689473fb5540fc806d19334aa0193cc9`; Business Mix keeps Net Premium as the bar metric, shows the premium value as a data label above each bar, replaces the former amount-at-right position with a **Policies** column using policy count, and keeps Share unchanged for By Insurer / By RM / By Source. Renewal Opportunity now extends to a non-overlapping Within 60 Days bucket (31–60 days), fetches a 60-day horizon, and shows policy count above every bar. `Verify web portal` workflow #4553 passed on the latest-main-synced head before merge. No schema, migration, RLS, permission, APK/AAB, or native-runtime change. **MERGED; production deployment requested.**

- **2026-09-24 — Customer App searchable-dropdown standardization:** branch `ui/customer-searchable-dropdown-standard`; searchable select controls now use one reusable anchored modal pattern matching the existing Vehicle Number selector behavior: auto-focused search, immediate keyboard, scrollable results, selected-row state, outside-tap close, and no form reflow. Applied to Add Policy → Insurer, Add Vehicle → Manufacturer, Add Vehicle → Insurer, and Dealership KYC → OEM. Async City autocomplete fields and ordinary non-search dropdowns are intentionally unchanged. No schema, migration, RLS, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**
...
- **2026-09-24 — Reports Business Mix + Renewal Opportunity:** PR #2328, merge commit `f97196d29e6ade3b99f41cc9493908907ce7cba3`; Business Mix now supports By Insurer / By RM / By Source using scoped report data, while the former Portfolio Risk card is replaced by Renewal Opportunity with Expired / Within 7 Days / Within 15 Days / Within 30 Days buckets, Net Premium at Risk only, and no insurer or Claims Exposure series. Existing report period/business scope is preserved across the Business Mix selector. `Verify web portal` workflow #4543 passed before merge. No schema, migration, RLS, permission, APK/AAB, or native-runtime change. **MERGED; deployment not verified.**

- **2026-09-24 — Customer App missing Honda/Hyundai/HDFC ERGO logo fix:** branch `fix/customer-policy-missing-brand-logos`; Customer App → Policies now includes Honda and Hyundai vehicle-brand PNG assets derived from the repo’s approved web-portal brand artwork, adds normalized Honda/Hyundai resolver aliases, and replaces the faint mobile catalog HDFC ERGO artwork with the existing clearer `apps/mobile-app/assets/vehicles/hdfc-ergo.png` asset. No schema, migration, RLS, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — RM Performance compact independent expansion:** branch `redesign/rm-performance-compact-independent-expand`; removes Avg/Policy from the top Today/MTD summary, compresses the top panels and replaces boxed Motor/Non-Motor chips with inline values. RM rows now use independent client-side expand state so multiple RMs can remain open simultaneously. RM cards use neutral white backgrounds; collapsed headers remain plain white while expanded headers switch to a distinct light header treatment and dark expand control. Today/MTD figures remain grouped with separate soft blue/green backgrounds. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — Reports Overview controls + interactive trend:** PR #2324, merge commit `fe2e294e0ce053213464cebe045758da89075d05`; removes the Overview Filters button; adds report-period MTD / Last Month / Last 6 Months / Custom with custom date modal and outside-click/Escape dismissal; adds All Business / Motor / Non Motor / Life / Health filtering for supported business/finance report data; and adds an independent Business Trend selector defaulting to Last 6 Months with MTD / Last Month daily timelines, Last 6 Months / 1 Year monthly timelines, plus hover data labels for Net Premium and Policies. `Verify web portal` workflow #4538 passed before merge. No schema, migration, RLS, permission, APK/AAB, or native-runtime change. **MERGED; deployment not verified.**

- **2026-09-24 — RM Performance collapsible RM rows:** branch `redesign/rm-performance-collapsible-summary`; RM rows are collapsed by default and show only RM name, contribution, Today policies/net and MTD policies/net. Source breakdown renders only when expanded. Today figures use a cool blue background group and MTD figures use a distinct soft green background group for immediate visual separation. Existing reporting calculations and source-card contents are unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — Operations Dashboard Policy Intake count parity:** branch `fix/dashboard-policy-intake-count-parity`; Dashboard v2 now runs the same policy-number duplicate classification used by the Policy Intake Queue before calculating Action Required / In Review / Processing workload, so duplicate intakes no longer inflate dashboard attention counts. Duplicate-classification failure suppresses the intake summary rather than showing known-inaccurate raw-status totals. Existing dashboard summary regression now guards queue/dashboard duplicate-parity behavior. No schema, migration, RLS, mobile, APK/AAB, or native-runtime change. **IMPLEMENTED; PR #2319; CI/merge/deployment pending.**

- **2026-09-24 — RM Performance hover + inline metric refinement:** branch `redesign/rm-performance-hover-and-inline-metrics-main`; latest-main replacement for PR #2320. Adds proper top spacing in Today/MTD/MTD Context, makes the trend chart interactive on hover/focus with tooltip and guide line, and replaces RM Today/MTD KPI cards with compact inline metrics. Source layout and reporting logic remain unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**
...
- **2026-09-24 — Customer App policy-card catalog-logo redesign:** branch `ui/customer-policy-card-catalog-logos`; Customer App → Policies now uses light green/red/amber status badges, keeps the matching light renewal/expired strip, renames `Type` to `Policy Product`, removes Source and Start from the card details, and presents exactly four details as Manufacturer + Model on the left and Insurer + Expiry on the right. Manufacturer and insurer logos resolve from the mobile catalog with normalized aliases and safe fallbacks; Bajaj Allianz was added to the mobile insurer catalog for Bajaj General/Bajaj Allianz names. The redundant top-left generic/status logo is removed so manufacturer/insurer logos appear only in their detail columns. No schema, migration, RLS, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — RM Performance three-panel daily summary refinement:** branch `redesign/rm-performance-three-panel`; removes the Recent MTD Policies section, moves MTD Context into the top summary so Today / Month-to-Date / MTD Context render as three peer panels, expands RM Daily Summary to full width, and redesigns per-RM source breakdown into spaced source cards inside subtly tinted RM blocks for faster scanning. Existing Today/MTD calculations, RM filters, source aggregation and report permissions are unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — Add Claim no-eligible-policy guard and popup:** branch `fix/add-claim-no-eligible-policy-message`; Operations → Add Claim now blocks creation when the selected vehicle has no active eligible internal policy, shows a clear `No Eligible Policy Available` modal with an OK action, and adds a server-side guard so the database constraint error is never exposed for this case. Existing valid-policy and duplicate-managed-claim behavior is unchanged. No schema, migration, RLS, mobile, Partner app, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — RM Performance daily-summary redesign:** branch `redesign/rm-performance-daily-summary`; removes the unused Retail/Fleet breakdown and redesigns `/rm-performance` as a compact daily + MTD reference. Replaces six KPI cards with a two-column Today/MTD summary, embeds source-wise Today/MTD Net Premium + policy counts under each RM, keeps a compact production trend and recent-policy drill-down, and uses the canonical report RPC with a bounded 5,000-row register fetch only for source aggregation. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-23 — RM Performance page:** branch `feat/rm-performance-page-main`; rebased onto latest main after the original PR's verification failed on an unrelated server-auth regression that has since advanced on main. Adds `/rm-performance` using canonical policy-business reporting for Today, MTD and YTD trend; scoped RM table and drill-down; Motor/Non-Motor mix; insurer mix; recent policy drill-down; dashboard shortcut; Retail/Fleet intentionally unclassified until master data is populated; Email Report reserved but disabled until general transactional mail is configured. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-23 — Non-Motor documents moved into save footer:** branch `ui/non-motor-documents-in-save-footer-main`; rebased onto latest main after PR #2300 diverged. On Non-Motor Policy Edit the standalone Section 06 Documents card is removed and the four document controls render on the left side of the bottom save/action bar, matching the Motor edit layout concept. Activity Status remains immediately above the footer. Create mode keeps its standalone Documents section so documents can still be staged before first save. Existing single-document-per-type, View/Re-upload/Delete, and Add behavior is unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**
...
- **2026-09-23 — External managed claims unrestricted Operations stage editing:** branch `feat/external-claims-unlocked-editing`; External claims with `policy_service_source='external'` and `claim_service_mode='broker_managed'` now keep all 9 Operations stages open and editable, including after Claim Complete/Settled/Closed. Editing a past/future/terminal stage is save-only and does not advance/regress `claims.current_status`; only the actual current stage may advance normally. Internal/SIBL and self-managed External claim behavior is unchanged. No schema/RLS/migration/native change. **IMPLEMENTED; no PR created; merge/deployment pending.** See `docs/CURRENT_CHAT_HANDOFF.md`.
...
- **2026-09-23 — Partner runtime 0.1.0 Claims parity + uploaded header delivery:** branch `fix/partner-0-1-claims-header-runtime`; synchronizes the runtime-0.1 compatibility Claims screen with the current approved Claims UI, uses the dedicated `claims-header-reference.jpg` background, removes the legacy foreground artwork overlay, and updates the existing runtime-0.1 OTA workflow to copy/validate that asset before publish. Target is the installed Partner APK `0.1.0 (5)` on preview runtime/channel. No schema, RPC, native config, runtime-version, APK, AAB or native EAS build change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB BUILD AUTHORIZED.** See `docs/MOBILE_EXPO_PREVIEW_HANDOFF.md`.

- **2026-09-23 — Partner Claims dedicated uploaded hero background:** branch `ui/partner-claims-header-background-20260923`; Claims now uses the user-supplied city/clipboard/shield/vehicle artwork as a dedicated `assets/partner/banners/claims-header-reference.jpg` hero background and removes the redundant foreground Claims artwork overlay so the supplied composition renders cleanly. Home and other Partner screens remain on their existing assets; claims data/search/filter/sort/pagination/RPC behavior is unchanged. No schema, RPC, native config, runtime, permission, APK, AAB or EAS build change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB BUILD AUTHORIZED.** See `docs/CHAT_HANDOFF_2026_09_22_PARTNER_CLAIMS.md`.

- **2026-09-23 — Policy Onboarding server-error diagnostics:** branch `diag/policy-onboarding-actual-server-error`; adds server-only structured logging for Policy Onboarding RPC failures and unexpected exceptions, including execution stage, intake-origin flag, backend error code/redacted message/hint, and stack for thrown errors. UUIDs and policy-number-like values are redacted before logging. Existing popup text, validation, RPC payloads, database/schema/RLS, booking rules and recovery behavior are unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.** See `docs/CURRENT_CHAT_HANDOFF.md`.

- **2026-09-22 — Partner Claims approved-reference compact redesign:** branch `ui/partner-claims-reference-redesign-v3`; Claims now uses the Home-approved Partner logo, Home header action/profile treatment, Home-scale title typography, a compact hero/search/KPI/tab/card layout matching the approved reference, and user-facing labels `In Progress` / `Settled` while preserving the existing backend claim states and RPC behavior. Rejected rows receive a red status pill when returned by existing data, but no unsupported rejected KPI/filter semantics were invented. No schema, RPC, native config, runtime, APK, AAB or EAS build change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB BUILD AUTHORIZED.** See `docs/CHAT_HANDOFF_2026_09_22_PARTNER_CLAIMS.md`.

- **2026-09-22 — Customer onboarding optional KYC copies + policy-origin active-status correction:** branch `fix/customer-onboarding-optional-docs-status`; PAN Copy, Aadhaar Front and Aadhaar Back are optional in Customer Onboarding at both UI and server validation layers, while GST Copy remains required only for GST-registered customers. Customer Edit no longer accepts `onboarding_status` from a hidden browser field; policy-onboarding-origin customers are presented/normalized as active without auto-activating Customer-App KYC records, and newly created Policy Onboarding customers are normalized active after successful booking. No schema/RLS/native/APK change. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-22 — Customer Add Vehicle fetched-policy lock correction v2:** branch `fix/customer-fetched-policy-lock-v2`; fixes the missing wiring left by PR #2260 by actually passing the fetched insurer lock into `SearchInsurer`, and adds a separate fetched Policy No. lock. Confidently fetched insurer is non-editable at 10% opacity; fetched Policy No. is masked and non-editable; non-fetched Policy No. remains masked when unfocused but editable; unresolved insurer remains editable. No API, RPC, schema, RLS, save payload, native config, runtime, permission, or business-rule change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB BUILD AUTHORIZED.** See `docs/MOBILE_EXPO_PREVIEW_HANDOFF.md`.

- **2026-09-22 — Reports Finance insurer logos:** branch `ui/reports-finance-insurer-logos`; Reports → Business → Finance now renders the insurer's shared catalog logo before the insurer name in Insurance company commercials and before the business date in Policy commercial register. Both use `getInsurerLogo`; unmapped insurers keep the existing text-only presentation. No commercial calculations, filters, access control, database, schema, API, or RPC behavior changed. **IMPLEMENTED; no PR created; CI/merge/deployment pending.**
...
- **2026-09-22 — Partner Portal low-risk loading/prefetch Phase 1:** branch `perf/partner-low-risk-loading-prefetch`; adds a Partner route loading skeleton and intent-only prefetch (hover/focus/pointer intent) for common read-only Partner destinations while retaining `prefetch={false}` to avoid broad automatic background execution. The mobile bottom navigation is kept in the persistent Partner layout during loading. Policy Intake, Search, Account and Support remain non-prefetched. No RPC, Supabase, RLS, schema, permission, business-rule or calculation change. **IMPLEMENTED; PR #2251; CI/merge/deployment pending.** See `docs/PERFORMANCE_OPTIMIZATION_HANDOFF.md`.
...
- **2026-09-22 — Master Data vehicle manufacturer logos:** same branch `ui/master-data-insurer-logos`; Administration → Master Data → Vehicle Manufacturer Master now falls back from each row's stored `logo_path` to the shared `getVehicleBrandLogo` resolver using display/legal manufacturer names, so catalog-backed manufacturers such as Ather, Atul Auto, BharatBenz, JCB and others show their existing local logos instead of initials when no row-specific logo path is populated. Existing row logo paths remain first priority and initials remain the final fallback. No database, schema, migration, API, or manufacturer-master record change. **IMPLEMENTED; no PR created; CI/merge/deployment pending.**

- **2026-09-22 — Master Data insurer logos:** branch `ui/master-data-insurer-logos`; Administration → Master Data → Insurance Companies now renders each insurer's shared catalog logo before the company name, retaining the existing neutral shield fallback for unmapped names. The shared resolver also maps Bajaj General/Bajaj Allianz General master-name variants to the existing curated `bajaj-allianz.png` asset. No database, schema, migration, API, or master-data record change. **IMPLEMENTED; no PR created; CI/merge/deployment pending.**

- **2026-09-22 — Reports Insurer Business logos:** same branch `ui/policy-product-insurer-logos`; Reports → Insurer Business · YTD now renders each insurer's catalog logo directly before the insurer name using the shared `getInsurerLogo` resolver, with text-only fallback for unmapped insurers. No report calculation, RPC, schema, or data contract change. **IMPLEMENTED; no PR created; CI/merge/deployment pending.**
...
- **2026-09-21 — Partner Business header reference parity:** branch `ui/partner-business-header-reference-v2`; refines the Business hero to match the approved compact reference composition: same Home hero background asset, INSUREIT-branded top-left icon/wordmark treatment, compact notification/profile actions, left-aligned Business/greeting copy, right-side MORE/POLICIES/STRONGER/RELATIONSHIPS/BRIGHTER/TOMORROW stack, and overlapping search/filter shell. Applied to both current Partner source and the 0.1.0 compatibility source. **IMPLEMENTED; PR/CI/merge/OTA verification pending.**
...
- **2026-09-19 — Partner Claims reference-layout redesign for installed 0.1.0 line:** branch `ui/partner-claims-reference-redesign`; Claims now mirrors the approved reference hierarchy with the existing official Partner logo preserved, deep-blue hero, search/filter bar, four live KPI cards, All/Active/Completed tabs, sort control, dense claim cards and manual Load More. Existing Partner claim RPCs/scope/detail routes are unchanged. 0.1.0 compatibility tooling is prepared for preview OTA from approved `74039199991888777911deb30cf248e8f36cf8a8`; **IMPLEMENTED; NOT MERGED, NOT PUBLISHED, NO APK BUILD.**
...
- **2026-09-18 — Partner Web registration documents + activation date repair:** PR #1998 merged as `04170dad010e5802db3ce39bcfa67f8b68097cfb`; PR #2000 merged as `7cfbd54db59634bc804f7f896e2ca19a5986610b`. Verified root cause was Partner RLS/storage access blocking Operations onboarding-document reads. Registration overview now returns the authenticated Partner's existing document metadata + activation timestamp, and scoped storage access is verified for the same six stored objects. Migrations are **APPLIED + VERIFIED** in production; PR #2004 adds the missing automated schema/deployment gate. Vercel deployment of the repaired code remains **BLOCKED** until PR #2004 is merged and its production run succeeds.
...
- **2026-09-19 — External Claim Proceed takeover trigger fix:** branch `fix/external-claim-takeover-snapshot-trigger`; preserved Customer milestone snapshots marked `external_customer_snapshot=true` now bypass managed-stage persistence before the `broker_managed` lookup, preventing false `Managed claim not found.` rollback during External Claim → Operations takeover. Dedicated schema/deploy gates and regression coverage included. **IMPLEMENTED; migration NOT APPLIED, PR/merge/deployment pending.**
...
## Performance remediation safety protocol
...
- Direct uploads must preserve authorization, type/size validation, metadata finalization, retry/failure cleanup, and orphan prevention before replacing server-mediated uploads.
- Do not replace `window.location` workflow transitions until success, validation failure, server failure, redirect, and refresh semantics are covered by focused regressions.
- Performance logs must never contain secrets, cookies, tokens, PII, customer/policy/document identifiers, filenames, storage paths, or form contents.
- Follow `docs/PERFORMANCE_REMEDIATION_PLAN_2026_08_24.md` for staged order and acceptance gates; update it only with verified evidence.
...
When modifying Policy Onboarding OCR, insurer-specific extraction, Google Document AI, Vercel OIDC federation, the policy review modal, Section 03 field application, or OCR training/refinement, read and follow:
...
- **PDF FINAL TRUTH:** during OCR training/comparison, the actual policy PDF/document is authoritative when a verified/database reference conflicts with it. Flag the reference conflict; never teach a contradicted reference value into parser logic.
- **INSURER -> LAYOUT:** route from the current-policy insurer header first, then select a dedicated product/document-layout parser. The same insurer can have multiple incompatible layouts; do not describe one working layout as universal insurer support.
- Previous-policy insurer, active/existing TP insurer, broker/footer text and historical policy references must not hijack the current-policy parser.
- A passing exact trained sample is not proof of generalization. Every supported layout should have privacy-safe trained/failure-shape coverage plus at least one fresh sibling regression with changed values/identifiers and, where practical, small layout variation.
- Prefer insurer/layout-specific table row/column relationships and bounded text fallbacks over global “label then next plausible token” regex logic. The 22-policy phase showed that table association—not basic OCR character reading—was the dominant remaining error class.
- Treat financial extraction as accounting. Separate OD, Basic TP, owner-driver CPA/PA, other liability additions, net, tax and gross; reject negative/implausible additions, percentages/codes used as amounts, and unreconciled financial sets.
- `cpa_opted = Yes` requires explicit current owner-driver PA/CPA evidence. Paid-driver/workmen/passenger liability additions do not imply owner-driver CPA. Legacy `cpa_premium` compatibility must never automatically flip the boolean to Yes.
- Liability-only/current TP layouts represented by the training corpus use IDV=0 and OD=0. Current SAOD layouts use TP=0 even when an active/existing TP policy is printed as a reference. Follow the actual layout/product semantics and PDF evidence.
...
- Before claiming support for a new insurer/policy type, obtain representative samples, define an approved mapping, add parser evidence/confidence behavior, create trained + fresh-sibling regressions, and validate live reruns.
...
- Do not log or commit raw OCR text, complete policy documents, policyholder PII, real policy/vehicle identifiers, credentials or tokens.
...
- Run typecheck, lint, build and the complete parser/regression suite before merge. Warnings must be reviewed; a successful build is not proof of a successful live OCR journey.
- Do not remove legacy OCR environment variables until Google OCR has been directly verified in production with the supported baseline samples.
...
- **CURRENT TRAINING RESULT:** the user reported approximately 70–75% successful extraction after the v5/v6 22-policy rerun. Record this as `USER-REPORTED`, not as a formal benchmark. The 90–95% target must be proven by a defined live field-by-field measurement and must not be reached by guessing uncertain fields.
