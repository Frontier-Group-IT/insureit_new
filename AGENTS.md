...
- **2026-09-21 — Partner Policies 0.1 OTA JPEG normalization hotfix:** branch `fix/partner-policies-banner-baseline-jpeg`; the user-supplied Policies banner is confirmed to be a real binary JPEG, so the erroneous base64-decode step is removed. The runtime-0.1 OTA workflow now copies the banner directly and normalizes its progressive JPEG encoding to a baseline JPEG with ffmpeg before Expo/Metro export. **IMPLEMENTED; PR/CI/merge/OTA verification pending.** See `docs/MOBILE_EXPO_PREVIEW_HANDOFF.md`.
