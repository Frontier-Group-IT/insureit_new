...
- **2026-09-25 — Customer App global bottom-up navigation:** branch `ui/customer-global-bottom-up-navigation`; added a Customer-level Expo Router stack so all Customer App page navigation uses `slide_from_bottom`, matching the existing Add Vehicle opening behavior. Removed the old one-off root Add Vehicle animation override and changed Customer KYC sub-navigation from `animation: 'none'` to the same bottom-up transition. In-page controls/modals remain unchanged. Added CI regression coverage. No database/schema/RLS/native dependency/runtime/APK/AAB change. **IMPLEMENTED; PR/CI pending.**
