...
- **2026-09-22 — Customer Add Vehicle fetched-policy UX:** branch `ui/customer-vehicle-fetched-policy-lock`; successful RC fetch now shows the normal review-success copy in a 2.5-second popup, Policy No. uses the same alternate-character masking behavior as chassis/engine identifiers, the redundant insurer `Selected:` line is removed, and a confidently fetched insurer is read-only at 10% opacity while unresolved insurer matches remain editable. No API, RPC, schema, RLS, save payload, native config, runtime, permission, or business-rule change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB BUILD AUTHORIZED.** See `docs/MOBILE_EXPO_PREVIEW_HANDOFF.md`.
