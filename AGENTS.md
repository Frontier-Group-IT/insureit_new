...
- **2026-09-25 — Voice Integration production-page compaction:** branch `redesign/voice-integration-production-compact`; removes the large readiness-card row, Production control card, Access & policy card, and duplicate Campaign summary card. Replaces them with one compact health strip, keeps only the active Pause/Resume action prominent, moves connection test/refresh/window edit/diagnostics into overflow, flattens the page header, moves Quick Add RC into the Calling Queue header, and gives the queue full width. Underlying readiness, kill-switch, DND/terminal, webhook, calling-window, Sarvam, recovery and permission logic is unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**
