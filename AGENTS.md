...
- **2026-09-25 — Claims Process column cleanup:** branch `ui/claims-process-display`; the main Claims page now shows only the primary process stage label in the Process column and removes the secondary owner-status subheading (`Customer action`, `Operations action`, `Complete`). Underlying claim workflow/status logic remains unchanged. **IMPLEMENTED; no PR created; CI/merge/deployment pending.**
