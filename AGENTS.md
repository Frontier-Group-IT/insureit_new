...
- **2026-09-25 — Customer Add Vehicle error popups:** branch `fix/customer-add-vehicle-error-popups-ci`; Customer App → Add Vehicle now presents validation, RC lookup, vehicle save, policy save, and policy-copy errors through one compact modal instead of the inline red error banner. Missing insurer uses the exact popup **Insurer not selected — Please select the insurer first to save the policy details.** with Cancel / Select Insurer actions. Informational RC loading/success status remains inline. Added dedicated mobile regression coverage and wired it into `Verify mobile app`. No schema, RLS, native dependency/runtime, APK/AAB, or data-contract change. **IMPLEMENTED; PR #2406 open as the replacement validation PR; initial mobile typecheck failure repaired; final CI/merge/OTA pending. NO APK/AAB CREATED.**
