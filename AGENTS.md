...
- **2026-09-25 — Customer Profile remove Documents & KYC count badge:** branch `ui/customer-profile-remove-document-count-badge`; removed the blue numeric document-count badge from the right side of the Customer App → Profile → Documents & KYC header. The subtitle document count, expand/collapse chevron, upload control, and document list remain unchanged. Added a mobile regression check. No schema, RLS, API/RPC, native runtime, APK/AAB, or data change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**
