...
- **2026-09-24 — Reports custom-period anchored panel:** branch `fix/reports-custom-period-dropdown-panel`; selecting Custom from the report period menu now closes the MTD menu and opens a compact From/To/OK panel directly below the period control instead of a centered page-blocking modal. Clicking outside or pressing Escape closes the panel without applying changes; existing date validation and report query behavior are preserved. No schema, migration, RLS, permission, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**

- **2026-09-24 — Reports Business Mix + Renewal Opportunity:** branch `fix/reports-custom-period-dropdown-panel`; Business Mix now supports By Insurer / By RM / By Source using scoped report data, while the former Portfolio Risk card is replaced by Renewal Opportunity with Expired / Within 7 Days / Within 15 Days / Within 30 Days buckets, Net Premium at Risk only, and no insurer or Claims Exposure series. Existing report period/business scope is preserved across the Business Mix selector. No schema, migration, RLS, permission, APK/AAB, or native-runtime change. **IMPLEMENTED; PR/CI/merge/deployment pending.**
