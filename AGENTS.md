...
- **2026-09-25 — Reports All Business neutral custom dropdown:** branch `fix/reports-business-neutral-custom-dropdown`; replaced the native top-level `All Business` selector inside `ReportCompactFilters` with a custom menu using the same neutral Reports dropdown pattern as Overview. This removes the Windows/Chromium native blue selected-option background on Business/Portfolio pages while preserving the exact `All Business` / `Motor` / `Non Motor` filtering, category reset, URL-query, and page-reset behavior. Selected menu rows now use neutral light gray, hover remains neutral, and keyboard focus uses a subtle neutral ring. No report data, calculations, export, schema, RLS, permissions, mobile, APK/AAB, or native-runtime changes. **PR #2400 MERGED as `daaa59a840b94b56490952b02759e15e32677e5b`; `Verify web portal` workflow #4645 passed; deployment not verified.**
