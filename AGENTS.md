...
- **2026-09-24 — Reports shared reference header:** branch `ui/reports-shared-reference-header`; standardized report pages on the compact reference header pattern with `Reports` + India-time update stamp, plain primary tabs (Overview / Business / Portfolio / Operations), compact Business subtabs (Performance / Distribution / Finance), and flat filter/action treatment. `ReportPageShell` now applies this header across report child pages; Accounts was moved onto the same shell; the existing Overview page keeps its approved reference header. No report calculations, schema, RLS, mobile, APK/AAB, or native-runtime changes. **IMPLEMENTED ON BRANCH; no PR; not merged/deployed.**
