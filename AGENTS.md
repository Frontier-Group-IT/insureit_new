...
- **2026-09-25 — Customer Add Vehicle error popups:** branch `fix/customer-add-vehicle-error-popups-v2`; Customer App → Add Vehicle now presents validation, RC lookup, vehicle save, policy save, and policy-copy errors through one compact modal instead of the inline red error banner. Missing insurer uses **Insurer not selected — Please select the insurer first to save the policy details.** with Cancel / Select Insurer. Informational RC loading/success remains inline. Added dedicated mobile regression coverage in `Verify mobile app`. No schema, RLS, API/RPC contract, native dependency/runtime, APK/AAB, or data-contract change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**
