...
- **2026-09-24 — RM Performance executive-summary redesign:** branch `redesign/rm-performance-premium-summary`; redesigns `/rm-performance` for faster executive scanning. Removes source-type badges and the 5,000-policy warning banner, compresses MTD Context into a compact sparkline panel to keep the three top panels balanced, promotes each RM's Today/MTD figures into high-emphasis metric blocks, and uses a 5-column source-card grid at desktop widths so five sources fit in one row. Existing report calculations, filters, permissions and source aggregation are unchanged. **IMPLEMENTED; PR/CI/merge/deployment pending.**
