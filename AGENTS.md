...
- **2026-09-25 — Reports smooth primary-tab navigation:** branch `perf/reports-smooth-tab-navigation`; moves the primary Overview / Business / Portfolio / Operations routes into a shared Next.js route group with a persistent Reports/AppShell header, keeps the shared tab row mounted while only the report content segment changes, and portals each page's existing filters/export controls into the persistent header. Existing report queries, calculations, permissions, filters, exports, schema/RLS, and `prefetch={false}` policy remain unchanged to avoid speculative loading of heavy report routes. **IMPLEMENTED; PR/CI/merge/deployment pending.**
