...
- **2026-09-24 — Customer App Policy Detail action + linked-vehicle simplification:** branch `ui/customer-policy-detail-action-linked-vehicle`; moves `Add renewed policy` out of the policy card and places it at the far right of the `Policy details` page-title row for renewal-action states. Linked Vehicle is reduced to one fully clickable card showing the resolved manufacturer logo at left, `Linked vehicle`, the vehicle number beneath it, and a right arrow; manufacturer/model subtitle, Vehicle Number row, Vehicle Type row, and separate View Vehicle Details row are removed. No policy/renewal logic, API/RPC, schema, RLS, APK/AAB, native config, or runtime-version change. **IMPLEMENTED; PR/CI/merge/OTA pending. NO APK/AAB CREATED.**
