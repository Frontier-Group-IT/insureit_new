...
- **2026-09-22 — Canonical vehicle multi-customer linking:** branch `feat/multi-customer-vehicle-links`; adds `vehicle_customer_links` so one globally unique RC/chassis vehicle can be associated with multiple customers without duplicating the physical vehicle row. Operations Add Vehicle and Customer Add Vehicle reuse/link an existing RC; Customer vehicle/home/detail/Add Policy reads use the link model; Customer external-policy creation accepts linked vehicles; existing RC/chassis uniqueness and single-active-policy-per-vehicle protection remain intact. Includes migration `20260922170000_multi_customer_vehicle_links.sql`, protected schema workflow/deploy gate, and CI regression. **IMPLEMENTED; PR/CI pending; migration NOT APPLIED; not merged/deployed; no APK/AAB authorized.** See `docs/CURRENT_CHAT_HANDOFF.md` and `docs/MOBILE_EXPO_PREVIEW_HANDOFF.md`.
