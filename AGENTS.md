...
- **2026-09-23 — RM Performance page:** branch `feat/rm-performance-page`; adds `/rm-performance` backed by the canonical policy business report (Today + MTD + YTD trend), scoped by existing `view_reports` employee/hierarchy permissions. Includes team/RM KPIs, Today-vs-MTD RM table, contribution, Motor/Non-Motor mix, top insurers, recent policy drill-down, and a dashboard shortcut. Retail/Fleet is intentionally shown as unclassified because production `policies.business_type` is currently not populated; no inference is made. The Email Report action is visibly reserved but disabled until a general transactional sender for `no-reply@insureit.in` is configured; password reset email alone is not reused as an arbitrary mail API. **IMPLEMENTED; PR/CI/merge/deployment pending.**
