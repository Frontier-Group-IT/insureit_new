...
- **2026-09-24 — Business report reference-layout redesign:** branch `redesign/business-report-reference-layout`; `/reports/business` is consolidated into a compact commercial dashboard matching the approved reference direction: the previous summary KPI row, premium trend and policy business register are removed; Commercial Flow is the first segmented row; Business Mix defaults to Business Type and supports scrollable Business Type / Insurer / RM views; Insurer, RM Performance and Intermediaries Business now show all currently loaded scoped records in fixed-height scrollable tables with Net Premium, Pay-in, Payout and amount-first Retention where requested. The page reuses canonical policy-business and finance report RPCs with a bounded 5,000-row register fetch for aggregation only. No schema, migration, RLS, mobile, APK/AAB, or native-runtime change. **IMPLEMENTED; no PR created; CI/merge/deployment pending.**
