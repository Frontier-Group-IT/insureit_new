...
path = Path("apps/web-portal/components/final-documents/final-documents-workspace-v2.tsx")
text = path.read_text()
old = '''      {row.documentId ? (
...
new = '''      {row.documentId ? (
        <div className="flex items-center gap-2 rounded-lg border border-[#E2EAF4] bg-white/80 p-2">
          <div className="grid h-8 w-8 shrink-0 place-items-center"><DocumentTypeHeaderIcon visual={visual} /></div>
          {row.viewUrl ? (
            <a href={row.viewUrl} target="_blank" rel="noreferrer" title="Open uploaded document" className="min-w-0 flex-1 truncate text-[11px] font-semibold text-[#071D49] transition hover:text-[#174EA6] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#174EA6]/25">{row.fileName ?? "Document uploaded"}</a>
          ) : <p className="min-w-0 flex-1 truncate text-[11px] font-semibold text-[#071D49]">{row.fileName ?? "Document uploaded"}</p>}
          {!verified ? (
            <button type="button" disabled={isPending} onClick={verify} className="h-8 shrink-0 rounded-md border border-[#21A366] bg-white px-2.5 text-[11px] font-semibold text-[#12844F] transition hover:bg-[#F1FBF6] disabled:cursor-not-allowed disabled:border-[#D9E3F0] disabled:text-[#9AA7BA]">{isPending && pendingAction === `verify-${row.type}` ? "..." : "Verify"}</button>
          ) : null}
...
if old not in text:
    raise SystemExit("Expected Stage 3 document card block was not found; refusing to modify unrelated code.")
