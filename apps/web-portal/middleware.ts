...
async function checkSession(accessToken: string): Promise<SessionCheck> {
  ...
  const authResponse = await fetch(`${env.supabaseUrl}/auth/v1/user`, { headers: { apikey: env.supabaseAnonKey, Authorization: `Bearer ${accessToken}` } });
  ...
  const profileResponse = await fetch(`${env.supabaseUrl}/rest/v1/profiles?id=eq.${user.id}&select=id,full_name,role,is_active`, { headers: { apikey: env.supabaseAnonKey, Authorization: `Bearer ${accessToken}` } });
...
async function refreshSession(refreshToken: string): Promise<RefreshedSession | null> {
  const env = getSupabaseEnvironment();
  if (!env) return null;
  const response = await fetch(`${env.supabaseUrl}/auth/v1/token?grant_type=refresh_token`, { method: "POST", headers: { apikey: env.supabaseAnonKey, "Content-Type": "application/json" }, body: JSON.stringify({ refresh_token: refreshToken }), cache: "no-store" });
  if (!response.ok) return null;
  const session = (await response.json()) as Partial<RefreshedSession>;
  if (!session.access_token || !session.refresh_token) return null;
  return { access_token: session.access_token, refresh_token: session.refresh_token, expires_in: Math.max(60, Math.min(session.expires_in ?? 3600, 3600)) };
}
...
export async function middleware(request: NextRequest) {
  ...
  if (check.status === "invalid" && refreshToken) {
    ...
    if (refreshedSession) { accessToken = refreshedSession.access_token; check = await checkSession(accessToken); }
  }
...
export const config = { matcher: ["/", "/login", "/dashboard/:path*", "/customers/:path*", "/vehicles/:path*", "/policies/:path*", "/claims/:path*", "/documents/:path*", "/timeline/:path*", "/tasks/:path*", "/reports/:path*", "/organization/:path*", "/users/:path*", "/notifications/:path*", "/settings/:path*", "/claim-documents/:path*", "/intermediary-portal/:path*"] };
