...
function fail(message) { throw new Error(`[backoffice-data-entry] ${message}`); }
...
const expectedLegacyCapabilities = new Set(["view_dashboard","view_customers","create_customers","view_vehicles","create_vehicles","view_policies","create_policies","create_external_policies","view_reports","view_notifications"]);
...
for (const [permission, access] of [["customers.view","view"],["customers.create","edit"],["vehicles.view","view"],["vehicles.create","edit"],["policies.view","view"],["policies.create","edit"],["reports.view","view"]]) {
...
expect(permissionManagement.includes("maximumPermissionAccessForRole"), "Employees & Access must be able to read the role ceiling before saving overrides.");
const ceilingBlock = permissionManagement.slice(permissionManagement.indexOf("const backofficePermissionCeiling"), permissionManagement.indexOf("export const permissionAccessRank"));
for (const forbidden of ["manage_customers", "view_kyc", "review_kyc", "manage_tasks", "view_intermediaries", "manage_users", "manage_master_data", "manage_system"]) expect(!ceilingBlock.includes(`${forbidden}:`), `Runtime Backoffice ceiling must not allow ${forbidden}.`);

const accessActions = source("app/system/access-control/actions.ts");
expect(accessActions.includes("maximumPermissionAccessForRole(target.role, capability)"), "Employees & Access save workflow must enforce role ceilings server-side.");
expect(accessActions.includes("permissionAccessRank[requested] > permissionAccessRank[ceiling]"), "Employees & Access must reject access above the Backoffice ceiling instead of storing misleading elevation.");
...
const customerDetail = source("app/customers/[id]/page.tsx");
const vehicleDetail = source("app/vehicles/[id]/page.tsx");
const policyDetail = source("app/policies/[id]/page.tsx");
for (const [name, text, capability] of [["customer",customerDetail,"view_customers"],["vehicle",vehicleDetail,"view_vehicles"],["policy",policyDetail,"view_policies"]]) {
  expect(text.includes(`requireCapability("${capability}")`), `Read-only ${name} detail must require ${capability}.`);
  expect(text.includes("View only"), `Read-only ${name} detail must communicate view-only state.`);
  expect(!text.includes("saveVehicleMaster") && !text.includes("requirePolicyEditor") && !text.includes("review_kyc"), `Read-only ${name} detail must not expose edit/approval workflow actions.`);
}
expect(policyDetail.includes("pay-in, billing, partner payout, retention, margin"), "Policy read-only detail must explicitly exclude sensitive finance controls.");

const customerRegister = source("components/backoffice-customer-register.tsx");
const vehicleRegister = source("components/backoffice-vehicle-register.tsx");
const policyRegister = source("components/backoffice-policy-register.tsx");
expect(customerRegister.includes('href={`/customers/${row.id}`}'), "Backoffice customer register must open read-only customer details.");
expect(vehicleRegister.includes('href={`/vehicles/${row.id}`}'), "Backoffice vehicle register must open read-only vehicle details.");
expect(policyRegister.includes('href={`/policies/${row.id}`}'), "Backoffice policy register must open read-only policy details.");
...
console.log("Backoffice data-entry and read-only detail security regression passed.");
