...
assert.match(queueComponent, /Open review/);
assert.match(queueComponent, /document=\$\{encodeURIComponent\(row\.documentId\)\}/);
assert.match(queueComponent, /ValueChoice/);
assert.match(queueComponent, /peer-checked:bg-emerald-600/);
...
const orchestratorActions = readFileSync("app/policies/ocr-training-orchestrator-actions.ts", "utf8");
