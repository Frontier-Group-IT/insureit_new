...
assert(workspace.includes("All Lead Source")&&workspace.includes("All Detail Status")&&workspace.includes("From date")&&workspace.includes("To date"),"Operations queue must expose lead-source, OCR and date filters");
...
assert(contextCard.includes('searchParams.get("intake_id")'),"Policy-copy visibility must use the authoritative Add Policy intake_id URL");
assert(contextCard.includes("verifyPolicyIntakeCurrentPolicyCopy(intakeId)"),"Policy-copy visibility must verify the current Intake document before rendering");
assert(contextCard.includes("verifyPolicyIntakeCurrentPolicyCopy(context.id)"),"Opening the Intake policy copy must re-check the current document to avoid stale copy access");
assert(!contextCard.includes("sessionStorage.getItem"),"Policy-copy visibility must not depend on stale sessionStorage Intake context");
