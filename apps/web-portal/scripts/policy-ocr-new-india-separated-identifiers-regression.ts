...
const result = refineApprovedMotorPolicyLayout(pages, [], text);
...
assert.equal(field(result, "vehicle_engine_number"), "EN9TEST0012345");
...
assert.match(result.parserVersion, /new-india-separated-vehicle-evidence-v4/);
...
assert.notEqual(field(ambiguous, "vehicle_manufacturing_year"), "2025", "fallback must not guess when multiple years exist in Vehicle Details");

console.log("New India separated identifier + unique vehicle year regression passed.");
