...
const vehicleBrandLogoResolver = readFileSync("lib/vehicle-brand-logo.ts", "utf8");
const partnerFleetSummary = readFileSync("app/partner/customers/[id]/partner-customer-fleet-summary.tsx", "utf8");
assert.match(partnerVehicleRegister, /displayVehicleRegistrationNumber/);
assert.match(partnerVehicleRegister, /getVehicleBrandLogo\(vehicle\.make\)/);
assert.match(partnerVehicleRegister, /from "@\/lib\/vehicle-brand-logo"/);
assert.match(partnerFleetSummary, /getVehicleBrandLogo\(vehicle\.make\)/);
assert.match(vehicleBrandLogoResolver, /bharatbenz: "\/assets\/vehicle-brands\/bharatbenz\.png"/);
assert.match(vehicleBrandLogoResolver, /tafe: "\/assets\/vehicle-brands\/tafe\.png"/);
assert.match(vehicleBrandLogoResolver, /jcb: "\/assets\/vehicle-brands\/jcb\.png"/);
assert.match(vehicleBrandLogoResolver, /vecv: "eicher"/);
assert.match(vehicleBrandLogoResolver, /daimler: "bharatbenz"/);
