...
export async function getEmployeeAccessScope(profileId: string, role: string | null | undefined): Promise<EmployeeAccessScope> {
  if (organizationWideRoles.includes(role as AppRole)) {
    return { mode: "organization", employeeIds: [], profileIds: [] };
  }
  ...
  if (!currentProfile?.employee_id) {
    return { mode: "none", employeeIds: [], profileIds: [profileId] };
  }
...
export async function getAccessibleIntermediaryApplicationIds(profileId: string, role: string | null | undefined) {
  ...
  const { data } = filters.length === 1
    ? await request.or(filters[0]).returns<ApplicationLink[]>()
    : await request.or(filters.join(",")).returns<ApplicationLink[]>();
