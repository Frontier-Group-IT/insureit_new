...
export async function loadRenewalReport(profile: ViewerProfile, query: RenewalQuery, pageSize = 25) {
  ...
  const [reportResult, rmResult] = await Promise.all([
    admin.rpc("get_renewal_report_v4", { p_customer_ids: customerIds, p_horizon_days: filters.horizonDays, p_insurer_id: filters.insurerId, p_rm_employee_id: filters.rmEmployeeId, p_intermediary_code: filters.intermediaryCode, p_business_line: filters.businessLine, p_category: filters.category, p_bucket: filters.bucket, p_page: filters.page, p_page_size: pageSize }),
