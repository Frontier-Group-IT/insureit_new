...
export type RenewalFilters = { horizonDays: 30 | 60 | 90 | 180 | 365; insurerId: string | null; rmName: string | null; intermediaryCode: string | null; bucket: RenewalBucket | null; page: number };
...
export type RenewalReport = {
  ...
  rms:Array<{rm_name:string;upcoming_policy_count:number;customer_count:number;due_30_count:number;expired_count:number;premium_at_risk:number;nearest_expiry:string|null}>;
  filters:{insurers:Array<{id:string;name:string}>;rms:string[];intermediaries:Array<{code:string;type:string|null;name:string}>};
...
export async function loadRenewalReport(profile: ViewerProfile, query: RenewalQuery, pageSize = 25) {
  ...
  const { data, error } = await admin.rpc("get_renewal_report", {
...
export function resolveRenewalFilters(query: RenewalQuery): RenewalFilters {
  return {
    horizonDays: horizon(query.horizon),
    insurerId: uuid(query.insurer),
    rmName: text(query.rm, 100),
    intermediaryCode: text(query.intermediary, 120),
    bucket: bucket(query.bucket),
...
function normalizeReport(value:unknown,page:number,pageSize:number):RenewalReport {
  ...
  return {
    ...
    rms:arr(raw.rms).map(v=>{const x=obj(v);return{rm_name:str(x.rm_name),upcoming_policy_count:num(x.upcoming_policy_count),customer_count:num(x.customer_count),due_30_count:num(x.due_30_count),expired_count:num(x.expired_count),premium_at_risk:num(x.premium_at_risk),nearest_expiry:nullable(x.nearest_expiry)}}),
    filters:{insurers:arr(filters.insurers).map(v=>{const x=obj(v);return{id:str(x.id),name:str(x.name)}}).filter(x=>x.id&&x.name),rms:arr(filters.rms).map(str).filter(Boolean),intermediaries:arr(filters.intermediaries).map(v=>{const x=obj(v);return{code:str(x.code),type:nullable(x.type),name:str(x.name)}}).filter(x=>x.code)},
