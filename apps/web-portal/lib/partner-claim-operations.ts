...
import type { DealershipDetailsV2, FinalDocumentRowV2 } from "@/components/final-documents/final-documents-workspace-v2";
import type { SpotSurveyClaim, SpotSurveyDocument, SpotSurveyVerification, SurveyorDetails } from "@/components/spot-survey/spot-survey-workspace-v2";
...
type ClaimDocument = SpotSurveyDocument & { storage_bucket: string; storage_path: string };
type StageDetailRow = { id: string; claim_id: string; details: Record<string, unknown> | null; stage: string | null; created_at: string };
type CustomerMilestoneRow = { milestone_key: string; milestone_status: string; details: Record<string, unknown> | null };

export async function loadPartnerClaimOperationsWorkspace(claimId: string) {
  // This is the authoritative Partner scope proof. It returns no claim outside
  // the authenticated intermediary's commercial scope.
  await getPartnerWebClaimDetail(claimId);
  ...
  const { data: externalPolicy } = claim.external_policy_id
    ? await admin
      ...
      .eq("customer_id", claim.customer_id)
      .eq("vehicle_id", claim.vehicle_id)
      .maybeSingle<{ id: string; policy_no: string | null; policy_type: string | null; start_date: string | null; end_date: string | null; premium_amount: number | null; insured_declared_value: number | null; document_storage_path: string | null }>()
    : { data: null };
  ...
  const linkedPolicy = externalPolicy
    ? { policy_no: externalPolicy.policy_no, policy_type: externalPolicy.policy_type, start_date: externalPolicy.start_date, end_date: externalPolicy.end_date, premium_amount: externalPolicy.premium_amount, insured_declared_value: externalPolicy.insured_declared_value }
    : vehicleRegisterPolicy ?? claim.policies ?? null;
  const claimForVerification: ClaimDetail = { ...claim, policies: linkedPolicy };

  const [{ data: documents }, { data: verificationRows }, { data: stageRows }] = await Promise.all([
    admin.from("claim_documents").select("id, document_type, file_name, storage_bucket, storage_path, verification_status, rejection_reason, created_at").eq("claim_id", claimId).order("created_at", { ascending: false }).returns<ClaimDocument[]>(),
    admin.from("claim_document_verifications").select("id, claim_id, document_id, document_type, verification_type, incident_date, is_valid, invalid_reason, details, created_at").eq("claim_id", claimId).order("created_at", { ascending: false }).returns<SpotSurveyVerification[]>(),
    admin.from("claim_stage_details").select("id, claim_id, stage, details, created_at").eq("claim_id", claimId).order("created_at", { ascending: false }).returns<StageDetailRow[]>(),
  ]);
  ...
  const policyDocument = claim.policy_id
    ...
    : externalPolicy
      ? (await admin.from("customer_documents").select("id,file_name").eq("customer_id", claim.customer_id).eq("external_policy_id", externalPolicy.id).eq("document_type", "policy_copy").order("created_at", { ascending: false }).limit(1).maybeSingle<{ id: string; file_name: string }>()).data
        ?? (externalPolicy.document_storage_path ? (await admin.from("customer_documents").select("id,file_name").eq("customer_id", claim.customer_id).eq("storage_path", externalPolicy.document_storage_path).eq("document_type", "policy_copy").maybeSingle<{ id: string; file_name: string }>()).data : null)
      : null;
  const policyCopy = policyDocument
    ? { fileName: policyDocument.file_name, signedUrl: claim.policy_id ? `/policies/documents/${policyDocument.id}/open` : `/customers/documents/${policyDocument.id}/open`, documentId: policyDocument.id }
    : null;
  ...
  const claimWithSpotIntimation = { ...claimForVerification, spotIntimationAt: spotDetails.spot_intimation_at, spotDetails };
  const finalRows: FinalDocumentRowV2[] = finalDocumentDefinitions.map((document, index) => {
    ...
    return { sr: index + 1, groupIndex: document.groupIndex, groupSr: document.groupSr, type: document.type, name: document.name, documentId: uploaded?.id ?? null, fileName: uploaded?.file_name ?? null, viewUrl: uploaded?.id ? `/claim-documents/${uploaded.id}/open` : null, status: uploaded?.verification_status === "verified" ? "Verified" : uploaded ? "Uploaded" : "Pending" };
  });
  ...
  const externalCustomerMilestoneResult = claim.policy_service_source === "external"
    ? await admin.from("claim_milestones").select("milestone_key, milestone_status, details").eq("claim_id", claimId).returns<CustomerMilestoneRow[]>()
    : null;
  const externalCustomerMilestones = externalCustomerMilestoneResult && !externalCustomerMilestoneResult.error ? externalCustomerMilestoneResult.data ?? [] : undefined;
  const externalCustomerFallbackRows: StageDetailRow[] = (externalCustomerMilestones ?? [])
    ...
    .map((milestone) => ({ id: `external-customer-${milestone.milestone_key}`, claim_id: claimId, stage: null, details: { ...(milestone.details ?? {}), milestone_key: milestone.milestone_key, external_customer_live_fallback: true }, created_at: "1970-01-01T00:00:00.000Z" }));
  ...
  const effectiveSpotDetails = claim.policy_service_source === "external" ? mergeExternalSpotDetails(spotDetails, customerSpotIntimationDetails) : spotDetails;
  const effectiveClaimWithSpotIntimation = { ...claimWithSpotIntimation, spotIntimationAt: effectiveSpotDetails.spot_intimation_at, spotDetails: effectiveSpotDetails };
  ...
  return {
    ...
    signedDocs,
    mergedVerifications,
    stageRows: [...(stageRows ?? []), ...externalCustomerFallbackRows],
    surveyorDetails,
    policyCopy,
    effectiveSpotDetails,
    effectiveClaimWithSpotIntimation,
    finalRows,
    dealershipDetails,
