...
const backofficePermissionCeiling: Partial<Record<PermissionKeyV2, Exclude<AccessLevel, "none">>> = {
  ...
  "policy_intakes.finalize": "approve",
  "reports.view": "view",
...
export function resolveEffectivePermissionV2(
  ...
  if (!input.employeeActive) return noAccess(input.permission, "inactive_identity", "Employee record is inactive.");
  if (!input.portalIdentityActive) return noAccess(input.permission, "inactive_identity", "Portal identity is inactive.");
  ...
  const activeRoles = activeAssignments.map((assignment) => roleByCode.get(assignment.roleCode)).filter((role): role is RoleDefinitionV2 => Boolean(role));
  ...
  if (protectedItRole) {
    ...
    if (protectedGrant) return { permission: input.permission, allowed: true, access: protectedGrant.access, scopes: dedupeScopes([protectedGrant.scope]), source: "protected_role", contributingRoles: ["it_super_user"], reason: "Protected IT Super User access cannot be downgraded by an ordinary employee override." };
  }

  const backofficeOnly = activeRoles.length > 0 && activeRoles.every((role) => role.code === "backoffice_executive");
  const matchingOverrides = (input.overrides ?? []).filter((override) => override.permission === input.permission && overrideIsCurrent(override, now));
  const directOverride = matchingOverrides.at(-1);

  if (backofficeOnly) {
    const ceiling = backofficePermissionCeiling[input.permission];
    if (!ceiling) return noAccess(input.permission, "no_grant", "Backoffice Executive is restricted to governed operational permissions.", ["backoffice_executive"]);
    if (directOverride?.access === "none") return noAccess(input.permission, "employee_deny", directOverride.reason?.trim() || "An active employee-specific deny overrides the Backoffice baseline.", ["backoffice_executive"]);
    if (directOverride) {
      const cappedAccess = accessRank[directOverride.access] > accessRank[ceiling] ? ceiling : directOverride.access;
      return { permission: input.permission, allowed: true, access: cappedAccess, scopes: dedupeScopes([directOverride.scope ?? "organization"]), source: "employee_override", contributingRoles: [], reason: "Employee override is capped by the Backoffice Executive safety ceiling." };
    }
    return { permission: input.permission, allowed: true, access: ceiling, scopes: input.permission === "notifications.view" ? ["self"] : ["organization"], source: "role_grant", contributingRoles: ["backoffice_executive"], reason: "Granted by the Backoffice Executive governed operational baseline." };
  }

  if (directOverride) {
    if (directOverride.access === "none") return noAccess(input.permission, "employee_deny", directOverride.reason?.trim() || "An active employee-specific deny overrides inherited role grants.", activeRoles.map((role) => role.code));
    return { permission: input.permission, allowed: true, access: directOverride.access, scopes: dedupeScopes([directOverride.scope ?? undefined]), source: "employee_override", contributingRoles: [], reason: directOverride.reason?.trim() || "Active employee-specific override takes precedence over role defaults." };
  }
  ...
  if (!roleGrants.length) return noAccess(input.permission, "no_grant", "No active role grants this permission.", activeRoles.map((role) => role.code));
  ...
  return { permission: input.permission, allowed: true, access, scopes: dedupeScopes(strongest.map(({ grant }) => grant.scope)), source: "role_grant", contributingRoles: strongest.map(({ role }) => role.code), reason: strongest.length === 1 ? `Granted by ${strongest[0].role.label}.` : `Granted by ${strongest.map(({ role }) => role.label).join(", ")}.` };
}
