...
export function LoginForm() {
  ...
  return <form className="space-y-4" onSubmit={handleSubmit}>
    ...
    <div className="grid gap-2"><label htmlFor="password">Password</label><input className={isSubmitting ? "opacity-40 transition-opacity" : "transition-opacity"} id="password" type="password" placeholder="••••••••" value={password} onChange={(event) => setPassword(event.target.value)} required disabled={isSubmitting} /></div>
