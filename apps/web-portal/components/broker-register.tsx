...
type RegisterViewTabsProps = {
  value: string;
  options: Array<{ value: string; label: string; count?: number; disabled?: boolean; title?: string }>;
...
export function BrokerRegisterShell({
  ...
  const useCompactMetrics = compactMetrics || policyIntakeCompactLayout;
  ...
  if (compactToolbar) {
    ...
    const compactTabs = tabs
      ? cloneElement(tabs, {
          options: title === "Customer Portfolio"
            ? tabs.props.options
    ...
    <section className="mx-auto max-w-[1480px] overflow-hidden rounded-2xl border border-[#DCE5EF] bg-white shadow-[0_18px_55px_rgba(15,23,42,0.07)]">
      <div className={`border-b border-[#E5ECF5] bg-[#F8FAFC] px-4 sm:px-5 ${compactReferenceLayout || useCompactMetrics ? "py-3" : "py-4"}`}>
        {compactReferenceLayout ? (
          ...
          <div className={`grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(620px,0.95fr)] xl:items-center ${useCompactMetrics ? "xl:gap-4" : "gap-4"}`}>
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                {showSupportingCopy && description && !policyIntakeCompactLayout ? <p className="mt-1 max-w-2xl text-[11.5px] leading-5 text-[#64748B]">{description}</p> : null}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
              {metrics.map((metric) => (
                <div key={metric.label} className={`rounded-xl border px-3 ${useCompactMetrics ? "py-2" : "py-2.5"} ${metricTones[metric.tone ?? "slate"]}`}>
                  <p className="text-[8.5px] font-bold uppercase tracking-[0.08em] opacity-70">{metric.label}</p>
                  <p className={`${useCompactMetrics ? "mt-0.5 text-[18px]" : "mt-1 text-[20px]"} font-semibold leading-none tabular-nums`}>{metric.value}</p>
                  {!useCompactMetrics ? <p className="mt-1 truncate text-[9.5px] font-medium opacity-75">{metric.hint}</p> : null}
                </div>
              ))}
            </div>
          </div>
