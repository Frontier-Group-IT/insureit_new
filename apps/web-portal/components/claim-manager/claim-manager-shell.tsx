...
type SecondaryItem = { href: string; label: string; count?: number };
...
export async function ClaimManagerShell({ title, backHref = "/dashboard", children, activeNav = "claims" }: Props) {
  ...
  const [{ count: customerCount }, notificationRows] = await Promise.all([
    supabase.from("customers").select("id", { count: "exact", head: true }),
    getNotificationRows(supabase)
  ]);
  const notificationGroups = buildNotificationGroups(notificationRows);
  const secondary = secondaryNavigation(activeNav, customerCount ?? 0);
  ...
  return (
    <div className="min-h-screen bg-[#F4F6FA] text-[#101828]">
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-16 border-r border-white/10 bg-[#15183B] text-white lg:flex lg:flex-col">
        <Link href="/dashboard" className="grid h-14 place-items-center border-b border-white/10" aria-label="InsureIt home"><span className="grid h-8 w-8 place-items-center rounded-lg bg-white/12 text-[11px] font-bold tracking-wide">IT</span></Link>
        <nav className="flex flex-1 flex-col items-center gap-1.5 px-2 py-3"><RailItem href="/dashboard" icon="⌂" label="Dashboard" active={activeNav === "dashboard"} /><RailItem href="/claims" icon="▤" label="Claims" active={activeNav === "claims"} /><RailItem href="/customers" icon="▦" label="Master Data" active={activeNav === "master-data"} /><RailItem href="/tasks" icon="✓" label="Tasks" active={activeNav === "tasks"} /><RailItem href="/reports" icon="▥" label="Reports" active={activeNav === "reports"} /></nav>
        <div className="border-t border-white/10 px-2 py-3"><RailItem href="/settings" icon="⚙" label="Settings" active={false} /></div>
      </aside>

      {secondary ? <aside className="fixed inset-y-0 left-16 z-40 hidden w-48 border-r border-[#D9E0EA] bg-[#E9EBFF] lg:block"><div className="flex h-14 items-center border-b border-[#D3D7EF] px-4"><p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#454A76]">{secondary.title}</p></div><nav className="space-y-1 px-3 py-4">{secondary.items.map((item) => <SecondaryNavItem key={item.href} {...item} active={matchesTitle(title, item.label)} />)}</nav></aside> : null}

      <div className={hasSecondary ? "lg:pl-64" : "lg:pl-16"}>
        <header className="sticky top-0 z-30 border-b border-[#DDE3EC] bg-white/95 backdrop-blur"><div className="flex h-14 items-center justify-between gap-4 px-4 lg:px-5"><div className="flex min-w-0 items-center gap-3"><Link href={backHref} className="grid h-8 w-8 shrink-0 place-items-center rounded-md border border-[#E1E6EE] text-lg text-[#344054] hover:bg-[#F6F8FB]" aria-label="Back">‹</Link><img src={logoUrl} alt="InsureIT" className="h-8 w-[118px] shrink-0 object-contain object-left" /><div className="hidden h-6 w-px bg-[#E3E7EE] sm:block" /><h1 className="truncate text-[16px] font-semibold tracking-tight text-[#17203A]">{title}</h1></div><div className="flex items-center gap-2"><label className="relative hidden md:block"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[13px] text-[#98A2B3]">⌕</span><input aria-label="Global search" placeholder="Search" className="h-8 w-56 rounded-md border border-[#D8DEE8] bg-[#FAFBFC] py-1 pl-8 pr-3 text-[12px]" /></label><NotificationMenu groups={notificationGroups} count={notificationRows.length} /><div className="ml-1 border-l border-[#E4E7EC] pl-2"><UserMenu profile={profile} user={user ? { id: user.id, email: user.email } : null} /></div></div></div></header>
...
function secondaryNavigation(activeNav: Props["activeNav"], customerCount: number) {
  const map: Partial<Record<NonNullable<Props["activeNav"]>, { title: string; items: SecondaryItem[] }>> = {
    claims: { title: "Claims", items: [{ href: "/claims", label: "All Claims" }, { href: "/claims?stage=documents", label: "Documents" }, { href: "/claims?stage=verification", label: "Verification" }, { href: "/claims?stage=survey", label: "Survey" }, { href: "/claims?stage=repair", label: "Repair" }, { href: "/claims?stage=settlement", label: "Settlement" }] },
    "master-data": { title: "Master Data", items: [{ href: "/customers", label: "Customers", count: customerCount }, { href: "/vehicles", label: "Vehicles" }, { href: "/policies", label: "Policies" }] },
    tasks: { title: "Tasks", items: [{ href: "/tasks", label: "My Tasks" }, { href: "/tasks?view=team", label: "Team Tasks" }, { href: "/tasks?status=completed", label: "Completed" }] },
    reports: { title: "Reports", items: [{ href: "/reports", label: "Overview" }, { href: "/reports?view=claims", label: "Claims Reports" }, { href: "/reports?view=operations", label: "Operations" }] }
  };
  return activeNav ? map[activeNav] : undefined;
}

function matchesTitle(title: string, label: string) { const a = title.toLowerCase(); const b = label.toLowerCase(); if (b === "all claims") return a.includes("claim"); if (b === "my tasks") return a.includes("task"); if (b === "overview") return a.includes("report"); return a.includes(b.replace(/s$/, "")); }
function RailItem({ href, icon, label, active }: { href: string; icon: string; label: string; active: boolean }) { return <Link href={href} title={label} aria-label={label} className={`group relative grid h-10 w-10 place-items-center rounded-lg text-[17px] transition ${active ? "bg-[#7067E8] text-white shadow-[0_5px_16px_rgba(74,65,190,0.35)]" : "text-white/68 hover:bg-white/10 hover:text-white"}`}>{icon}<span className="pointer-events-none absolute left-12 z-50 hidden whitespace-nowrap rounded-md bg-[#10132F] px-2 py-1 text-[10px] font-medium text-white shadow-lg group-hover:block">{label}</span></Link>; }
function SecondaryNavItem({ href, label, count, active }: SecondaryItem & { active: boolean }) { return <Link href={href} className={`flex items-center justify-between rounded-md px-3 py-2 text-[12px] font-medium transition ${active ? "bg-[#242653] text-white shadow-sm" : "text-[#454A68] hover:bg-white/70 hover:text-[#242653]"}`}><span>{label}</span>{typeof count === "number" ? <span className={`rounded-full px-2 py-0.5 text-[9px] font-semibold ${active ? "bg-white/15 text-white" : "bg-white/75 text-[#454A76]"}`}>{count}</span> : null}</Link>; }
