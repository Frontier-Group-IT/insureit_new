...
export type NavigationItem={kind?:"item";href:string;label:string;icon:LucideIcon;capability:Capability};
export type NavigationGroup={kind:"group";key:string;label:string;icon:LucideIcon;capability:Capability;items:NavigationItem[]};
export type NavigationNode=NavigationItem|NavigationGroup;
export type NavigationSection={key:SectionKey;label:string;icon:LucideIcon;tint:string;capability:Capability;items:NavigationNode[]};
...
export const navigationSections:NavigationSection[]=[
 ...
 {key:"master-data",label:"Customers & Fleet",icon:LayoutGrid,tint:"from-[#6759ff] to-[#8f7cff]",capability:"view_customers",items:[
  ...
  {kind:"group",key:"fleet",label:"Fleet Management",icon:Gauge,capability:"view_vehicles",items:[
   ...
   {href:"/vehicles/new",label:"Add Vehicle",icon:Plus,capability:"manage_customers"},
...
export function visibleNavigationSections(role:string|null|undefined){const filterNode=(node:NavigationNode):NavigationNode|null=>{if(!hasCapability(role,node.capability))return null;if(node.kind!=="group")return node;const items=node.items.filter(item=>hasCapability(role,item.capability));return items.length?{...node,items}:null};const sections=navigationSections.filter(section=>hasCapability(role,section.capability)).map(section=>({...section,items:section.items.map(filterNode).filter((node):node is NavigationNode=>Boolean(node))})).filter(section=>section.items.length);return role==="it_super_user"?[...sections,developmentSection]:sections}
...
export function AppNavigation({activeNav,role}:Props){
 ...
 return <aside className="fixed inset-y-0 left-0 z-50 hidden w-[268px] overflow-hidden border-r border-white/10 bg-[#111a35] text-white shadow-[20px_0_60px_rgba(17,26,53,0.22)] lg:flex lg:flex-col"><div className="pointer-events-none absolute inset-0 overflow-hidden"><div className="absolute -left-20 top-14 h-52 w-52 rounded-full bg-[#6759ff]/25 blur-3xl"/><div className="absolute -right-20 bottom-20 h-56 w-56 rounded-full bg-[#17c7c9]/15 blur-3xl"/><div className="portal-noise absolute inset-0 opacity-20"/></div><Link href="/dashboard" className="relative flex h-[78px] items-center border-b border-white/10 px-5" aria-label="InsureIt home"><BrandLockup compact inverse/></Link><nav className="relative flex-1 overflow-y-auto px-3.5 py-4">{hasCapability(role,"view_dashboard")?<Link href="/dashboard" className={`group mb-2 flex h-12 items-center gap-3 rounded-2xl px-3.5 text-[12px] font-bold ${activeNav==="dashboard"&&!routeSection?"bg-white text-[#141d3b] shadow-[0_14px_35px_rgba(0,0,0,.18)]":"text-white/90 hover:bg-white/10 hover:text-white"}`}><span className="grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-[#66B5FF] via-[#2F6BFF] to-[#1746C8] text-white"><Gauge className="h-4 w-4"/></span><span className="flex-1">Dashboard</span></Link>:null}{sections.length?<p className="mb-2 mt-6 px-3 text-[9px] font-black uppercase tracking-[0.18em] text-white/65">Workspaces</p>:null}<div className="space-y-1.5">{sections.map(section=>{const open=openSection===section.key;const active=resolved===section.key;const SectionIcon=section.icon;return <div key={section.key} className={`overflow-hidden rounded-2xl border ${active?"border-white/15 bg-white/10":"border-transparent"}`}><button type="button" onClick={()=>setOpenSection(current=>current===section.key&&!active?null:section.key)} className="flex h-11 w-full items-center gap-3 px-3.5 text-left text-[12px] font-bold text-white"><span className={`grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br ${section.tint}`}><SectionIcon className="h-4 w-4"/></span><span className="flex-1">{section.label}</span><ChevronRight className={`h-4 w-4 ${open?"rotate-90":""}`}/></button>{open?<div className="space-y-1 px-2.5 pb-2.5 pl-[42px]">{section.items.map(node=>node.kind==="group"?<NavigationGroupView key={node.key} sectionKey={section.key} group={node} pathname={pathname} currentQuery={currentQuery} open={Boolean(openGroups[`${section.key}:${node.key}`])} onToggle={()=>setOpenGroups(current=>({...current,[`${section.key}:${node.key}`]:!current[`${section.key}:${node.key}`]}))}/>:<NavigationLink key={node.href} item={node} pathname={pathname} currentQuery={currentQuery}/>)}</div>:null}</div>})}</div></nav><div className="relative border-t border-white/10 p-3.5"><Link href="/settings" className="group flex h-11 items-center gap-3 rounded-2xl px-3.5 text-[12px] font-bold text-white/88 hover:bg-white/10"><span className="grid h-8 w-8 place-items-center rounded-xl bg-white/10"><Settings className="h-4 w-4"/></span><span>Settings</span></Link></div></aside>
}
function NavigationGroupView({sectionKey,group,pathname,currentQuery,open,onToggle}:{sectionKey:SectionKey;group:NavigationGroup;pathname:string;currentQuery:string;open:boolean;onToggle:()=>void}){const Icon=group.icon;const active=group.items.some(item=>isCurrent(item.href,pathname,currentQuery));return <div className="rounded-xl"><button type="button" onClick={onToggle} className={`flex min-h-9 w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-[10.5px] font-semibold ${active?"bg-white/12 text-white":"text-white/82 hover:bg-white/10"}`}><Icon className="h-3.5 w-3.5"/><span className="flex-1 truncate">{group.label}</span><ChevronRight className={`h-3.5 w-3.5 ${open?"rotate-90":""}`}/></button>{open?<div className="ml-3 mt-1 space-y-1 border-l border-white/12 pl-2">{group.items.map(item=><NavigationLink key={`${sectionKey}:${item.href}`} item={item} pathname={pathname} currentQuery={currentQuery}/>)}</div>:null}</div>}
function NavigationLink({item,pathname,currentQuery}:{item:NavigationItem;pathname:string;currentQuery:string}){const active=isCurrent(item.href,pathname,currentQuery);const Icon=item.icon;return <Link href={item.href} title={item.label} className={`group flex min-h-9 items-center gap-2 rounded-xl px-2.5 py-2 text-[10.5px] font-semibold ${active?"bg-white text-[#17213e]":"text-white/82 hover:bg-white/10 hover:text-white"}`}><Icon className={`h-3.5 w-3.5 ${active?"text-[#6759ff]":"text-white/60"}`}/><span className="truncate">{item.label}</span></Link>}
export function sectionForPath(pathname:string):SectionKey|null{if(pathname==="/customers/posp-misp/icall-uat")return"development";if(pathname==="/claims"||pathname.startsWith("/claims/"))return"claims";if(pathname==="/intermediaries"||pathname.startsWith("/intermediaries/")||pathname==="/customers/posp-misp"||pathname.startsWith("/customers/posp-misp/"))return"distribution";if(pathname==="/tasks"||pathname.startsWith("/tasks/"))return"tasks";if(pathname==="/reports"||pathname.startsWith("/reports/"))return"reports";if(pathname==="/employees"||pathname.startsWith("/employees/")||pathname==="/customers"||pathname.startsWith("/customers/")||pathname==="/customer-kyc"||pathname.startsWith("/customer-kyc/")||pathname==="/vehicles"||pathname.startsWith("/vehicles/")||pathname==="/policies"||pathname.startsWith("/policies/"))return"master-data";return null}
