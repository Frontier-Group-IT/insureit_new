...
import { BarChart3, CheckSquare2, ChevronRight, ClipboardList, FileChartColumn, FileCheck2, FlaskConical, Gauge, LayoutGrid, Plus, Settings, ShieldCheck, Sparkles, Upload, UserPlus, UsersRound } from "lucide-react";
...
export const navigationSections:NavigationSection[]=[
 ...
 {key:"master-data",label:"Customers & Fleet",icon:LayoutGrid,tint:"from-[#6759ff] to-[#8f7cff]",capability:"view_customers",items:[
  ...
  {kind:"group",key:"fleet",label:"Fleet Management",icon:Gauge,capability:"view_vehicles",items:[
   {href:"/vehicles",label:"Vehicles",icon:Gauge,capability:"view_vehicles"},
   {href:"/policies",label:"Policies",icon:ShieldCheck,capability:"view_policies"}
  ]},
...
export function visibleNavigationSections(role:string|null|undefined){
 const filterNode=(node:NavigationNode):NavigationNode|null=>{
  if(!hasCapability(role,node.capability))return null;
  if(node.kind!=="group")return node;
  const items=node.items.filter(item=>hasCapability(role,item.capability));
  return items.length?{...node,items}:null;
 };
 const sections=navigationSections.filter(section=>hasCapability(role,section.capability)).map(section=>({...section,items:section.items.map(filterNode).filter((node):node is NavigationNode=>Boolean(node))})).filter(section=>section.items.length);
 return role==="it_super_user"?[...sections,developmentSection]:sections;
}
