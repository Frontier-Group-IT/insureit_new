...
export type FinalVerificationData = {
  claim: { id: string; customer_id: string; accident_at: string | null; policies: PolicyDates | null };
...
export async function loadFinalDocumentVerificationData(claimId: string): Promise<Result> {
  try {
    ...
    await requireClaimWorkflowAccess(claimId, "You do not have permission to verify claim documents.");
    ...
    if (claim.external_policy_id) {
      const { data } = await admin.from("external_policies").select("start_date, end_date").eq("id", claim.external_policy_id).eq("customer_id", claim.customer_id).maybeSingle<PolicyDates>();
      policyDates = data ?? null;
    } else if (claim.policy_id) {
      const { data } = await admin.from("policies").select("start_date, end_date").eq("id", claim.policy_id).eq("customer_id", claim.customer_id).maybeSingle<PolicyDates>();
      policyDates = data ?? null;
    }

    const [{ data: documents, error: documentsError }, { data: verifications, error: verificationsError }] = await Promise.all([
      admin.from("claim_documents").select("id, document_type, file_name, verification_status, rejection_reason, created_at").eq("claim_id", claimId).order("created_at", { ascending: false }).returns<Array<Omit<FinalVerificationDocument, "signedUrl">>>(),
      admin.from("claim_document_verifications").select("id, claim_id, document_id, document_type, verification_type, incident_date, is_valid, invalid_reason, details, created_at").eq("claim_id", claimId).order("created_at", { ascending: false }).returns<FinalVerificationRow[]>(),
    ]);
    ...
    return {
      ok: true,
      data: {
        claim: { id: claim.id, customer_id: claim.customer_id, accident_at: claim.accident_at, policies: policyDates },
        documents: (documents ?? []).map((document) => ({ ...document, file_name: document.file_name || "Unnamed claim document", signedUrl: `/claim-documents/${document.id}/open` })),
