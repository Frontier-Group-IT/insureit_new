...
function canRetryOcr(row: PolicyIntakeWorkspaceRow) {
  ...
  return Number.isFinite(created) && Date.now() - created >= STALE_OCR_MS;
}
...
export function PolicyIntakeWorkspace({ rows, reviewer, creator, currentProfileId, initialView }: { rows: PolicyIntakeWorkspaceRow[]; reviewer: boolean; creator: boolean; currentProfileId: string; initialView?: PolicyIntakeViewKey }) {
  ...
  }), [baseFiltered, view, currentProfileId]);
    ...
    <div className="hidden overflow-x-auto md:block">
      <table className="w-full min-w-[1280px] table-fixed text-left text-[10px] text-[#334155]">
        ...
        <tbody className="divide-y divide-[#E8EDF4]">{pageRows.map((row) => <tr key={row.id} className={`h-[58px] transition ${rowTones[row.status] ?? "hover:bg-[#FAFCFF]"}`} onClick={() => { window.location.href = `/policy-intakes/${row.id}`; }} style={{ cursor: "pointer" }}>
          <td className="px-3"><div className="flex items-center gap-1.5">{getInsurerLogo(field(row, "insurer_name")) ? <img src={getInsurerLogo(field(row, "insurer_name"))!} alt="" aria-hidden="true" className="h-4 w-4 shrink-0 object-contain" /> : null}<p className="truncate font-bold text-[#17365D]">{row.intake_number}</p></div><p className="mt-0.5 truncate text-[8px] text-[#7A8798]">{row.file_name}</p></td>
