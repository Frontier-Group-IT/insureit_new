...
export function VehicleForm({ action, customers, manufacturers = [], values, submitLabel = "Save record", beforeActions, footerContent, actionExtra, createCustomerHref, allowPolicyContinuation = false, showUnregisteredRegistrationIdentity = false }: { action: FormAction; customers: SelectOption[]; manufacturers?: SelectOption[]; values?: VehicleValues; submitLabel?: string; beforeActions?: ReactNode; footerContent?: ReactNode; actionExtra?: ReactNode; createCustomerHref?: string; allowPolicyContinuation?: boolean; showUnregisteredRegistrationIdentity?: boolean }) {
...
function VehicleSection({ number, title, children, columns, reserveHeaderAction = false }: { number: string; title: string; children: ReactNode; columns: "two" | "three" | "four" | "five" | "six"; reserveHeaderAction?: boolean }) {
  ...
  return <section className={`relative ${reserveHeaderAction ? "overflow-visible" : "overflow-hidden"} rounded-xl border border-[#D9E2F0] bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)]`}>
    ...
    <div className={`grid min-w-0 grid-cols-1 gap-x-3 gap-y-3 ${bodyPadding} ${grid}`}>{children}</div>
  </section>;
}
