...
function PolicyDetails({ claim }: { claim: SpotSurveyClaim }) { const policy = claim.policies; return <section className="grid gap-2 rounded-xl border border-[#DFE8F4] bg-white px-4 py-2.5 text-[11px] sm:grid-cols-2 lg:grid-cols-4"><PolicyMeta label="Policy source" value={claim.policySource === "external" ? "External policy" : "Sankalp policy"} /><PolicyMeta label="Cover dates" value={`${formatDateShort(policy?.start_date)} - ${formatDateShort(policy?.end_date)}`} /><PolicyMeta label="Premium / IDV" value={`${formatAmount(policy?.premium_amount)} / ${formatAmount(policy?.insured_declared_value)}`} />{claim.policyCopy?.signedUrl ? <Link href={claim.policyCopy.signedUrl} target="_blank" className="flex min-w-0 items-center justify-between rounded-lg bg-[#F4F8FF] px-3 py-2 font-semibold text-[#174EA6]">Policy copy <span className="truncate pl-2 text-[10px]">{claim.policyCopy.fileName}</span></Link> : <PolicyMeta label="Policy copy" value="Not available" />}</section>; }
function PolicyMeta({ label, value }: { label: string; value: string }) { return <div className="min-w-0"><span className="mr-1 uppercase tracking-[0.08em] text-[#68758A]">{label}:</span><span className="font-semibold text-[#071D49]">{value}</span></div>; }
