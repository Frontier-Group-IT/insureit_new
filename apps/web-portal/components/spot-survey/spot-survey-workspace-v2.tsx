...
import { SpotMediaUploadButton } from "./spot-media-upload-button";
...
type Item = { key: string; number: number; title: string; icon: string; accent: string; documentType: string; document?: SpotSurveyDocument | null; documentCount?: number };
...
function buildDocumentItems(documents: SpotSurveyDocument[]): Item[] { const doc = (key: keyof typeof aliases) => documents.find((d) => aliases[key].some((alias) => d.document_type.toLowerCase().includes(alias)) && d.verification_status !== "rejected") ?? documents.find((d) => aliases[key].some((alias) => d.document_type.toLowerCase().includes(alias))) ?? null; return [{ key: "spot", number: 1, title: "Accident Photo", icon: "📷", accent: "bg-[#EAF4FF]", documentType: "Accident Photo", document: doc("spot") }, { key: "rc", number: 2, title: "RC Copy", icon: "📄", accent: "bg-[#F1ECFF]", documentType: "RC Copy", document: doc("rc") }, { key: "insurance", number: 3, title: "Insurance Copy", icon: "📃", accent: "bg-[#FFF3D9]", documentType: "Insurance Copy", document: doc("insurance") }, { key: "dl", number: 4, title: "Driver Licence", icon: "🪪", accent: "bg-[#EAF8EF]", documentType: "Driver Licence", document: doc("dl") }, { key: "gr", number: 5, title: "GR / Load Bill", icon: "🚚", accent: "bg-[#FFF1E6]", documentType: "GR / Load Bill", document: doc("gr") }, { key: "video", number: 6, title: "Accident Video", icon: "🎥", accent: "bg-[#F2EEFF]", documentType: "Accident Video", document: doc("video") }]; }
/*
