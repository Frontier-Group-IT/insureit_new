...
function Info({ icon, label, title, subtitle, logo, last = false }: { icon?: string; label: string; title: string; subtitle?: string | null; logo?: ReactNode; last?: boolean }) { return <div className={`flex min-h-[58px] items-center gap-2 px-3 py-2 ${last ? "" : "border-b border-white/15 md:border-b-0 md:border-r"}`}><div className="grid h-8 w-8 shrink-0 place-items-center text-[18px]">{logo ?? icon}</div><div className="min-w-0 flex-1"><p className="text-[9px] font-medium uppercase tracking-[0.04em] leading-3.5 text-[#9FC5FF]">{label}</p><p className="mt-0.5 whitespace-normal break-words text-[13px] font-semibold leading-4 text-white">{title}</p>{subtitle ? <p className="whitespace-normal break-words text-[11px] leading-3.5 text-[#DCE6F5]">{subtitle}</p> : null}</div></div>; }
