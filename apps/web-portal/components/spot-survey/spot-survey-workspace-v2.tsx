  ...
  return (
    <div className="mt-3 rounded-2xl border border-green-200 bg-green-50 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-[15px] font-semibold text-green-800">Spot surveyor already deputed</h2>
          <p className="mt-1 text-[12px] text-green-700">This claim has moved to the surveyor appointed stage.</p>
        </div>
        {details?.deputedAt ? <span className="rounded-full border border-green-200 bg-white px-3 py-1 text-[11px] font-semibold text-green-700">Deputed on {formatDateShort(details.deputedAt)}</span> : null}
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <SurveyorInfo label="Surveyor Name" value={details?.name || "Not available"} icon="👤" />
        <SurveyorInfo label="Mobile No." value={details?.mobile || "Not available"} icon="☎" href={details?.mobile ? `tel:${details.mobile}` : undefined} />
        <SurveyorInfo label="Email ID" value={details?.email || "Not available"} icon="✉" href={details?.email ? `mailto:${details.email}` : undefined} />
      </div>
    </div>
  );
}

function SurveyorInfo({ label, value, icon, href }: { label: string; value: string; icon: string; href?: string }) {
  const content = <><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-[16px] shadow-sm">{icon}</span><span className="min-w-0"><span className="block text-[10px] font-semibold uppercase tracking-[0.08em] text-green-700">{label}</span><span className="mt-0.5 block truncate text-[13px] font-semibold text-[#071D49]">{value}</span></span></>;
  const className = "flex items-center gap-3 rounded-xl border border-green-200 bg-white/80 px-3 py-3";
  return href ? <a href={href} className={`${className} hover:bg-white`}>{content}</a> : <div className={className}>{content}</div>;
}
