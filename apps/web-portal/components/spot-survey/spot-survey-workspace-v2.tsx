...
function InfoStrip({ claim }: { claim: SpotSurveyClaim }) { const customerName = claim.customers?.company_name || claim.customers?.contact_name || "-"; const insurer = claim.insurance_companies?.name || "-"; const insurerRef = claim.insurer_claim_no || claim.policies?.policy_no || claim.claim_no; const make = claim.vehicles?.make || "-"; const model = claim.vehicles?.model || "-"; return <section className="overflow-hidden rounded-2xl border border-[#DFE8F4] bg-[#F8FBFF] shadow-[0_6px_18px_rgba(7,29,73,0.03)]"><div className="grid md:grid-cols-3 xl:grid-cols-5"><Info icon="👤" label="Customer" title={customerName} subtitle={claim.customers?.phone ?? "-"} /><Info icon="🚗" label="Vehicle No." title={claim.vehicles?.vehicle_no ?? "-"} /><Info label="Make & Model" title={make} subtitle={model} logo={<ManufacturerLogo name={make} />} /><Info label="Insurer" title={insurer} subtitle={insurerRef} logo={<InsurerLogo name={insurer} />} /><Info icon="📅" label="Loss Date" title={formatDateShort(claim.accident_at)} last /></div><div className="grid border-t border-[#DFE8F4] md:grid-cols-3 xl:grid-cols-6"><Info icon="🧾" label="Policy No." title={claim.policies?.policy_no ?? "-"} /><Info icon="#" label="Control No." title={claim.claim_no} /><Info icon="▣" label="Claim No." title={claim.insurer_claim_no ?? "-"} /><Info icon="✓" label="Claim Status" title={claim.current_status ?? "-"} /><Info icon="📅" label="Spot Intimation Date" title={formatIntimationDate(claim.created_at)} /><Info icon="🕒" label="Spot Intimation Time" title={formatIntimationTime(claim.created_at)} last /></div></section>; }
...
function buildDocumentItems(documents: SpotSurveyDocument[]): Item[] { const matches = (key: keyof typeof aliases) => documents.filter((d) => { const documentType = d.document_type?.toLowerCase() ?? ""; return aliases[key].some((alias) => documentType.includes(alias)); }); const item = (key: keyof typeof aliases, number: number, title: string, icon: string, accent: string, documentType: string): Item => { const matching = matches(key); return { key, number, title, icon, accent, documentType, documents: matching, document: matching.find((d) => d.verification_status !== "rejected") ?? matching[0] ?? null }; }; return [item("spot", 1, "Accident Photo", "📷", "bg-[#EAF4FF]", "Accident Photo"), item("rc", 2, "RC Copy", "📄", "bg-[#F1ECFF]", "RC Copy"), item("insurance", 3, "Insurance Copy", "📃", "bg-[#FFF3D9]", "Insurance Copy"), item("dl", 4, "Driver Licence", "🪪", "bg-[#EAF8EF]", "Driver Licence"), item("gr", 5, "GR / Load Bill", "🚚", "bg-[#FFF1E6]", "GR / Load Bill"), item("video", 6, "Accident Video", "🎥", "bg-[#F2EEFF]", "Accident Video")]; }
/*
