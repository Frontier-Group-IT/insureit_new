...
function InsuranceRows({ incidentDate, values, setValues }: { incidentDate: string | null; values: InsuranceState; setValues: (next: InsuranceState | ((prev: InsuranceState) => InsuranceState)) => void }) {
  ...
  return <div className="divide-y divide-[#E6EEF7]"><div className="grid grid-cols-[52px_1fr_210px_210px] items-center gap-4 px-6 py-4"><NumberBadge number={1} /><p className="text-[14px] font-semibold text-[#071D49]">Insurance Policy Period</p><ReadonlyDateInput label="Start Date" name="insurance_start_date" value={values.start} invalid={dateOrderInvalid || !values.start} /><ReadonlyDateInput label="End Date" name="insurance_end_date" value={values.end} invalid={endStatus === "Invalid" || dateOrderInvalid || !values.end} valid={endStatus === "Valid" && !dateOrderInvalid} /></div><input type="hidden" name="policy_status" value={dateOrderInvalid ? "Invalid" : endStatus} /><InsuranceRow number={2} label="NCB Verification"><select name="ncb_verified" value={values.ncb} onChange={(event) => setValues((prev) => ({ ...prev, ncb: event.target.value }))} className="h-10 w-full rounded-md border border-[#C9D4E3] bg-white px-3 text-[13px] text-[#071D49]"><option value="">Select</option><option value="Yes">Yes</option><option value="No">No</option></select></InsuranceRow><InsuranceRow number={3} label="Hazardous or Non Hazardous Policy"><select name="policy_type_check" value={values.policy} onChange={(event) => setValues((prev) => ({ ...prev, policy: event.target.value }))} className="h-10 w-full rounded-md border border-[#C9D4E3] px-3 text-[13px]"><option value="">Select</option><option>Hazardous</option><option>Non Hazardous</option></select></InsuranceRow><InsuranceRow number={4} label="GVW Mention (in Kgs)"><div className="relative"><input name="gvw_kg" type="number" value={values.gvw} onChange={(event) => setValues((prev) => ({ ...prev, gvw: event.target.value }))} placeholder="Enter GVW" className="h-10 w-full rounded-md border border-[#C9D4E3] px-3 pr-12 text-[13px]" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-[#68758A]">Kgs</span></div></InsuranceRow><div className="px-6 py-3"><div className={`rounded-lg border px-3 py-2 text-[12px] font-semibold ${endStatus === "Invalid" || dateOrderInvalid ? "border-red-200 bg-red-50 text-red-700" : endStatus === "Valid" ? "border-green-200 bg-green-50 text-green-700" : "border-slate-200 bg-slate-50 text-slate-600"}`}>Insurance validity status: {dateOrderInvalid ? "Invalid - start date is after end date" : endStatus || "Policy date unavailable"}</div></div></div>;
}
