...
export function PolicyUnifiedForm({ mode, insurers, rms, sources, initialValues }: Props) {
  ...
  function changeRegistrationMode(mode:VehicleRegistrationMode){ if(isEdit)return; setVehicleRegistrationMode(mode); setLookupError(null); setRcReview(null); setAppliedRc(null); if(mode==="unregistered")setForm(current=>({...current,registrationNo:""})); }

  function fetchRcDetails(){ if(isEdit||vehicleRegistrationMode==="unregistered")return; setLookupError(null);setRcReview(null);startLookup(async()=>{const result=await lookupPolicyRegistrationRc(form.registrationNo);if(!result.ok){setLookupError(result.error);return;}setApplyGroups(defaultGroups);setRcReview(result.review);}); }
  ...
  function submitPolicy(){ if(isEdit){const policyId=initialValues?.policyId;if(!policyId){setSubmitError("Policy reference is missing.");return;}setSubmitError(null);startSubmit(async()=>{const result=await updatePolicyOnboarding(policyId,buildEditPayload());if(!result.ok){setSubmitError(result.error);return;}const billingResult=await savePolicyPayinBilling(policyId,{billNumber:form.payinBillNo,billedAmount:form.payinBilledAmount,billDate:form.payinBillDate,status:form.payinStatus as PolicyPayinStatus});if(!billingResult.ok){setSubmitError(`Policy details were saved, but PayIn billing could not be saved: ${billingResult.error}`);return;}const policyRef=result.policyCode||initialValues?.policyCode||form.policyNo;router.push(`/policies?success=policy_updated&policy=${encodeURIComponent(policyRef||"")}`);router.refresh();});return;}runCreate(buildCreatePayload()); }
    ...
    <div className={`grid gap-4 ${isMotorPolicy?"xl:grid-cols-[minmax(0,1fr)_300px]":""}`}><div className={`space-y-4 ${isMotorPolicy?"":"[&>#policy-section-2]:hidden [&>#policy-section-3]:hidden [&>#policy-section-4]:hidden [&>#policy-section-5]:hidden"}`}>
      ...
      <Section number="02" title="Insured & vehicle identification" subtitle={isEdit?"Linked customer and vehicle details are protected from policy-level edits.":undefined} action={!isEdit?<VehicleRegistrationModeToggle mode={vehicleRegistrationMode} onChange={changeRegistrationMode}/>:null}>
        <div>
          ...
          <div className="flex">
            <input className={`${inputClass} min-w-0 rounded-r-none border-r-0 uppercase focus:z-10`} value={form.registrationNo} onChange={e=>{if(!isEdit&&vehicleRegistrationMode==="registered"){update("registrationNo",e.target.value.toUpperCase());setAppliedRc(null);setLookupError(null);}}} readOnly={isEdit||vehicleRegistrationMode==="unregistered"} disabled={isEdit||vehicleRegistrationMode==="unregistered"} placeholder={vehicleRegistrationMode==="registered"?"MP20AB1234":"Pending after RTO registration"} required={vehicleRegistrationMode==="registered"}/>
            {!isEdit?<button type="button" onClick={fetchRcDetails} disabled={vehicleRegistrationMode==="unregistered"||isLookingUp||form.registrationNo.replace(/[^A-Z0-9]/gi,"").length<6} aria-label={vehicleRegistrationMode==="unregistered"?"RC lookup unavailable for unregistered vehicles":isLookingUp?"Fetching RC details":"Fetch RC details"} title={vehicleRegistrationMode==="unregistered"?"Available after permanent registration number is issued":isLookingUp?"Fetching RC details":"Fetch RC details"} className="group grid h-10 w-11 shrink-0 place-items-center rounded-l-none rounded-r-xl border border-[#17365D] bg-[#17365D] text-white transition hover:bg-[#214A7A] focus:outline-none focus:ring-2 focus:ring-[#DCE8FA] disabled:cursor-not-allowed disabled:border-[#A8B4C3] disabled:bg-[#A8B4C3] disabled:opacity-70">{isLookingUp?<RcFetchSpinner/>:<RcFetchIcon/>}</button>:null}
          </div>
          {vehicleRegistrationMode==="unregistered"?<p className="mt-1 text-[8.5px] font-semibold text-[#315B9A]">RC lookup will be available after permanent registration is issued.</p>:lookupError?<p className="mt-1 text-[8.5px] font-semibold text-red-600">{lookupError}</p>:null}
        </div>
      ...
      <Section number="04" title="Projected insurer pay-in">
        ...
        <Field label="PayIn Bill Date" type="date" value={form.payinBillDate} onChange={e=>update("payinBillDate",e.target.value)}/>
