...
import { markPolicyPayinReceived, savePolicyPayinBilling, type PolicyPayinStatus } from "@/app/policies/policy-payin-billing-actions";
import {
  onboardPolicy,
...
function assessPayinStatus(amount:string,billNumber:string,billDate:string,currentStatus:string){
...
export function PolicyUnifiedForm({ mode, insurers, rms, sources, initialValues }: Props) {
  ...
  const [isMarkingReceived,startMarkReceived]=useTransition();
  ...
  useEffect(()=>{
    setForm(current=>{
      ...
      const nextStatus=assessPayinStatus(nextAmount,current.payinBillNo,current.payinBillDate,current.payinStatus);
  ...
  function markPayinReceived(){const policyId=initialValues?.policyId;if(!isEdit||!policyId){setSubmitError("Save the policy before marking the insurer pay-in as received.");return;}if(form.payinStatus!=="Billed"){setSubmitError("Complete the PayIn bill details before marking the amount as received.");return;}setSubmitError(null);startMarkReceived(async()=>{const result=await markPolicyPayinReceived(policyId,{receivedAmount:form.payinReceivedAmount||form.payinBilledAmount,receivedDate:form.payinReceivedDate||today(),receiptReference:form.payinReceiptReference});if(!result.ok){setSubmitError(result.error);return;}setForm(current=>({...current,payinStatus:"Received",payinReceivedAmount:current.payinReceivedAmount||current.payinBilledAmount,payinReceivedDate:current.payinReceivedDate||today()}));router.refresh();});}
...
function PayinReceiptPanel({status,isEdit,amount,date,reference,busy,onAmountChange,onDateChange,onReferenceChange,onMarkReceived}:{status:string;isEdit:boolean;amount:string;date:string;reference:string;busy:boolean;onAmountChange:(value:string)=>void;onDateChange:(value:string)=>void;onReferenceChange:(value:string)=>void;onMarkReceived:()=>void}){const received=status==="Received";return <div className={`rounded-xl border p-3 ${received?"border-[#B7D8F5] bg-[#F6FAFF]":"border-[#DCE6F2] bg-[#FBFCFE]"}`}><div className="mb-2 flex items-center justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[.06em] text-[#475467]">Insurer receipt</p><p className="mt-0.5 text-[8.5px] text-[#7A8798]">{received?"Receipt confirmed and audit recorded.":isEdit?"Confirm the amount received from the insurer.":"Save the policy first to record receipt."}</p></div>{received?<span className="rounded-full bg-[#E8F3FF] px-2.5 py-1 text-[8px] font-bold text-[#2563A6]">Received</span>:null}</div><div className="grid gap-2 md:grid-cols-[1fr_1fr_1.25fr_auto]"><Field label="Received Amount" type="number" min="0" step="0.01" value={amount} onChange={e=>onAmountChange(e.target.value)} disabled={received||!isEdit}/><Field label="Received Date" type="date" value={date} onChange={e=>onDateChange(e.target.value)} disabled={received||!isEdit}/><Field label="Reference / UTR" value={reference} onChange={e=>onReferenceChange(e.target.value)} placeholder="Optional receipt reference" disabled={received||!isEdit}/>{!received?<div className="flex items-end"><button type="button" onClick={onMarkReceived} disabled={!isEdit||busy} className="h-10 whitespace-nowrap rounded-xl bg-[#2563A6] px-4 text-[9px] font-bold text-white transition hover:bg-[#1F568F] disabled:cursor-not-allowed disabled:opacity-50">{busy?"Confirming…":"Mark as Received"}</button></div>:null}</div></div>;}
