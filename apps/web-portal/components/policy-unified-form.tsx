...
function assessPayinStatus(amount:string,billNumber:string,billDate:string){
  ...
  const hasBillDate=Boolean(billDate.trim());
  if(!hasAmount&&!hasBillNumber&&!hasBillDate)return"Unbilled";
  if(hasAmount&&hasBillNumber&&hasBillDate)return"Billed";
...
export function PolicyUnifiedForm({ mode, insurers, rms, sources, manufacturers = [], initialValues }: Props) {
  ...
  const payinBilledAmountManual=useRef(Boolean(initialValues?.payinBilledAmount));
  ...
  const calculations=useMemo(()=>{ const od=numeric(form.od),tp=numeric(form.tp),cpa=form.cpaOpted==="Yes"?numeric(form.cpa):0,tpWithCpa=tp+cpa; const net=od+tpWithCpa,gst=form.vehicleClass==="GCV"?((od+cpa)*.18)+(tp*.05):net*.18,gross=net+gst; const projectedOd=od*numeric(form.projectedOdPercent)/100,projectedTp=tpWithCpa*numeric(form.projectedTpPercent)/100; const totalPayin=projectedOd+projectedTp+numeric(form.insurerScheme),tds=totalPayin*.10,payinAfterTds=totalPayin-tds; const payoutOd=od*numeric(form.payoutOdPercent)/100,payoutTp=form.payoutBasis==="OD"?0:tpWithCpa*numeric(form.payoutTpPercent)/100; const grossPayout=Math.max(0,payoutOd+payoutTp),indicativeMargin=payinAfterTds-grossPayout; return{net,gst,gross,projectedOd,projectedTp,totalPayin,tds,payinAfterTds,payoutOd,payoutTp,grossPayout,indicativeMargin}; },[form]);
  useEffect(()=>{
    setForm(current=>{
      const nextAmount=payinBilledAmountManual.current?current.payinBilledAmount:(calculations.totalPayin>0?calculations.totalPayin.toFixed(2):"");
      const nextStatus=assessPayinStatus(nextAmount,current.payinBillNo,current.payinBillDate);
      if(nextAmount===current.payinBilledAmount&&nextStatus===current.payinStatus)return current;
      return{...current,payinBilledAmount:nextAmount,payinStatus:nextStatus};
    });
  },[calculations.totalPayin,form.payinBillNo,form.payinBillDate,form.payinBilledAmount]);
  ...
  function submitPolicy(){ if(!isEdit&&vehicleRegistrationMode==="registered"&&!isValidRegisteredVehicleNumber(form.registrationNo)){setRegistrationTouched(true);setSubmitError(registrationValidationMessage);goToSection(1);return;} if(!selectedSourceId){setSubmitError("Select a valid active Partner/POSP/MISP as the Lead Source.");goToSection(0);return;} if(form.vehicleClass==="GCV"&&numeric(form.cpa)<=0){setSubmitError("CPA amount is mandatory for GCV policies and must be greater than ₹0.");goToSection(2);return;} if(isEdit){const policyId=initialValues?.policyId;if(!policyId){setSubmitError("Policy reference is missing.");return;}setSubmitError(null);startSubmit(async()=>{const result=await updatePolicyOnboarding(policyId,buildEditPayload());if(!result.ok){setSubmitError(result.error);return;}const billingResult=await savePolicyPayinBilling(policyId,{billNumber:form.payinBillNo,billedAmount:form.payinBilledAmount,billDate:form.payinBillDate,status:form.payinStatus as PolicyPayinStatus});if(!billingResult.ok){setSubmitError(`Policy details were saved, but billing could not be saved. Please try again.`);return;}const policyRef=result.policyCode||initialValues?.policyCode||form.policyNo;router.push(`/policies?success=policy_updated&policy=${encodeURIComponent(policyRef||"")}`);router.refresh();});return;}runCreate(buildCreatePayload()); }
  ...
  return <div className="mx-auto max-w-[1480px] pb-24">
    ...
    <div className={`grid gap-4 ${isMotorPolicy?"xl:grid-cols-[minmax(0,1fr)_300px]":""}`}><div className={`space-y-4 ${isMotorPolicy?"":"[&>#policy-section-2]:hidden [&>#policy-section-3]:hidden [&>#policy-section-4]:hidden [&>#policy-section-5]:hidden"}`}>
      ...
      <Section number="04" title="Projected insurer pay-in">
        ...
        {billingDetailsOpen?<>
          <PayinBilledAmountField value={form.payinBilledAmount} calculated={calculations.totalPayin} onChange={value=>{payinBilledAmountManual.current=true;update("payinBilledAmount",value);}} onReset={()=>{payinBilledAmountManual.current=false;setForm(current=>({...current,payinBilledAmount:calculations.totalPayin>0?calculations.totalPayin.toFixed(2):""}));}}/>
          <Field label="PayIn Bill No." value={form.payinBillNo} onChange={e=>update("payinBillNo",e.target.value.toUpperCase())} placeholder="Bill / invoice reference"/>
          <DateField label="PayIn Bill Date" value={form.payinBillDate} onChange={value=>update("payinBillDate",value)}/>
          <PayinStatusField status={form.payinStatus}/>
        </>:null}
        <div className="md:col-span-2 xl:col-span-4 flex flex-wrap items-center justify-between gap-2 border-t border-[#E4EAF1] pt-2">
          <button type="button" onClick={()=>setBillingDetailsOpen(open=>!open)} className="inline-flex items-center gap-1.5 text-[9px] font-semibold text-[#526277] transition hover:text-[#17365D]" aria-expanded={billingDetailsOpen}><span className="text-[13px] font-light leading-none">{billingDetailsOpen?"−":"+"}</span>{billingDetailsOpen?"Hide Billing Details":"Billing Details"}</button>
          <PayinOutcomeLine total={calculations.totalPayin} tds={calculations.tds} net={calculations.payinAfterTds}/>
        </div>
      </Section>
...
function PayinBilledAmountField({value,calculated,onChange,onReset}:{value:string;calculated:number;onChange:(value:string)=>void;onReset:()=>void}){return <div><div className="mb-1.5 flex items-center justify-between gap-2"><label className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[0.055em] text-[#475467]">PayIn Billed Amt Rs.</label><button type="button" onClick={onReset} className="text-[8px] font-semibold text-[#315B9A] transition hover:text-[#17365D]">Reset to auto</button></div><input className={inputClass} type="number" min="0" step="0.01" value={value} onChange={e=>onChange(e.target.value)} placeholder="₹ 0.00"/><p className="mt-1.5 px-0.5 text-[8.5px] text-[#7A8798]">Default: OD PayIn + TP/CPA PayIn + scheme · {money.format(calculated)}</p></div>;}
function PayinStatusField({status}:{status:string}){const tone=status==="Billed"?"border-[#BFE8D5] bg-[#F0FBF6] text-[#14845B]":status==="Billing details incomplete"?"border-[#F1D59A] bg-[#FFF9EB] text-[#A96A00]":"border-[#E1E7EF] bg-[#F8FAFC] text-[#667085]";return <div><label className={labelClass}>PayIn Status</label><div className={`flex h-10 items-center rounded-xl border px-3 ${tone}`}><span className="text-[10px] font-bold">{status}</span><span className="ml-auto text-[7.5px] font-semibold uppercase tracking-[.05em] opacity-70">Auto</span></div></div>;}
function PayinOutcomeLine({total,tds,net}:{total:number;tds:number;net:number}){return <div className="flex flex-wrap items-center justify-end gap-x-3 gap-y-1 text-[9px]"><span className="font-bold uppercase tracking-[.06em] text-[#667085]">Pay-in outcome</span><span className="text-[#98A2B3]">Total <strong className="font-semibold text-[#17365D]">{money.format(total)}</strong></span><span className="text-[#CBD2DC]">|</span><span className="text-[#98A2B3]">TDS <strong className="font-semibold text-[#17365D]">{money.format(tds)}</strong></span><span className="text-[#CBD2DC]">|</span><span className="text-[#667085]">Net <strong className="font-bold text-[#315B9A]">{money.format(net)}</strong></span></div>;}
function SettlementField({date,voucher,onDateChange,onVoucherChange}:{date:string;voucher:string;onDateChange:(value:string)=>void;onVoucherChange:(value:string)=>void}){return <div><label className={labelClass}>Settlement</label><div className="grid h-10 grid-cols-[.88fr_1.12fr] overflow-hidden rounded-xl border border-[#D8DEE9] bg-white transition hover:border-[#B8C2D1] focus-within:border-[#315B9A] focus-within:ring-2 focus-within:ring-[#DCE8FA]"><label className="relative min-w-0 border-r border-[#E1E6ED]"><span className="pointer-events-none absolute left-3 top-1 z-10 text-[7px] font-bold uppercase tracking-[.06em] text-[#7A8798]">Date</span><FormattedDateInput ariaLabel="Payout date" value={date} onChange={onDateChange} className="h-full w-full border-0 bg-transparent px-3 pb-0.5 pt-3 text-[9.5px] font-medium text-[#17203A] outline-none"/></label><label className="relative min-w-0"><span className="pointer-events-none absolute left-3 top-1 text-[7px] font-bold uppercase tracking-[.06em] text-[#7A8798]">Voucher</span><input aria-label="Payout voucher number" value={voucher} onChange={e=>onVoucherChange(e.target.value)} placeholder="Reference" className="h-full w-full border-0 bg-transparent px-3 pb-0.5 pt-3 text-[9.5px] font-medium text-[#17203A] outline-none placeholder:text-[#B0BAC8]"/></label></div></div>;}
