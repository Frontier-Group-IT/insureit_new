...
export function PolicyUnifiedForm({ mode, insurers, customers = [], rms, sources, manufacturers = [], initialValues, nonMotorInitialValues, commercialAccess = true, preselectedCustomerId = null, sourceIntakeId = null, initialDraftRevision = null, initialRegistrationMode = "registered" }: Props) {
  ...
  const isCpmClass=form.vehicleClass==="CPM";
    ...
    {isNonMotorPolicy?<NonMotorUnifiedMode mode={mode} policyId={initialValues?.policyId} initialValues={nonMotorInitialValues} sourceSection={sourceSection} source={{issuanceDate:form.issuanceDate,intermediaryType:form.intermediaryType,sourceId:selectedSourceId,leadSource:form.leadSource,intermediaryCode:form.intermediaryCode,rmName:form.rmName}} insurers={insurers} customers={customers} sources={sources} onProgressChange={setNonMotorProgress}/>:isMotorPolicy?<>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_336px]"><div className="space-y-4">
        ...
        <Section number="03" title="Policy product, premium & validity">
          ...
          <Field label="TP premium" type="number" min="0" value={isCpmClass?"0":form.tp} onChange={e=>update("tp",e.target.value)} placeholder={isCpmClass?"Not applicable for CPM":"₹ 0.00"} disabled={isCpmClass} required={!isCpmClass}/>
          <Field label="CPA amount" type="number" min={form.vehicleClass==="GCV"?"0.01":"0"} step="0.01" value={isCpmClass?"0":form.cpa} onChange={e=>{const value=e.target.value;setForm(current=>({...current,cpa:value,cpaOpted:Number(value||0)>0?"Yes":"No"}));}} placeholder={isCpmClass?"Not applicable for CPM":"₹ 0.00"} disabled={isCpmClass} required={!isCpmClass&&form.vehicleClass==="GCV"}/>
