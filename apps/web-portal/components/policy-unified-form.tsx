...
function stateFrom(values?: PolicyUnifiedInitialValues): FormState {
  return {
    ...
    policyProduct: values?.policyProduct ?? "", idv: values?.idv ?? "", od: values?.od ?? "", tp: values?.tp ?? "", cpaOpted: values?.cpaOpted ?? "Yes", cpa: values?.cpa ?? "", policyNo: values?.policyNo ?? "", insurerId: values?.insurerId ?? "", validFrom: values?.validFrom ?? "", validUpto: values?.validUpto ?? "",
...
export function PolicyUnifiedForm({ mode, insurers, rms, sources, initialValues }: Props) {
  ...
  const calculations=useMemo(()=>{ const od=numeric(form.od),tp=numeric(form.tp),cpa=form.cpaOpted==="Yes"?numeric(form.cpa):0; const net=od+tp+cpa,gst=form.vehicleClass==="GCV"?((od+cpa)*.18)+(tp*.05):net*.18,gross=net+gst; const projectedOd=od*numeric(form.projectedOdPercent)/100,projectedTp=tp*numeric(form.projectedTpPercent)/100; const totalPayin=projectedOd+projectedTp+numeric(form.insurerScheme),tds=totalPayin*.10,payinAfterTds=totalPayin-tds; const payoutOd=od*numeric(form.payoutOdPercent)/100,payoutTp=form.payoutBasis==="OD"?0:tp*numeric(form.payoutTpPercent)/100; return{net,gst,gross,projectedOd,projectedTp,totalPayin,tds,payinAfterTds,grossPayout:Math.max(0,payoutOd+payoutTp-numeric(form.retention))}; },[form]);
  ...
  return <div className="mx-auto max-w-[1480px] pb-24">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]"><div className="space-y-4">
      ...
      <Section number="02" title="Insured & vehicle identification" subtitle={isEdit?"Linked customer and vehicle details are protected from policy-level edits.":"Verify the registration, then review and apply approved RC details."} badge={isEdit?"Linked master · read-only":"AuthBridge API"}>
        <div>
      ...
      <Section number="03" title="Policy product, premium & validity" subtitle="Premium calculations are saved with calculation version prototype_v1." badge="Manual + calculated"><Select label="Policy product" value={form.policyProduct} onChange={e=>update("policyProduct",e.target.value)} options={policyProducts} placeholder="Select product" disabled={!form.vehicleClass} required/><Field label="IDV / Sum insured" type="number" min="0" value={form.idv} onChange={e=>update("idv",e.target.value)} placeholder="₹ 0.00" required/><Field label="OD premium" type="number" min="0" value={form.od} onChange={e=>update("od",e.target.value)} placeholder="₹ 0.00" required/><Field label="Third party premium" type="number" min="0" value={form.tp} onChange={e=>update("tp",e.target.value)} placeholder="₹ 0.00" required/><Select label="CPA opted" value={form.cpaOpted} onChange={e=>update("cpaOpted",e.target.value as "Yes"|"No")} options={["Yes","No"]} placeholder="Select"/><Field label="CPA amount" type="number" min="0" value={form.cpaOpted==="Yes"?form.cpa:"0"} onChange={e=>update("cpa",e.target.value)} disabled={form.cpaOpted==="No"} placeholder="₹ 0.00"/><PremiumCalculationBand net={calculations.net} gst={calculations.gst} gross={calculations.gross} gstRule={form.vehicleClass==="GCV"?"18% OD + CPA · 5% TP":"18% on Net"}/><Field label="Policy number" value={form.policyNo} onChange={e=>update("policyNo",e.target.value.toUpperCase())} placeholder="Policy number" required/><div><label className={labelClass}>Insurance company <Required/></label><select className={inputClass} value={form.insurerId} onChange={e=>update("insurerId",e.target.value)} required><option value="">Select insurer</option>{insurers.map(i=><option key={i.value} value={i.value}>{i.label}</option>)}</select></div><Field label="Valid from" type="date" value={form.validFrom} onChange={e=>update("validFrom",e.target.value)} required/><Field label="Valid upto" type="date" value={form.validUpto} onChange={e=>update("validUpto",e.target.value)} required/></Section>

      <Section number="04" title="Projected insurer pay-in" subtitle="Projected receivable and billing values are saved separately." badge="prototype_v1"><PercentField label="Projected OD pay-in %" value={form.projectedOdPercent} onChange={v=>update("projectedOdPercent",v)}/><CalculatedOutcome label="Projected OD amount" value={money.format(calculations.projectedOd)}/><PercentField label="Projected TP pay-in %" value={form.projectedTpPercent} onChange={v=>update("projectedTpPercent",v)}/><CalculatedOutcome label="Projected TP amount" value={money.format(calculations.projectedTp)}/><Field label="Any insurer scheme" type="number" min="0" value={form.insurerScheme} onChange={e=>update("insurerScheme",e.target.value)} placeholder="₹ 0.00"/><PayinCalculationBand total={calculations.totalPayin} tds={calculations.tds} afterTds={calculations.payinAfterTds}/><Field label="Retention" type="number" min="0" value={form.retention} onChange={e=>update("retention",e.target.value)} placeholder="₹ 0.00"/></Section>

      <Section number="05" title="Intermediary payout & settlement" subtitle="Stores the proposed partner payout and settlement status." badge="Finance workflow"><PercentField label="Payout OD %" value={form.payoutOdPercent} onChange={v=>update("payoutOdPercent",v)}/><PercentField label="Payout TP %" value={form.payoutTpPercent} onChange={v=>update("payoutTpPercent",v)} disabled={form.payoutBasis==="OD"}/><CalculatedOutcome label="Gross partner payout" value={money.format(calculations.grossPayout)} accent/><Select label="Payout status" value={form.payoutStatus} onChange={e=>update("payoutStatus",e.target.value)} options={["Pending","Approved","On Hold","Processed","Paid","Cancelled"]} placeholder="Select status"/><Field label="Payout date" type="date" value={form.payoutDate} onChange={e=>update("payoutDate",e.target.value)}/><Field label="Payout voucher number" value={form.payoutVoucherNo} onChange={e=>update("payoutVoucherNo",e.target.value.toUpperCase())} placeholder="Voucher / reference"/><div className="md:col-span-2 xl:col-span-4"><label className={labelClass}>Remarks</label><textarea className="min-h-20 w-full rounded-xl border border-[#D8DEE9] px-3 py-2 text-[11px] outline-none focus:border-[#315B9A]" value={form.remarks} onChange={e=>update("remarks",e.target.value)} placeholder="Add policy, billing or payout notes"/></div></Section>
    </div><LiveSummary net={calculations.net} gst={calculations.gst} gross={calculations.gross} payinAfterTds={calculations.payinAfterTds} grossPayout={calculations.grossPayout}/></div>
...
function Tag({text,tone}:{text:string;tone:"amber"|"green"}){return <span className={`rounded px-1.5 py-0.5 text-[7px] font-bold normal-case ${tone==="amber"?"bg-amber-50 text-amber-700":"bg-emerald-50 text-emerald-700"}`}>{text}</span>;}
