...
export function PolicyUnifiedForm({ mode, insurers, rms, sources, initialValues }: Props) {
  ...
  const calculations=useMemo(()=>{ const od=numeric(form.od),tp=numeric(form.tp),cpa=form.cpaOpted==="Yes"?numeric(form.cpa):0; const net=od+tp+cpa,gst=form.vehicleClass==="GCV"?((od+cpa)*.18)+(tp*.05):net*.18,gross=net+gst; const projectedOd=od*numeric(form.projectedOdPercent)/100,projectedTp=tp*numeric(form.projectedTpPercent)/100; const totalPayin=projectedOd+projectedTp+numeric(form.insurerScheme),tds=totalPayin*.10,payinAfterTds=totalPayin-tds; const payoutOd=od*numeric(form.payoutOdPercent)/100,payoutTp=form.payoutBasis==="OD"?0:tp*numeric(form.payoutTpPercent)/100; const grossPayout=Math.max(0,payoutOd+payoutTp-numeric(form.retention)),indicativeMargin=payinAfterTds-grossPayout; return{net,gst,gross,projectedOd,projectedTp,totalPayin,tds,payinAfterTds,grossPayout,indicativeMargin}; },[form]);
...
function CalculatedSubline({value}:{value:string}){return <div className="mt-1.5 flex items-center gap-1.5 px-0.5 text-[9px]"><span className="font-semibold text-[#315B6B]">{value}</span><span className="text-[7.5px] font-medium uppercase tracking-[.05em] text-[#98A2B3]">Calculated</span></div>;}
