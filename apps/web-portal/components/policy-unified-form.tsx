...
export function PolicyUnifiedForm({ mode, insurers, rms, sources, manufacturers = [], initialValues, commercialAccess = true }: Props) {
  ...
  return <div className="mx-auto max-w-[1480px] pb-24">
    ...
    <div className={`grid gap-4 ${isMotorPolicy?"xl:grid-cols-[minmax(0,1fr)_300px]":""}`}><div className={`space-y-4 ${isMotorPolicy?"":"[&>#policy-section-2]:hidden [&>#policy-section-3]:hidden"}`}>
      ...
      {isMotorPolicy?<CommercialCard access={commercialAccess} payinEntered={payinEntered} payoutEntered={payoutEntered} totalPayin={calculations.totalPayin} totalPayout={calculations.grossPayout} onOpen={setCommercialModal}/>:null}
...
function CommercialCard({access,payinEntered,payoutEntered,totalPayin,totalPayout,onOpen}:{access:boolean;payinEntered:boolean;payoutEntered:boolean;totalPayin:number;totalPayout:number;onOpen:(kind:Exclude<CommercialModal,null>)=>void}){return <section className="overflow-hidden rounded-2xl border border-[#D9E2F0] bg-white shadow-sm"><div className="flex items-center justify-between border-b border-[#E5EAF1] bg-[#FBFCFE] px-4 py-3"><div><h2 className="text-[13px] font-semibold text-[#17365D]">Commercials</h2><p className="mt-0.5 text-[9px] text-[#667085]">Optional sensitive terms. They do not affect policy onboarding completion.</p></div><span className={`rounded-full px-2.5 py-1 text-[8px] font-bold ${access?"bg-[#EAF7F2] text-[#18794E]":"bg-[#F2F4F7] text-[#667085]"}`}>{access?"Authorized":"Restricted"}</span></div>{access?<div className="grid gap-3 p-4 sm:grid-cols-2"><button type="button" onClick={()=>onOpen("payin")} className="rounded-xl border border-[#D8E2EF] bg-[linear-gradient(135deg,#F8FBFF,#F1F6FC)] px-4 py-4 text-left transition hover:border-[#9FB5D3] hover:shadow-sm"><div className="flex items-center justify-between gap-3"><div className="text-[10px] font-bold text-[#17365D]">Projected Pay-in</div><span className={`rounded-full px-2 py-1 text-[7.5px] font-bold ${payinEntered?"bg-[#EAF7F2] text-[#18794E]":"bg-[#F2F4F7] text-[#667085]"}`}>{payinEntered?money.format(totalPayin):"Not entered"}</span></div><div className="mt-1 text-[9px] leading-4 text-[#667085]">Expected OD, TP/CPA and scheme pay-in. 0% is a valid rate.</div><div className="mt-3 text-[9px] font-bold text-[#315B9A]">Open details →</div></button><button type="button" onClick={()=>onOpen("payout")} className="rounded-xl border border-[#D8E2EF] bg-[linear-gradient(135deg,#FBFCFF,#F6F4FF)] px-4 py-4 text-left transition hover:border-[#B9AED8] hover:shadow-sm"><div className="flex items-center justify-between gap-3"><div className="text-[10px] font-bold text-[#17365D]">Partner Payout</div><span className={`rounded-full px-2 py-1 text-[7.5px] font-bold ${payoutEntered?"bg-[#EAF7F2] text-[#18794E]":"bg-[#F2F4F7] text-[#667085]"}`}>{payoutEntered?money.format(totalPayout):"Not entered"}</span></div><div className="mt-1 text-[9px] leading-4 text-[#667085]">Actual agreed OD and TP/CPA payout commercial. 0% is valid.</div><div className="mt-3 text-[9px] font-bold text-[#5D4E9C]">Open details →</div></button></div>:<div className="p-4"><div className="rounded-xl border border-dashed border-[#D7DDE6] bg-[#F8FAFC] px-4 py-4 text-[10px] font-semibold text-[#667085]">Commercial details restricted</div></div>}</section>;}

function ProjectedPayinModal({form,update,calculations,onClose}:{form:FormState;update:<K extends keyof FormState>(key:K,value:FormState[K])=>void;calculations:{projectedOd:number;projectedTp:number;totalPayin:number};onClose:()=>void}){return <ModalShell title="Projected Insurer Pay-in" subtitle="Commercial expectation only. This does not create billing or confirm what the insurer will actually pay." onClose={onClose} footer={<div className="flex justify-end"><button type="button" onClick={onClose} className="rounded-xl bg-[#17365D] px-5 py-2.5 text-[10px] font-bold text-white">Save & Close</button></div>}><div className="grid gap-3 sm:grid-cols-2"><div><PercentField label="OD Pay-in %" value={form.projectedOdPercent} onChange={v=>update("projectedOdPercent",v)}/><CalculatedSubline value={money.format(calculations.projectedOd)}/></div><div><PercentField label="TP/CPA Pay-in %" value={form.projectedTpPercent} onChange={v=>update("projectedTpPercent",v)}/><CalculatedSubline value={money.format(calculations.projectedTp)}/></div><Field label="Insurer scheme" type="number" min="0" value={form.insurerScheme} onChange={e=>update("insurerScheme",e.target.value)} placeholder="₹ 0.00"/><CalculatedField label="Projected total pay-in" value={money.format(calculations.totalPayin)}/></div><p className="mt-4 rounded-xl border border-[#DCE6F2] bg-[#F8FAFD] px-3 py-2.5 text-[9px] leading-4 text-[#667085]">Blank means the commercial has not been entered. Entering 0 explicitly is valid and is preserved as 0%.</p></ModalShell>;}

function PartnerPayoutModal({form,update,calculations,onClose}:{form:FormState;update:<K extends keyof FormState>(key:K,value:FormState[K])=>void;calculations:{payoutOd:number;payoutTp:number;grossPayout:number};onClose:()=>void}){return <ModalShell title="Partner Payout" subtitle="Actual agreed payout commercial for this policy. It remains independent from insurer actual pay-in." onClose={onClose} footer={<div className="flex justify-end"><button type="button" onClick={onClose} className="rounded-xl bg-[#17365D] px-5 py-2.5 text-[10px] font-bold text-white">Save & Close</button></div>}><div className="grid gap-3 sm:grid-cols-2"><div><PercentField label="Payout OD %" value={form.payoutOdPercent} onChange={v=>update("payoutOdPercent",v)}/><CalculatedSubline value={money.format(calculations.payoutOd)}/></div><div><PercentField label="Payout TP/CPA %" value={form.payoutTpPercent} onChange={v=>update("payoutTpPercent",v)} disabled={form.payoutBasis==="OD"}/><CalculatedSubline value={money.format(calculations.payoutTp)}/></div><Select label="Payout basis" value={form.payoutBasis} onChange={e=>update("payoutBasis",e.target.value)} options={["NET","OD"]} placeholder="Select basis"/><CalculatedField label="Total agreed payout" value={money.format(calculations.grossPayout)}/></div><p className="mt-4 rounded-xl border border-[#E4DFF2] bg-[#FAF8FF] px-3 py-2.5 text-[9px] leading-4 text-[#667085]">Payment status, voucher and settlement date are intentionally not part of policy onboarding. They belong to the payout/payment workflow.</p></ModalShell>;}
