...
export function PartnerNavigation({ hideAccount = false }: { hideAccount?: boolean }) {
  const visibleAccountItems = hideAccount ? accountItems.filter((item) => item.href !== "/partner/account") : accountItems;
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-[268px] overflow-hidden border-r border-white/10 bg-[#111a35] text-white shadow-[20px_0_60px_rgba(17,26,53,0.22)] lg:flex lg:flex-col">
      ...
      <nav className="relative flex-1 overflow-y-auto px-3.5 py-4" aria-label="Partner workspace navigation"><NavGroup label="Partner Workspace" items={primaryItems} /><NavGroup label="Commercial" items={secondaryItems} /><NavGroup label="Account" items={visibleAccountItems} /></nav>
    </aside>
  );
}
