...
const primaryItems: PartnerNavItem[] = [
  ...
  { href: "/partner/vehicles", label: "Vehicle", icon: "vehicle" },
...
function RenewalsNav({ item }: { item: PartnerNavItem }) {
  ...
  return (
    <div>
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className={`group flex min-h-11 w-full items-center gap-3 rounded-xl px-3.5 text-left text-[12px] font-bold transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/45 ${renewalsActive ? "bg-white text-[#141d3b] shadow-[0_3px_12px_rgba(5,18,45,0.12)]" : "text-white/88 hover:bg-white/8 hover:text-white"}`}>
        <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg transition-colors duration-200 ${renewalsActive ? "bg-[#EAF1FF]" : "group-hover:bg-white/8"}`}><PartnerCustomIcon name={item.icon} size={22} className="h-[22px] w-[22px]" /></span>
      ...
      {open ? (
        <div className="ml-[29px] mt-1.5 border-l border-white/35 pl-4"><div className="space-y-1">
          <Link href="/partner/renewals" prefetch={false} className={`block rounded-lg px-3 py-2 text-[11px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35 ${internalActive ? "bg-white/14 text-white" : "text-white/72 hover:bg-white/8 hover:text-white"}`}>Internal Renewal</Link>
          <Link href="/partner/renewals/external" prefetch={false} className={`block rounded-lg px-3 py-2 text-[11px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35 ${externalActive ? "bg-white/14 text-white" : "text-white/72 hover:bg-white/8 hover:text-white"}`}>External Renewal</Link>
        </div></div>
      ) : null}
    </div>
  );
...
export function PartnerNavigation() {
  return (
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-[268px] overflow-hidden border-r border-white/10 bg-[#111a35] text-white shadow-[20px_0_60px_rgba(17,26,53,0.22)] lg:flex lg:flex-col">
      ...
      <Link href="/partner" className="relative flex h-[78px] items-center border-b border-white/10 px-5" aria-label="INSUREIT Partner home"><BrandLockup compact inverse /></Link>
      <nav className="relative flex-1 overflow-y-auto px-3.5 py-4" aria-label="Partner workspace navigation"><NavGroup label="Partner Workspace" items={primaryItems} /><NavGroup label="Commercial" items={secondaryItems} /><NavGroup label="Account" items={accountItems} /></nav>
    </aside>
  );
}
