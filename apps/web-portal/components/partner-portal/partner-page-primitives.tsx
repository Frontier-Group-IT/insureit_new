  ...
  return (
    <div className="flex flex-col gap-3 border-b border-[#DCE4ED] pb-4 sm:flex-row sm:items-end sm:justify-between sm:gap-5">
      <div className="min-w-0">
        ...
        <h2 className="mt-1 break-words text-[22px] font-extrabold leading-tight tracking-[-0.025em] text-[#142541] sm:text-[24px]">{title}</h2>
        {description ? <p className="mt-1 max-w-3xl break-words text-[10.5px] font-medium leading-5 text-[#71819A]">{description}</p> : null}
      </div>
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
      <div className="min-w-0">
        ...
        <h3 className="mt-0.5 break-words text-[14px] font-extrabold leading-5 tracking-[-0.01em] text-[#182A47] sm:text-[15px]">{title}</h3>
        {description ? <p className="mt-1 break-words text-[10px] font-medium leading-4 text-[#7A899F]">{description}</p> : null}
      </div>
      {action ? <div className="w-full sm:w-auto sm:shrink-0">{action}</div> : null}
    </div>
  );
    ...
    <div className={"grid border-y border-[#DCE4ED] sm:grid-cols-2 " + grid}>
      {items.map((item, index) => (
        ...
        >
          ...
          <div className="mt-1.5 break-words text-[18px] font-extrabold leading-tight tracking-[-0.025em] text-[#162746] sm:text-[20px]">{item.value}</div>
          {item.meta ? <div className="mt-1 break-words text-[9.5px] font-medium leading-4 text-[#8794A7]">{item.meta}</div> : null}
        </div>
      ))}
    </div>
  );
