      ...
      <header className="flex min-h-14 items-center justify-between gap-3 border-b border-white/15 bg-[#071D49] px-4 py-2 text-white sm:px-5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            ...
            disabled={isPending}
            className="hidden h-9 items-center gap-2 rounded-xl border border-white/25 px-3 text-[9.5px] font-semibold text-white hover:bg-white/10 disabled:opacity-60 sm:inline-flex"
          >
          ...
          <button
            type="button"
            onClick={close}
            className="grid h-9 w-9 place-items-center rounded-xl border border-white/25 text-white hover:bg-white/10"
  ...
  return (
    <>
      <button
        ...
        className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#111A35] px-4 text-[10.5px] font-bold text-white transition hover:bg-[#182549] disabled:cursor-not-allowed disabled:opacity-50"
      >
        {isPending ? "Preparing secure session…" : buttonLabel}
        {!isPending ? <ChevronRight className="h-4 w-4" /> : null}
      </button>
      {error ? <div className="mt-2 max-w-lg rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[9.5px] font-semibold text-red-700">{error}</div> : null}
