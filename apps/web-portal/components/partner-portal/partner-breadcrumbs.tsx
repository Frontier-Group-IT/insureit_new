  ...
  return (
    <div className="flex min-w-0 items-center gap-1.5 sm:gap-2" data-partner-route-breadcrumbs="true">
      <button
        ...
        onClick={() => router.back()}
        aria-label="Go back"
        className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-[#BFCDE0]/55 bg-[#B9CCE4]/32 text-[#35557C] transition hover:bg-[#B4C8E0]/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5A96E8]/20"
      >
        <ArrowLeft className="h-4 w-4" />
      </button>

      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-white/80 bg-white/45 text-[#456486]" aria-hidden="true">
      ...
      <nav aria-label="Partner page path" className="flex min-w-0 items-center gap-1.5 overflow-hidden sm:gap-2">
        {items.map((item, index) => {
          ...
          const pillClass = "relative inline-flex h-9 min-w-0 items-center rounded-xl border border-white/80 bg-[#D9E5F4]/42 px-3 text-[12px] font-semibold text-[#203F64] before:absolute before:bottom-1.5 before:left-0 before:top-1.5 before:w-[3px] before:rounded-r-full before:bg-[#2F91FF]/85 sm:px-4 sm:text-[13px]";

          return (
            <div key={`${item.label}-${index}`} className="flex min-w-0 items-center gap-1.5 sm:gap-2">
              {index > 0 ? (
                <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-white/38 text-[#7C8FA8]" aria-hidden="true">
                  <ChevronRight className="h-3.5 w-3.5" />
                </span>
              ) : null}
              {item.href && !current ? (
                <Link href={item.href} prefetch={false} className={`${pillClass} transition hover:bg-[#D2E1F1]/58`}>
