...
export default async function VehicleReadOnlyPage({
  ...
  const [vehicleResult, policiesResult] = await Promise.all([
    admin
      .from("vehicles")
      .select("id,customer_id,vehicle_no,vehicle_type,make,model,year,chassis_no,engine_no,fuel_type,registration_status,registration_date,fitness_expiry_date,puc_expiry_date,road_tax_expiry_date,national_permit_expiry_date,local_permit_expiry_date,engine_capacity_cc,seating_capacity,gvw_kg,permit_no,customers:customers!vehicles_customer_id_fkey(customer_code,contact_name,phone)")
      .eq("id", id)
