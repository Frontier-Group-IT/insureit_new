...
import { claimStatuses, isCustomerActionAwaited, isDocumentVerificationPending, isManagerActionRequired, isOpenClaimStatus, operationsQueueForKey, terminalClaimStatuses, type ClaimStatus } from "@/lib/claim-workflow";
...
export function ClaimsWorkspace({ rows, initialParams, loadError, canAddClaim }: { rows: QueueClaimRow[]; initialParams: SearchParams; loadError: string | null; canAddClaim: boolean }) {
  ...
  const visibleRows = useMemo(() => rows.filter((claim) => {
    const journeyStage = projectInternalClaim(claim.current_status).stageLabel;
    const haystack = [claim.claim_no, claim.insurer_claim_no, claim.current_status, journeyStage, claim.customers?.company_name, claim.customers?.contact_name, claim.customers?.phone, claim.vehicles?.vehicle_no, claim.vehicles?.make, claim.vehicles?.model, claim.policies?.policy_no, claim.insurance_companies?.name, claim.assignee?.full_name].filter(Boolean).join(" ").toLowerCase();
    ...
    <div className="overflow-hidden rounded-lg border border-[#E1E7F0] bg-white shadow-[0_8px_22px_rgba(7,29,73,0.045)]">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1120px] border-separate border-spacing-y-0 text-left text-[11px] leading-tight text-[#071D49]">
          ...
          <tbody>{visibleRows.length ? visibleRows.map((claim) => {
            ...
            const insurerName = claim.insurance_companies?.name ?? null;
            const insurerLogo = getInsurerLogo(insurerName);
            return <tr key={claim.id} className="group bg-white align-middle shadow-[0_1px_0_rgba(226,232,240,0.86)] transition hover:bg-[#F8FBFF]"><td className="border-r border-[#E7ECF3] px-2 py-2"><div className="min-w-0"><span className="block truncate font-medium">{customer}</span><span className="block truncate text-[10px] text-[#344256]">{claim.customers?.phone ?? "-"}</span></div></td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center">{claim.vehicles?.vehicle_no ?? "-"}</td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center">{[claim.vehicles?.make, claim.vehicles?.model].filter(Boolean).join(" ") || "-"}</td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center">{formatDate(claim.accident_at ?? claim.created_at)}</td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center"><span className="flex items-center justify-center">{insurerLogo ? <Image src={insurerLogo} alt={insurerName ? `${insurerName} logo` : "Insurance company"} width={32} height={32} className="max-h-7 max-w-[34px] w-auto object-contain" /> : <ShieldCheck className="h-4 w-4 text-[#7E91A8]" />}</span></td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center">{claim.policies?.policy_no ?? "-"}</td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center">{claim.claim_no}</td><td className="border-r border-[#E7ECF3] px-2 py-2 text-center">{claim.insurer_claim_no ?? "-"}</td><td className="border-r border-[#E7ECF3] px-2 py-2"><span className="block">{customerProjection.stageLabel}</span><span className="mt-0.5 block text-[9px] text-[#65748A]">{customerProjection.nextActionOwner === "customer" ? "Customer action" : customerProjection.nextActionOwner === "operations" ? "Operations action" : "Complete"}</span></td><td className="px-2 py-2 text-center">{isExternalClaim(claim) ? <ExternalClaimOperationsEntry claimId={claim.id} /> : <Link prefetch={false} href={`/claims/${claim.id}`} className="inline-flex h-7 items-center justify-center rounded-md bg-[#003A83] px-3 text-[10.5px] font-medium text-white">Proceed</Link>}</td></tr>;
          }) : <tr><td className="px-3 py-8 text-center text-sm text-slate-500" colSpan={10}>No matching claims found.</td></tr>}</tbody>
        </table>
      </div>
    </div>
