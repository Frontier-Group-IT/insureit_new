...
const rcDateFields = ["fitness_valid_upto", "tax_valid_upto", "insurance_valid_upto", "local_permit_valid_upto"] as const;
...
export async function verifySpotSurveyDocument(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const { profile, claim, supabase, isPartner } = await context(claimId);
    ...
    const attempts = documentsToVerify.map((document) => {
      const { finalDetails, isValid, invalidReason } = applyAutomaticValidity(rawDetails, incidentDate, document.document_type);
      const detailsPayload = { verification_type: "spot_survey_document", document_type: document.document_type, document_id: document.id, incident_date: incidentDate, is_valid: isValid, invalid_reason: invalidReason, ...finalDetails, verified: isValid, verified_at: verifiedAt };
    ...
    for (const attempt of attempts) {
      await insertAuthorizedClaimStageDetail({ isPartner, supabase }, { claim_id: claimId, stage: claim.current_status, details: attempt.detailsPayload, created_by: profile.id });
    }
...
export async function requestSpotSurveyDocumentReupload(formData: FormData): Promise<ActionResult> {
  try {
    ...
    await insertAuthorizedClaimStageDetail({ isPartner, supabase }, { claim_id: claimId, stage: claim.current_status, details: detailsPayload, created_by: profile.id });
...
export async function verifySpotSurveyDetail(formData: FormData): Promise<ActionResult> {
  try {
    ...
    await insertAuthorizedClaimStageDetail({ isPartner, supabase }, { claim_id: claimId, stage: claim.current_status, details: detailPayload, created_by: profile.id });
