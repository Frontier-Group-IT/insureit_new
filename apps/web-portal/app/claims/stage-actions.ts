...
type StageDetailsRow = { details: Record<string, unknown> | null; created_at: string };

const orderedStageKeys: readonly OperationsStageKey[] = ["spot_status", "claim_intimation", "work_approval", "repair_ri", "billing", "delivery_order", "vehicle_delivery", "payment_encashment"];
...
function booleanValue(formData: FormData, name: string) { return textValue(formData, name) === "true"; }
function isStageKey(value: string | null): value is OperationsStageKey { return Boolean(value && value in completionTargets); }
function currentStageKey(status: ClaimStatus) { return orderedStageKeys.find((key) => stageStatuses[key].includes(status)) ?? null; }
...
function stageDetailsFromForm(formData: FormData, stageKey: OperationsStageKey) {
  ...
  for (const [key, value] of formData.entries()) {
    ...
    details[key] = Number.isFinite(numericValue) && /amount|tds|gst|labour|parts|bill|received|estimate/i.test(key) ? numericValue : value.trim();
  }
...
function detailText(details: Record<string, unknown> | null | undefined, key: string) { const value = details?.[key]; return typeof value === "string" ? value.trim() : ""; }
function spotSurveyorDetailsChanged(previousDetails: Record<string, unknown> | null | undefined, formData: FormData) {
  return detailText(previousDetails, "surveyor_name") !== (textValue(formData, "surveyor_name") ?? "")
    || detailText(previousDetails, "surveyor_email") !== (textValue(formData, "surveyor_email") ?? "")
    || detailText(previousDetails, "surveyor_phone") !== (textValue(formData, "surveyor_phone") ?? "");
}
...
export async function completeClaimJourneyStage(claimId: string, formData: FormData) {
  ...
  for (const field of requiredFields[stageKey]) if (!textValue(formData, field)) throw new Error("Complete all mandatory stage fields before saving details.");
  if (stageKey === "vehicle_delivery" && booleanValue(formData, "vehicle_received") && !textValue(formData, "vehicle_received_date")) throw new Error("Vehicle Received Date is mandatory when the vehicle is received.");

  const { data: claim, error: claimError } = await supabase.from("claims").select("id,current_status,claim_service_mode,policy_service_source").eq("id", claimId).maybeSingle<ManagedClaim>();
  ...
  if (stageKey === "spot_status" && claim.policy_service_source !== "external") {
    for (const field of internalSpotStatusSurveyorFields) if (!textValue(formData, field)) throw new Error("Surveyor Name, Surveyor Email and Surveyor Number are mandatory for Internal Claim Stage 2.");
  }
  ...
  if (!terminal && targetIndex > activeIndex) {
    const externalSaveOnlyEdit = claim.policy_service_source === "external" && saveOnly;
    if (!externalSaveOnlyEdit) throw new Error("This stage is not available yet. Complete the current stage first.");
  }
  ...
  if (completedSpotStatusEdit) {
    const { data: previousSpotStatus, error: previousSpotStatusError } = await supabase.from("claim_stage_details").select("details,created_at").eq("claim_id", claimId).eq("stage", detailStageStatus.spot_status).eq("details->>milestone_key", "spot_status").order("created_at", { ascending: false }).limit(1).maybeSingle<StageDetailsRow>();
  ...
  if (!shouldAdvance) {
    if (notifySpotStatusEdit) {
      const { error: historyError } = await supabase.from("claim_status_history").insert({ claim_id: claimId, from_status: claim.current_status, to_status: claim.current_status, notes: spotSurveyorUpdateNote(formData), changed_by: profile.id });
  ...
  const { data: persisted, error: persistedError } = await supabase.from("claims").select("id,current_status").eq("id", claimId).maybeSingle<{ id: string; current_status: ClaimStatus }>();
  if (persistedError) throw new Error(persistedError.message);
  if (!persisted || persisted.current_status !== nextStatus) throw new Error("The claim stage could not be persisted. Refresh the claim and try again.");

  const { error: historyError } = await supabase.from("claim_status_history").insert({ claim_id: claimId, from_status: claim.current_status, to_status: nextStatus, notes: stageHistoryNote(formData, stageKey), changed_by: profile.id });
  if (historyError) throw new Error(historyError.message);
  revalidateClaimPaths(claimId);
