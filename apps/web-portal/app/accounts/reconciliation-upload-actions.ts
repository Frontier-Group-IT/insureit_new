...
export async function previewAccountsReconciliationUpload(formData: FormData): Promise<ReconciliationUploadPreview> {
  ...
  try {
    ...
    return emptyPreview("The workbook could not be read. Upload an .xlsx file generated from the Accounts workbook.");
  }

  if (!workbook.SheetNames.includes("Pay-In") || !workbook.SheetNames.includes("Pay-Out")) {
    return emptyPreview("The workbook must contain both Pay-In and Pay-Out sheets generated by INSUREIT.");
  }
  ...
  const activePayin = payinRaw
    .map((row, index) => ({ row, rowNumber: index + 2 }))
    .filter(({ row }) => hasPayinInput(row));
  const activePayout = payoutRaw
    .map((row, index) => ({ row, rowNumber: index + 2 }))
    .filter(({ row }) => hasPayoutInput(row));
  const skippedRows = payinRaw.length + payoutRaw.length - activePayin.length - activePayout.length;

  if (!activePayin.length && !activePayout.length) {
    return { ...emptyPreview("No Pay-In or Pay-Out transaction values were entered in the workbook."), skippedRows };
  }

  const policyIds = [...new Set([
    ...activePayin.map(({ row }) => text(row["System Policy ID"])),
    ...activePayout.map(({ row }) => text(row["System Policy ID"])),
  ].filter(validUuid))];
  ...
  const [{ data: policies, error: policyError }, { data: payins, error: payinError }, { data: payouts, error: payoutError }] = await Promise.all([
    policyIds.length ? policyQuery : Promise.resolve({ data: [], error: null }),
    policyIds.length ? db.from("policy_payin_details").select("policy_id,total_projected_payin,tds_amount,payin_after_tds").in("policy_id", policyIds) : Promise.resolve({ data: [], error: null }),
    payoutIds.length ? db.from("policy_intermediary_payouts").select("id,policy_id,intermediary_type,intermediary_code,od_payout_percent,tp_payout_percent,gross_payout,retention_amount,commercial_status,status").in("id", payoutIds) : Promise.resolve({ data: [], error: null }),
  ]);
  if (policyError || payinError || payoutError) throw new Error(policyError?.message ?? payinError?.message ?? payoutError?.message ?? "Unable to validate reconciliation workbook.");
  ...
  const payinRefs = activePayin.map(({ row }) => text(row["UTR / Reference"])).filter(Boolean);
  const payoutRefs = activePayout.map(({ row }) => text(row["UTR / Reference"])).filter(Boolean);
  const [{ data: existingReceipts }, { data: existingPartnerPayments }] = await Promise.all([
    payinRefs.length ? db.from("accounts_receipts").select("insurance_company_id:insurer_id,bank_reference").in("bank_reference", [...new Set(payinRefs)]) : Promise.resolve({ data: [] }),
    payoutRefs.length ? db.from("partner_payments").select("intermediary_code,payment_reference").in("payment_reference", [...new Set(payoutRefs)]) : Promise.resolve({ data: [] }),
  ]);
  const existingReceiptKeys = new Set((existingReceipts ?? []).map((row) => `${row.insurance_company_id}|${text(row.bank_reference).toUpperCase()}`));
  const existingPayoutKeys = new Set((existingPartnerPayments ?? []).map((row) => `${text(row.intermediary_code).toUpperCase()}|${text(row.payment_reference).toUpperCase()}`));
  ...
  const payinRows: PayinPreviewRow[] = activePayin.map(({ row, rowNumber }) => {
    const policyId = text(row["System Policy ID"]);
    const policy = policyMap.get(policyId);
    const livePayin = payinMap.get(policyId);
    ...
    const projectedPayin = money(livePayin?.total_projected_payin);
    const projectedTds = money(livePayin?.tds_amount);
    const projectedNetPayin = money(livePayin?.payin_after_tds);
    ...
    const actualTds = optionalMoney(row["Actual TDS"]);
    const amountReceived = optionalMoney(row["Amount Received"]);
    const receiptDate = normalizedDate(row["Receipt Date"]);
    const reference = text(row["UTR / Reference"]);
    const difference = billAmount === null ? null : money(projectedPayin - billAmount);
    ...
    if (!amountReceived && (receiptDate || reference)) issues.push(error("Amount Received is required when receipt details are entered."));
    ...
    if (amountReceived !== null && reference && policy?.insurance_company_id) {
      const key = `${policy.insurance_company_id}|${reference.toUpperCase()}`;
      if (existingReceiptKeys.has(key)) issues.push(error("This insurer UTR / Reference already exists in INSUREIT."));
      if (seenPayinRefs.has(key)) issues.push(error("This insurer UTR / Reference is repeated in the uploaded Pay-In sheet."));
      seenPayinRefs.add(key);
    }
    ...
    return {
      rowNumber,
      policyId,
      policyNumber: policy?.policy_no ?? text(row["Policy Number"]),
      insurer,
      projectedPayin,
      projectedTds,
      projectedNetPayin,
  ...
  const payoutRows: PayoutPreviewRow[] = activePayout.map(({ row, rowNumber }) => {
    const payoutId = text(row["System Payout ID"]);
    const policyId = text(row["System Policy ID"]);
    const policy = policyMap.get(policyId);
    const livePayout = payoutMap.get(payoutId);
    const intermediaryType = text(livePayout?.intermediary_type ?? policy?.intermediary_type ?? row["Intermediary Type"]);
    const intermediaryCode = text(livePayout?.intermediary_code ?? policy?.intermediary_code ?? row["Intermediary Code"]);
    const payoutOdPercent = money(livePayout?.od_payout_percent);
    const payoutTpPercent = money(livePayout?.tp_payout_percent);
    const projectedGrossPayout = money(livePayout?.gross_payout);
    const projectedRetention = money(livePayout?.retention_amount);
    ...
    if (reference && intermediaryCode) {
      const key = `${intermediaryCode.toUpperCase()}|${reference.toUpperCase()}`;
      if (existingPayoutKeys.has(key)) issues.push(error("This intermediary UTR / Reference already exists in INSUREIT."));
      if (seenPayoutRefs.has(key)) issues.push(error("This intermediary UTR / Reference is repeated in the uploaded Pay-Out sheet."));
      seenPayoutRefs.add(key);
    }
...
function hasPayinInput(row: Record<string, unknown>) {
  return ["Bill Number", "Bill Amount", "Bill Date", "Actual TDS", "Amount Received", "Receipt Date", "UTR / Reference"].some((key) => text(row[key]) !== "");
}
function hasPayoutInput(row: Record<string, unknown>) {
  return ["Paid Amount", "Paid Date", "UTR / Reference"].some((key) => text(row[key]) !== "");
}
function validateHeaders(rows: Array<Record<string, unknown>>, required: readonly string[], sheet: string) {
  if (!rows.length) return null;
  const present = new Set(Object.keys(rows[0] ?? {}).map((value) => value.trim()));
  const missing = required.filter((header) => !present.has(header));
  return missing.length ? `${sheet} is missing required column${missing.length > 1 ? "s" : ""}: ${missing.join(", ")}.` : null;
}
function summarize(issues: Array<{ status: PreviewStatus; message: string }>): { status: PreviewStatus; message: string } {
  if (!issues.length) return { status: "Ready", message: "Ready for Confirm Import once the posting stage is enabled." };
...
function emptyPreview(message: string): ReconciliationUploadPreview {
  return { totalRows: 0, readyRows: 0, warningRows: 0, errorRows: 0, skippedRows: 0, payinRows: [], payoutRows: [], message };
}
