...
export async function previewAccountsReconciliationUpload(formData: FormData): Promise<ReconciliationUploadPreview> {
  ...
  catch { return emptyPreview("The workbook could not be read. Upload an .xlsx file generated from the Accounts workbook."); }
  ...
  if (policyIds.length) {
    ...
    const [{ data: policyData, error: policyError }, { data: payinData, error: payinError }] = await Promise.all([
      policyQuery,
  ...
  if (payinRefs.length) {
    const { data } = await db.from("accounts_receipts").select("insurer_id,bank_reference").in("bank_reference", payinRefs);
    existingReceipts = data ?? [];
  }
  if (payoutRefs.length) {
    const { data } = await db.from("partner_payments").select("intermediary_code,payment_reference").in("payment_reference", payoutRefs);
    existingPartnerPayments = data ?? [];
  }
  const existingReceiptKeys = new Set(existingReceipts.map((row) => `${row.insurer_id}|${text(row.bank_reference).toUpperCase()}`));
