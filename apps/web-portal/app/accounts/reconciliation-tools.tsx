...
export function ReconciliationTools({ period, fromDate, toDate, insurerId }: Props) {
  ...
  return <section className="rounded-2xl border border-[#dbe3ee] bg-white p-4 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[8px] font-black uppercase tracking-[.1em] text-[#0f766e]">Excel reconciliation</p><h2 className="mt-1 text-[14px] font-semibold text-[#17365D]">Download → enter Pay-In / Pay-Out → upload → validate → confirm</h2></div><span className="rounded-full border border-[#cfe8df] bg-[#f0faf6] px-2.5 py-1 text-[8px] font-bold text-[#0f766e]">Validation required before posting</span></div>
    <div className="mt-3 grid gap-3 lg:grid-cols-2">
      <div className="rounded-xl border border-[#e2e8f0] bg-[#fbfcfe] p-3"><div className="flex gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#e8f5f3] text-[#0f766e]"><FileSpreadsheet className="h-4 w-4" /></span><div><p className="text-[10px] font-bold text-[#17365D]">1. Download Accounts workbook</p><a href={downloadHref} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-[#17365D] px-3 py-2 text-[9px] font-bold text-white"><Download className="h-3.5 w-3.5" />Download workbook</a></div></div></div>
      <div className="rounded-xl border border-[#e2e8f0] bg-[#fbfcfe] p-3"><div className="flex gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#eef4ff] text-[#3156b8]"><UploadCloud className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-bold text-[#17365D]">2. Upload for validation</p><div className="mt-3 flex flex-wrap gap-2"><label className="flex h-9 min-w-[220px] flex-1 cursor-pointer items-center rounded-lg border border-[#d8e1ec] bg-white px-3 text-[9px] text-[#475467]"><input type="file" accept=".xlsx" className="hidden" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setPreview(null); setImportResult(null); setImportError(""); }} /><span className="truncate">{file?.name ?? "Choose completed workbook"}</span></label><button type="button" disabled={isPreviewPending || isImportPending || !file} onClick={runPreview} className="h-9 rounded-lg bg-[#0f766e] px-3 text-[9px] font-bold text-white disabled:opacity-40">{isPreviewPending ? "Validating…" : "Preview upload"}</button></div></div></div></div>
    </div>
    {importResult ? <div className="mt-3 flex items-start gap-2 rounded-xl border border-[#bfe3d5] bg-[#f0faf6] px-3 py-2 text-[9px] font-semibold text-[#0f766e]"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" /><span>{importResult.message} The Business MIS export now reflects the posted Accounts records.</span></div> : null}
    {importError ? <div className="mt-3 rounded-xl border border-[#f3c7c3] bg-[#fff5f4] px-3 py-2 text-[9px] font-semibold text-[#b42318]">{importError}</div> : null}
...
function PreviewPanel({ preview, isImportPending, onConfirm }: { preview: ReconciliationUploadPreview; isImportPending: boolean; onConfirm: () => void }) {
  ...
  return <div className="mt-3 border-t border-[#edf1f5] pt-3"><div className="grid gap-2 sm:grid-cols-5"><Metric label="Transactions" value={preview.totalRows} /><Metric label="Ready" value={preview.readyRows} /><Metric label="Warnings" value={preview.warningRows} /><Metric label="Errors" value={preview.errorRows} /><Metric label="Blank ignored" value={preview.skippedRows} /></div>
    ...
    <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-dashed border-[#cfd9e6] bg-[#f8fafc] px-3 py-2"><p className="max-w-4xl text-[8.5px] font-semibold leading-4 text-[#667085]">Confirm Import re-validates the workbook against live data, then posts bills, TDS, insurer receipts and partner payments in one database transaction. If any posting rule fails, the complete batch is rolled back.</p><button type="button" disabled={!canImport || isImportPending} onClick={onConfirm} className="h-9 rounded-lg bg-[#17365D] px-4 text-[9px] font-bold text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-40">{isImportPending ? "Importing…" : preview.errorRows ? "Resolve errors first" : "Confirm Import"}</button></div>
  </div>;
}
