...
export async function GET(request: Request) {
  ...
  let query = db.from("policies").select("id,policy_no,customer_id,insurance_company_id,issuance_date,start_date,created_at,intermediary_type,intermediary_code,insurance_companies(name)").limit(5000);
  ...
  const payinRows = filtered.map((policy) => {
    ...
    return {
      ...
      "Insurance Company": relationName(policy.insurance_companies),
...
function workbookResponse(payinRows: Array<Record<string, unknown>>, payoutRows: Array<Record<string, unknown>>, fromDate: string, toDate: string) {
  const workbook = XLSX.utils.book_new();
  const payinHeaders = ["System Policy ID", "Policy Number", "Insurance Company", "Projected Pay-In", "Projected TDS", "Projected Net Pay-In", "Bill Number", "Bill Amount", "Bill Date", "Actual TDS", "Amount Received", "Receipt Date", "UTR / Reference"];
  const payinSheet = XLSX.utils.json_to_sheet(payinRows, { header: payinHeaders });
  payinSheet["!cols"] = [{ wch: 38, hidden: true }, { wch: 24 }, { wch: 30 }, { wch: 17 }, { wch: 15 }, { wch: 19 }, { wch: 20 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 18 }, { wch: 14 }, { wch: 28 }];
  XLSX.utils.book_append_sheet(workbook, payinSheet, "Pay-In");

  const payoutHeaders = ["System Payout ID", "System Policy ID", "Policy Number", "Intermediary Type", "Intermediary Code", "Payout OD %", "Payout TP %", "Projected Gross Payout", "Projected Retention", "Paid Amount", "Paid Date", "UTR / Reference"];
  const payoutSheet = XLSX.utils.json_to_sheet(payoutRows, { header: payoutHeaders });
  payoutSheet["!cols"] = [{ wch: 38, hidden: true }, { wch: 38, hidden: true }, { wch: 24 }, { wch: 20 }, { wch: 22 }, { wch: 13 }, { wch: 13 }, { wch: 22 }, { wch: 20 }, { wch: 16 }, { wch: 14 }, { wch: 28 }];
  ...
  const instructions = XLSX.utils.aoa_to_sheet([
    ...
    ["Partial Pay-In", "Duplicate the policy row for each separate insurer installment and enter its receipt amount/date/UTR. The policy itself is not duplicated in INSUREIT."],
    ["Partial Pay-Out", "Duplicate the Pay-Out row for each separate payout installment and enter its payment amount/date/UTR."],
...
function relationName(value: PolicyRow["insurance_companies"]) { return Array.isArray(value) ? value[0]?.name ?? "" : value?.name ?? ""; }
function money(value: unknown) { const parsed = Number(value ?? 0); return Number.isFinite(parsed) ? Math.round(parsed * 100) / 100 : 0; }
