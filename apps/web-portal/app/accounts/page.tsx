...
import { ArrowRight, BarChart3, ClipboardList, FileCheck2, Landmark, ReceiptText, Scale, WalletCards } from "lucide-react";
    ...
    <AppShell title="Accounts">
      <div className="mx-auto max-w-[1560px] space-y-4 pb-10">
        <section className="rounded-2xl border border-[#dbe3ee] bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-4xl text-[10px] leading-5 text-[#667085]">Operational brokerage accounts workspace from commercial expectation through reconciliation, billing and insurer receivables. Reports remains the read-only analytical layer.</p>
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <WorkspaceCard href="/accounts/receivables" icon={WalletCards} title="Insurer Receivables" description="Review insurer debtor balances and accounting movements for raised invoices and future settlements." />
        </section>
        ...
        <section className="rounded-2xl border border-dashed border-[#cbd5e1] bg-[#f8fafc] px-5 py-4">
          ...
          <p className="mt-1 text-[9.5px] leading-5 text-[#667085]">Implemented: Commercial Control → Reconciliation → Brokerage Billing → Insurer Receivables. Next: Receipts & TDS → Partner Payables → Period Close. Future modules remain hidden until their accounting controls exist.</p>
        </section>
      </div>
    </AppShell>
  );
