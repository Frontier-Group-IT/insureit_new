...
export default async function AccountsPage({ searchParams }: Props) {
  ...
  return <AppShell title="Accounts Dashboard"><div className="mx-auto max-w-[1560px] space-y-3 pb-6">
    <section className="rounded-2xl border border-[#dbe3ee] bg-white px-4 py-3 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2.5"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#e8f5f3] text-[#0f766e]"><Landmark className="h-4 w-4" /></span><div><h1 className="text-[17px] font-semibold text-[#17365D]">Accounts Dashboard</h1><p className="mt-0.5 text-[9px] font-medium text-[#7c899b]">{data.periodLabel}</p></div></div><div className="flex rounded-xl border border-[#dce4ee] bg-[#f8fafc] p-1">{PERIODS.map(item => <Link key={item.value} prefetch={false} href={periodHref(item.value, filters.insurerId)} className={`rounded-lg px-3 py-1.5 text-[9px] font-bold ${filters.period === item.value ? "bg-[#17365D] text-white shadow-sm" : "text-[#667085] hover:bg-white hover:text-[#17365D]"}`}>{item.label}</Link>)}</div></div>
