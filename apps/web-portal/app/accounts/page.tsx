...
import { ArrowRight, BarChart3, ClipboardList, FileCheck2, HandCoins, Landmark, ReceiptIndianRupee, ReceiptText, Scale, WalletCards } from "lucide-react";
    ...
    <AppShell title="Accounts">
      <div className="mx-auto max-w-[1560px] space-y-4 pb-10">
        <section className="rounded-2xl border border-[#dbe3ee] bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-4xl text-[10px] leading-5 text-[#667085]">Operational brokerage accounts workspace from commercial expectation through reconciliation, billing, insurer settlement and partner liabilities. Reports remains the read-only analytical layer.</p>
            </div>
        ...
        <section className="rounded-2xl border border-dashed border-[#cbd5e1] bg-[#f8fafc] px-5 py-4">
          ...
          <p className="mt-1 text-[9.5px] leading-5 text-[#667085]">Implemented: Commercial Control → Reconciliation → Brokerage Billing → Insurer Receivables → Receipts & TDS → Partner Payables. Next: Period Close → Accounts Reports.</p>
        </section>
      </div>
    </AppShell>
  );
