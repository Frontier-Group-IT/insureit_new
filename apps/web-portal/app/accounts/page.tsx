    ...
    <AppShell title="Accounts">
      <div className="mx-auto max-w-[1560px] space-y-4 pb-10">
        <section className="rounded-2xl border border-[#dbe3ee] bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-4xl text-[10px] leading-5 text-[#667085]">Operational brokerage accounts workspace from commercial expectation through reconciliation, billing, insurer settlement, partner liabilities and accounting close. Reports is the read-only analytical layer.</p>
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <WorkspaceCard href="/accounts/partner-payables" icon={HandCoins} title="Partner Payables" description="Govern eligibility, approval, holds and payments for actual agreed partner payouts." />
          <WorkspaceCard href="/accounts/period-close" icon={CalendarCheck2} title="Period Close" description="Review accounting exceptions, close a period with a preserved snapshot, and govern reopen corrections." />
          <WorkspaceCard href="/reports/accounts" icon={BarChart3} title="Accounts Reports" description="Read-only recognized brokerage, invoices, receipts, TDS, receivable aging, partner payables, contribution and close reporting." />
        </section>

        <section className="rounded-2xl border border-[#cbd5e1] bg-[#f8fafc] px-5 py-4">
          <h2 className="text-[12px] font-semibold text-[#17365D]">Accounts module rollout</h2>
          <p className="mt-1 text-[9.5px] leading-5 text-[#667085]">Phases A–H are implemented: Accounts workspace → Commercial Control → Reconciliation → Billing & Receivables → Receipts & TDS → Partner Payables → Period Close → read-only Accounts Reports.</p>
        </section>
      </div>
    </AppShell>
  );
