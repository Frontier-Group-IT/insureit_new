...
export default async function MispRegisterPage({searchParams}:{searchParams:Promise<Query>}){const q=await searchParams;return <IntermediaryRegister selectedType="misp" search={q.q?.trim().slice(0,80)??""} accountStatus={q.account_status??""} registrationStatus={q.registration_status??""} portalAccess={q.portal_access??""} success={q.success} error={q.error}/>}
