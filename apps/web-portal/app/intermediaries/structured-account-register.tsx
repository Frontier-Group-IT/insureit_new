...
export async function StructuredAccountRegister({ type, search = "", status = "all" }: { type: AccountType; search?: string; status?: "all" | "active" | "onboarding" }) {
    ...
    <AppShell title={`${title} Register`}>
      <div className="mx-auto max-w-[1480px] space-y-4 pb-6">
        <section className="overflow-hidden rounded-2xl border border-[#DCE5EF] bg-white shadow-sm">
          ...
          {error ? <div className="px-5 py-14 text-center text-[11px] text-red-700">The register could not be loaded.</div> : visibleRows.length ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[940px] table-fixed text-left text-[10.5px]">
                ...
                <tbody className="divide-y divide-[#E7ECF3]">{visibleRows.map((row) => {
