...
export function IcallTrainingLauncher({ applicationId }: Props) {
    ...
    <div className="fixed inset-0 z-[1000] grid place-items-center bg-[#07152F]/65 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="icall-cookie-notice-title">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-[#DCE5EF] bg-white shadow-2xl">
        <div className="border-b border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4 sm:px-6">
          ...
          <p className="mt-2 text-[11px] leading-5 text-[#64748B]">iCall Training opens securely inside INSUREIT and needs third-party cookies to keep your iCall session active.</p>
        </div>
        <div className="space-y-4 px-5 py-5 sm:px-6">
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[10px] leading-4 text-amber-900">If third-party cookies remain blocked, iCall may return to its login page instead of opening your training dashboard. INSUREIT cannot change this browser setting automatically.</div>
        </div>
        <div className="flex flex-col-reverse gap-2 border-t border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
          <button type="button" onClick={() => setCookieNoticeOpen(false)} className="h-10 rounded-xl border border-[#CBD5E1] bg-white px-4 text-[10px] font-semibold text-[#334155] transition hover:bg-[#F1F5F9]">Cancel</button>
          <button type="button" onClick={continueLaunch} className="h-10 rounded-xl bg-[#071D49] px-4 text-[10px] font-semibold text-white transition hover:bg-[#0B2B66]">I&apos;ve allowed cookies — Continue</button>
        </div>
      </div>
    </div>
  ) : null;
    ...
    <div className="fixed inset-0 z-[1000] flex flex-col bg-[#07152F]" role="dialog" aria-modal="true" aria-label="iCall training portal">
      <header className="flex min-h-14 items-center justify-between gap-3 border-b border-white/15 bg-[#071D49] px-4 py-2 text-white sm:px-5">
        <div className="min-w-0">
          <p className="text-[9px] font-semibold uppercase tracking-[.08em] text-blue-200">Secure iCall Production session</p>
          <p className="mt-0.5 truncate text-[12px] font-semibold">Training &amp; Examination</p>
        </div>
  ...
  return <>
    <button type="button" onClick={requestLaunch} disabled={isPending} className="inline-flex h-10 items-center justify-center rounded-xl bg-[#071D49] px-4 text-[10px] font-semibold text-white transition hover:bg-[#0B2B66] disabled:cursor-not-allowed disabled:opacity-60">{isPending ? "Opening secure session…" : "Open training"}</button>
...
function Instruction({ number, text }: { number: string; text: string }) {
  return <div className="flex gap-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-blue-100 text-[9px] font-semibold text-blue-800">{number}</span><p className="pt-0.5 text-[10.5px] leading-5 text-[#334155]">{text}</p></div>;
}
