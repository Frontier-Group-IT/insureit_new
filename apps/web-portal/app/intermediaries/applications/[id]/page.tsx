...
export default async function IntermediaryAccountReviewPage({
  params,
  searchParams,
}: {
  ...
  const { data: related } = application.partner_record_id
    ? await admin
        ...
        .select("id,requested_type,registration_status,draft_data,updated_at")
  ...
  const { data: linkedIntermediary } = linked
    ? await admin
        .from("intermediaries")
        .select("id,intermediary_code,portal_access_status,account_status,activated_at")
        .eq("application_id", linked.id)
        ...
        <section id="details" className="grid gap-4 lg:grid-cols-2">
          <Card title="Identity and contact">
            <dl className="grid gap-x-4 gap-y-2 sm:grid-cols-2">
              ...
              <Info label="Aadhaar" value={maskAadhaar(aadhaar)} />
              <Info label="Date of birth" value={date(profile.partner_type === "misp" ? profile.dp_date_of_birth : profile.date_of_birth)} />
              <Info label="Mobile" value={phone ?? "-"} />
              <Info label="Email" value={email ?? "-"} />
              {profile.partner_type === "misp" ? <><Info label="Designated person" value={profile.dp_name ?? "-"} /><Info label="OEM" value={profile.oem_name ?? "-"} /></> : null}
            </dl>
          </Card>
...
function linkedAccountType(app: LinkedApplication) {
  const context = asObject(app.draft_data).account_context;
  return context === "misp" ? "misp" : context === "posp" ? "posp" : app.requested_type;
}
function permanentCode(value: string | null | undefined) {
  const code = value?.trim();
  return code && !code.startsWith("PENDING-") && !code.startsWith("PART-") ? code : null;
}
...
function partnerJourney(profile: Profile, docs: Document[], activePartner: boolean): JourneyItem[] {
  const primary = profile.workflow_stage !== "pre_iib" || Boolean(profile.partner_id);
  const types = new Set(docs.map((item) => item.document_type));
  const complete = activePartner || ["aadhaar_front", "pan_copy", "cancelled_cheque"].every((type) => types.has(type));
  return [{ label: "Primary details", done: primary, active: !primary }, { label: "Partner documents", done: complete, active: primary && !complete }];
}
function registrationJourney(context: AccountContext, profile: Profile, assignment: Assignment | null, app: Application): JourneyItem[] {
  const agreement = assignment?.agreement_status === "signed";
  const iib = app.registration_status === "iib_registered";
  const training = assignment?.training_status === "completed";
  const exam = assignment?.exam_status === "passed";
  return [
    { label: "Partner linked", done: true, active: false },
    { label: "Training", done: training, active: !training },
    { label: "Exam", done: exam, active: training && !exam },
    { label: "Agreement", done: agreement, active: exam && !agreement },
    { label: `${context.toUpperCase()} active`, done: iib, active: agreement && !iib },
  ];
}
...
function DocumentChecklist({ documents }: { documents: Document[] }) {
  ...
  const expected = [...partnerDocuments.filter(([type]) => !type.startsWith("education_")), ["education", "Education Marksheet"] as const];
  return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{expected.map(([type, label]) => { const document = type === "education" ? education : documents.find((item) => item.document_type === type); return <DocumentStatus key={type} type={type} label={label} document={document} />; })}</div>;
}
...
function Icon({ name, className = "h-4 w-4" }: { name: IconName; className?: string }) {
  const paths: Record<IconName, React.ReactNode> = {
    ...
    account: <><circle cx="12" cy="12" r="9" /><path d="M8 15c1-2.5 7-2.5 8 0M12 7.5a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5Z" /></>,
    id: <><rect x="3" y="5" width="18" height="14" rx="2" /><circle cx="8" cy="11" r="2" /><path d="M6 16c.8-1.8 3.2-1.8 4 0M13 10h5M13 14h5" /></>,
    rm: <><circle cx="9" cy="8" r="3" /><path d="M3 20a6 6 0 0 1 12 0M17 8v6M14 11h6" /></>,
    portal: <><circle cx="12" cy="12" r="9" /><path d="M8 12h8M13 9l3 3-3 3" /></>,
