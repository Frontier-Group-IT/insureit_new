...
function SixStepNavigation({applicationId,viewStage,registrationStatus,documentsComplete,agreementSigned}:{applicationId:string;viewStage:ViewStage;registrationStatus:string;documentsComplete:boolean;agreementSigned:boolean}){
 ...
 return <nav className="overflow-x-auto rounded-2xl border border-[#DCE5EF] bg-white/85 px-5 py-5 shadow-sm backdrop-blur"><div className="relative grid min-w-[760px] grid-cols-6 gap-0 before:absolute before:left-[8.4%] before:right-[8.4%] before:top-[18px] before:h-px before:bg-[#CBD5E1] before:content-['']">{steps.map((step,index)=>{const number=index+1;const completed=number<current||(number===2&&documentsComplete&&current>2)||(number===5&&agreementSigned&&current>5);const active=number===current;return <Link key={step[0]} href={step[2]} className="relative z-[1] min-w-0 text-center"><span className={`mx-auto grid h-9 w-9 place-items-center rounded-full border text-[11px] font-bold shadow-[0_0_0_7px_#F8FAFC] transition ${completed?"border-emerald-600 bg-emerald-600 text-white":active?"border-[#071D49] bg-[#071D49] text-white":"border-[#D7E0EB] bg-[#F1F5F9] text-[#94A3B8]"}`}>{completed?"✓":number}</span><span className={`mt-2 block truncate text-[10.5px] font-semibold ${active?"text-[#071D49]":completed?"text-emerald-800":"text-[#64748B]"}`}>{step[1]}</span><span className="mt-0.5 block text-[8.5px] font-medium text-[#64748B]">{completed?"Completed":active?"Current":"Upcoming"}</span></Link>})}</div></nav>
}
