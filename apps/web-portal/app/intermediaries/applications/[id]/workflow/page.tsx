...
type Application={id:string;requested_type:"posp"|"misp";final_type:"posp"|"misp"|"partner"|null;status:string;registration_status:string;partner_status:string|null;applicant_phone:string|null;applicant_email:string|null;updated_at:string};
...
export default async function IntermediaryWorkflowPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string;success?:string;stage?:string}>}){
  ...
  const [{data:application},{data:profile},{data:documents},{data:panJob},{data:assignment}]=await Promise.all([
    admin.from("intermediary_onboarding_applications").select("id,requested_type,final_type,status,registration_status,partner_status,applicant_phone,applicant_email,updated_at").eq("id",id).maybeSingle<Application>(),
  ...
  const permanentReference=profile.partner_id??(profile.external_onboarding_id&&!profile.external_onboarding_id.startsWith("PENDING-")?profile.external_onboarding_id:null);
  ...
  const requested=(query.stage==="primary"||query.stage==="documents"||query.stage==="review"?query.stage:null)as ViewStage|null;
  ...
  return <AppShell title="Intermediary Workflow">
    ...
    <div className="mx-auto max-w-[1480px] space-y-4 pb-8">
      ...
      {query.error?<div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[10.5px] text-red-700">{errors[query.error]??"The action could not be completed."}</div>:null}
      ...
      {viewStage==="review"?<ReviewStageNavigation applicationId={id}/>:null}
