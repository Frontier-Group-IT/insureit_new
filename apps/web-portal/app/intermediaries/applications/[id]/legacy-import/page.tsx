...
export default async function LegacyImportPage({ params, searchParams }: { params:Promise<{id:string}>; searchParams:Promise<{error?:string}> }) {
  ...
  const [{ data: application }, { data: profile }, { data: linked }] = await Promise.all([
    admin.from("intermediary_onboarding_applications").select("id,requested_type,partner_status,partner_record_id,draft_data").eq("id", id).maybeSingle<Application>(),
    ...
    admin.from("intermediary_onboarding_applications").select("id,draft_data").eq("partner_record_id", (await admin.from("intermediary_onboarding_applications").select("partner_record_id").eq("id", id).maybeSingle<{partner_record_id:string|null}>()).data?.partner_record_id ?? "00000000-0000-0000-0000-000000000000").neq("id", id),
  ]);
      ...
      <form action={importExistingLinkedIntermediary} className="space-y-4">
        ...
        <Section title="Permanent identifiers" note="These values will replace generated IDs and must exactly match the identifiers already used in your existing records.">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Existing Partner ID" name="partner_code" defaultValue={profile.partner_id?.startsWith("PENDING-") ? "" : profile.partner_id ?? ""} placeholder="PART-2024-00127" required />
            <Field label={`Existing ${type.toUpperCase()} ID`} name="registration_code" placeholder={type === "posp" ? "POSP-2024-00481" : "MISP-2023-00018"} required />
          </div>
        </Section>
        ...
        <Section title="Historical dates" note="Enter the original dates from the physical or previous-system records, not today's date.">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Field label="Original onboarding date" name="original_onboarding_date" type="date" required />
            <Field label="Active since" name="original_activation_date" type="date" required />
        ...
        <Section title="Verification and audit" note="This remark will remain with the imported account as an audit reference.">
          ...
          <textarea name="migration_remarks" required minLength={10} className="mt-1.5 min-h-28 w-full rounded-xl border border-[#CBD5E1] bg-white px-3 py-2.5 text-[10.5px] outline-none focus:border-[#635BFF] focus:ring-2 focus:ring-[#E7E5FF]" placeholder="State where the original IDs, agreement and IIB registration were verified." />
