  ...
  useEffect(() => {
    ...
    root.querySelectorAll<HTMLAnchorElement | HTMLButtonElement>("a, button").forEach((action) => {
      ...
      if (label === "open linked account" && !action.closest("#linked-account")) {
        ...
        action.dataset.reviewAction = "open-posp";
      }
    });

    const linkedAccountSection = root.querySelector<HTMLElement>("section#linked-account");
    linkedAccountSection?.querySelectorAll<HTMLAnchorElement>("a").forEach((action) => {
      const label = action.textContent?.trim().toLowerCase();
      if (label === "open posp account" || label === "open misp account") {
  ...
  return (
    <div ref={rootRef} className={isPartnerReview ? "application-review-refined partner-review-refined" : "application-review-refined"}>
      ...
      <style>{`
        ...
        .application-review-refined [data-review-action="open-posp"] {
          display: inline-flex !important;
          align-items: center !important;
          justify-content: center !important;
