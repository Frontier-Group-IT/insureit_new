...
import { IcallProductionPanel } from "./icall-uat-panel";
import {
...
export function TrainingExamStage({ applicationId, profile, assignment, documents, iibVerified, finalType }: Props) {
  ...
  return <div id="qualification-process" className="space-y-4 scroll-mt-24">
    ...
    <ProcessSection id="registration-requirement" number="3" title="Registration" state={registrationCompleted ? "completed" : "current"} statusText={registrationCompleted ? "Signed form uploaded" : "Action required"}>
    ...
    {registrationCompleted ? <ProcessSection id="training-requirement" number="4" title={isIcall ? "Training & Examination" : "Training"} state={isIcall ? (examPassed ? "completed" : "current") : (trainingCompleted ? "completed" : "current")} statusText={isIcall ? (examPassed ? "Training and exam completed" : examFailed ? "Exam failed" : profile.training_login_id ? "Live status" : "Training pending") : (trainingCompleted ? "Training completed" : "Action required")}>
      {isIcall && !trainingAssigned ? <IcallProductionPanel applicationId={applicationId} partnerType={profile.partner_type} loginId={profile.training_login_id} trainingStatus={profile.training_status} examStatus={profile.exam_status} /> : trainingAssigned && isIcall && assignment ? <IcallTrainingDashboard applicationId={applicationId} assignment={assignment} /> : trainingCompleted ? <OutcomeDetails title="Training outcome" facts={[{ label: "Result", value: "Completed" }, { label: "Training", value: assignment?.training_title ?? "-" }, { label: "Assigned on", value: formatDateTime(assignment?.training_assigned_at) }, { label: "Started on", value: formatDateTime(assignment?.training_started_at) }, { label: "Deadline", value: formatDateTime(assignment?.training_deadline) }, { label: "Completed on", value: formatDateTime(assignment?.training_completed_at) }]} /> : trainingAssigned ? <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
          <a href={assignment?.training_url ?? "#"} target="_blank" rel="noreferrer" className={darkActionClassName}>Open training</a>
      ...
      {examPassed ? <OutcomeDetails title="Examination outcome" facts={examFacts(assignment)} /> : <div className="space-y-4">
        ...
        {examAllotted ? <>
          ...
          <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-center">
            <a href={assignment?.exam_url ?? "#"} target="_blank" rel="noreferrer" className={primaryActionClassName}>Open examination</a>
