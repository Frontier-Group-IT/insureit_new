        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-[10.5px]">
            ...
            <tbody className="divide-y">{pageRows.map((row) => {
              ...
              return <tr key={intermediary.id} className="hover:bg-[#FAFCFF]"><td className="px-4 py-3"><p className="font-semibold text-[#0F2A55]">{intermediary.display_name}</p><p className="mt-0.5 text-[8.5px] text-[#64748B]">{intermediary.intermediary_code ?? "Partner ID pending"}</p></td><td className="px-3 py-3"><p className="text-[#334155]">{account?.email ?? intermediary.email ?? "Email not recorded"}</p></td><td className="px-3 py-3"><Status value={resolvedStatus(row)} /></td><td className="px-3 py-3 text-[#475569]">{formatDateTime(account?.invited_at)}</td><td className="px-3 py-3">{intermediary.application_id ? <Link prefetch={false} href={`/intermediaries/applications/${intermediary.application_id}`} className="font-semibold text-[#4F46E5] hover:underline">Open account</Link> : <span className="text-[#94A3B8]">Direct partner</span>}</td><td className="px-3 py-3 text-right"><PortalAction row={row} /></td></tr>;
            })}</tbody>
          </table>
        </div>
