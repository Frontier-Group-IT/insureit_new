    ...
    <section className="overflow-hidden rounded-2xl border border-[#DCE5EF] bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b bg-[#F8FAFC] px-4 py-3 lg:flex-row lg:items-center">
        ...
        <form onSubmit={(event) => event.preventDefault()} className="w-full lg:max-w-[460px]">
          <input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search intermediary user accounts" placeholder="Search partner name, email or ID" className="h-10 w-full rounded-lg border border-[#CBD5E1] bg-white px-3 text-[11.5px] outline-none transition focus:border-[#1D4ED8] focus:ring-2 focus:ring-[#DBEAFE]" />
        </form>
      ...
      {loadError ? <div className="px-4 py-12 text-center"><p className="text-[11px] font-semibold text-red-700">Partner portal users are temporarily unavailable.</p><p className="mt-1 text-[9.5px] text-[#64748B]">Please refresh the page or try again shortly.</p></div> : visibleRows.length ? (
