...
export default async function ReconciliationHistoryPage() {
  ...
  const deleteOptions = cycles.map((cycle) => ({
    id: String(cycle.id),
    label: `${firstName(cycle.insurance_companies as Relation)} · ${String(cycle.period_start)} → ${String(cycle.period_end)}`,
    detail: [String(cycle.status), cycle.statement_reference ? `Ref ${String(cycle.statement_reference)}` : null, `${Number(cycle.row_count ?? 0)} rows`].filter(Boolean).join(" · "),
  }));

  return <AppShell title="Reconciliation History">
    <div className="mx-auto max-w-[1560px] space-y-2.5 pb-6">
      {profile.role === "it_super_user" ? <ItSuperUserFinancialDeletePanel entity="reconciliation_cycle" title="Delete reconciliation cycle" records={deleteOptions} /> : null}
