...
type Application = { id:string; profile_id:string|null; partner_type:string|null; status:string; applicant_phone:string|null; draft_data:Draft|null };
...
export async function approveMobileCorporateApplication(formData: FormData) {
  ...
  const { data: application } = await admin.from("customer_onboarding_applications").select("id,profile_id,partner_type,status,applicant_phone,draft_data").eq("id",applicationId).single<Application>();
  ...
  const applicantContact = (contacts??[]).find((contact)=>samePhone(contact.phone, application.applicant_phone));
  if (!applicantContact) redirect(`/customers/applications/${applicationId}?error=applicant_contact_mismatch`);
  ...
  const memberships = (contacts??[]).map((contact)=>({ customer_id:customerId, profile_id:contact.contact_role===applicantContact.contact_role?application.profile_id:null, invited_phone:contact.phone, invited_email:contact.email, membership_role:contact.contact_role, is_primary:contact.contact_role==="dedicated_spoc", status:contact.contact_role===applicantContact.contact_role?"active":"pending", created_by:reviewer.id }));
  ...
  const permanentContacts = (contacts??[]).map((contact)=>({ customer_id:customerId, contact_role:contact.contact_role, full_name:contact.full_name, phone:contact.phone, email:contact.email, profile_id:contact.contact_role===applicantContact.contact_role?application.profile_id:null, login_required:true, access_status:contact.contact_role===applicantContact.contact_role?"active":"pending", created_by:reviewer.id }));
  ...
  await admin.from("customer_onboarding_contacts").update({linked_profile_id:application.profile_id,membership_status:"active",updated_at:now}).eq("application_id",applicationId).eq("contact_role",applicantContact.contact_role);
