...
export async function updateIntermediaryApplication(data:FormData){
 ...
 const account=value(data,"bank_account_number")?.replace(/\D/g,"")??"";const ifsc=compact(value(data,"bank_ifsc_code"));const hasGst=data.get("has_gst")==="true";const gst=hasGst?compact(value(data,"gst_number")):null;if(!/^[0-9]{6,20}$/.test(account)||!IFSC.test(ifsc??""))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 if((hasGst&&!gst)||(gst&&!GST.test(gst)))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&field=gst_number&stage=primary`);
