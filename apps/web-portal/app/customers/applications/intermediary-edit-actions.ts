...
export async function updateIntermediaryApplication(data:FormData){
 ...
 const [{data:application},{data:profile}]=await Promise.all([
  ...
  admin.from("posp_misp_onboarding_profiles").select("id,partner_type,workflow_stage,aadhaar_last_four,aadhaar_hash,aadhaar_number_encrypted,dp_aadhaar_last_four,dp_aadhaar_hash,dp_aadhaar_number_encrypted").eq("application_id",applicationId).maybeSingle<{id:string;partner_type:"posp"|"misp";workflow_stage:"pre_iib"|"iib_processing"|"training"|"completed";aadhaar_last_four:string|null;aadhaar_hash:string|null;aadhaar_number_encrypted:string|null;dp_aadhaar_last_four:string|null;dp_aadhaar_hash:string|null;dp_aadhaar_number_encrypted:string|null}>()
 ]);
 ...
 if(!applicantPhone||!applicantEmail||!dob||Number.isNaN(Date.parse(dob))||!businessPan||!PAN.test(businessPan)||!value(data,"external_onboarding_id")||!value(data,"address")||!value(data,"city")||!value(data,"state")||!/^[0-9]{6}$/.test(value(data,"postal_code")??""))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 ...
 const aadhaarDigits=value(data,"aadhaar_number")?.replace(/\D/g,"")??"";if(aadhaarDigits&&!/^[0-9]{12}$/.test(aadhaarDigits))redirect(`${path(applicationId)}?error=posp_misp_aadhaar_invalid&stage=primary`);
 ...
 const associates=await loadPospMispAssociates(admin);const associate=associates.find(row=>row.id===value(data,"associate_employee_id"));const bankId=value(data,"bank_id");const{data:bank}=bankId?await admin.from("banks").select("id,name").eq("id",bankId).eq("is_active",true).maybeSingle<{id:string;name:string}>():{data:null};const{data:oem}=type==="misp"&&value(data,"oem_name")?await admin.from("vehicle_manufacturers").select("name").eq("name",value(data,"oem_name")!).eq("is_active",true).maybeSingle<{name:string}>():{data:null};if(!associate||!bank||(type==="misp"&&!oem))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 ...
 revalidatePath(path(applicationId));const returnStage=profile.workflow_stage==="pre_iib"?"primary":profile.workflow_stage==="iib_processing"?"documents":"review";redirect(`${path(applicationId)}?success=primary_details_saved&stage=${returnStage}`);
}
