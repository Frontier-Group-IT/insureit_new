...
export async function updateIntermediaryApplication(data:FormData){
 ...
 if(editSection==="documents"){
  ...
  if(missingStandard)redirect(`${path(applicationId)}?error=documents_incomplete&stage=documents`);
  ...
  revalidatePath(path(applicationId));revalidatePath(`/intermediaries/applications/${applicationId}`);redirect(`${path(applicationId)}?success=documents_saved&stage=documents`);
 }
 ...
 const account=value(data,"bank_account_number")?.replace(/\D/g,"")??"";const ifsc=compact(value(data,"bank_ifsc_code"));const gst=compact(value(data,"gst_number"));if(!/^[0-9]{6,20}$/.test(account)||!IFSC.test(ifsc??""))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 if((type==="misp"&&!gst)||(gst&&!GST.test(gst)))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&field=gst_number&stage=primary`);
 ...
 revalidatePath(path(applicationId));revalidatePath(`/intermediaries/applications/${applicationId}`);redirect(`${path(applicationId)}?success=primary_details_saved&stage=documents`);
}
