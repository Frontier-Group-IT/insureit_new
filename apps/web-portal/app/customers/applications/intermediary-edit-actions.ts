...
export async function updateIntermediaryApplication(data:FormData){
 ...
 if(editSection==="documents"){
  ...
  const requiredDocuments=[...REQUIRED_DOCUMENT_FIELDS,...(profile.gst_number?["gst_copy" as const]:[])];
  const missingStandard=requiredDocuments.find(type=>!resultingTypes.has(type));
 ...
 const gst=compact(value(data,"gst_number"));if(gst&&!GST.test(gst))redirect(`${path(applicationId)}?error=posp_misp_edit_invalid&field=gst_number&stage=primary`);
