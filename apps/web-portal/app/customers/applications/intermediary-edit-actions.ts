...
export async function updateIntermediaryApplication(data:FormData){
 ...
 if(!applicantPhone||!applicantEmail||!dob||Number.isNaN(Date.parse(dob))||!businessPan||!PAN.test(businessPan)||!value(data,"address")||!value(data,"city")||!value(data,"state")||!/^[0-9]{6}$/.test(value(data,"postal_code")??""))redirectFresh(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 ...
 if(type==="misp"&&(!mispName||!validName(dpFirst)||!validName(dpLast)||(dpMiddle&&!validName(dpMiddle))||!dpPan||!PAN.test(dpPan)))redirectFresh(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 const account=value(data,"bank_account_number")?.replace(/\D/g,"")??"";const ifsc=compact(value(data,"bank_ifsc_code"));const gst=compact(value(data,"gst_number"));if(!/^[0-9]{6,20}$/.test(account)||!IFSC.test(ifsc??""))redirectFresh(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
 if((type==="misp"&&!gst)||(gst&&!GST.test(gst)))redirectFresh(`${path(applicationId)}?error=posp_misp_edit_invalid&field=gst_number&stage=primary`);
 ...
 const associates=await loadPospMispAssociates(admin);const submittedAssociate=value(data,"associate_employee_id");const associate=associates.find(row=>row.id===submittedAssociate||row.profile_id===submittedAssociate)??associates.find(row=>row.id===profile.associate_employee_id||row.profile_id===profile.associate_profile_id);const bankId=value(data,"bank_id")??profile.bank_id;const{data:bank}=bankId?await admin.from("banks").select("id,name").eq("id",bankId).maybeSingle<{id:string;name:string}>():{data:null};const{data:oem}=type==="misp"&&value(data,"oem_name")?await admin.from("vehicle_manufacturers").select("name").eq("name",value(data,"oem_name")!).eq("is_active",true).maybeSingle<{name:string}>():{data:null};if(!associate||!bank||(type==="misp"&&!oem))redirectFresh(`${path(applicationId)}?error=posp_misp_edit_invalid&stage=primary`);
