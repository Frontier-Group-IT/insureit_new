...
export async function retryPospMispPanVerification(data:FormData){
 ...
 const {error:profileError}=await admin.from("posp_misp_onboarding_profiles").update({...profilePanUpdate,iib_remarks:null,iib_upload_status:"pending",iib_uploaded:false,iib_uploaded_at:null,iib_completed_at:null,requested_account_type:profile.partner_type,final_account_type:profile.partner_type,partner_decision:"not_applicable",workflow_stage:profile.partner_id?"training":"pre_iib",record_source:"new_onboarding",updated_by:actorId,updated_at:now}).eq("id",profile.id);
...
export async function completePospMispDocumentStage(data:FormData){
 ...
 if(profile.existing_registration_confirmed){const {error}=await admin.from("posp_misp_onboarding_profiles").update({workflow_stage:"completed",partner_status:"active_partner",final_account_type:profile.partner_type,updated_by:actorId,updated_at:now}).eq("id",profile.id);if(error)redirectTo(applicationId,"workflow_save_failed");await admin.from("intermediary_onboarding_applications").update({final_type:profile.partner_type,partner_status:"active_partner",status:"approved",registration_status:"existing_posp_ready_for_activation",updated_at:now}).eq("id",applicationId);revalidatePartnerViews(applicationId);redirect(`${partnersPath}?success=partner_activated&partner_id=${encodeURIComponent(partnerId)}`)}
 const initialRegistrationStatus=profile.partner_type==="posp"?"training_pending":"agreement_pending";
 const nextWorkflowStage=profile.partner_type==="posp"?"training":"agreement";
 const {error}=await admin.from("posp_misp_onboarding_profiles").update({workflow_stage:nextWorkflowStage,partner_status:"active_partner",requested_account_type:profile.partner_type,final_account_type:profile.partner_type,training_status:profile.partner_type==="posp"?"not_assigned":null,exam_status:profile.partner_type==="posp"?"not_allotted":null,updated_by:actorId,updated_at:now}).eq("id",profile.id);if(error)redirectTo(applicationId,"workflow_save_failed");
 await admin.from("intermediary_training_exam_assignments").upsert({application_id:applicationId,training_status:profile.partner_type==="posp"?"not_assigned":"not_required",exam_status:profile.partner_type==="posp"?"not_allotted":"not_required",agreement_status:"not_started",updated_by:actorId,updated_at:now},{onConflict:"application_id"});
 await admin.from("intermediary_onboarding_applications").update({final_type:profile.partner_type,partner_status:"active_partner",status:"approved",registration_status:initialRegistrationStatus,updated_at:now}).eq("id",applicationId);
