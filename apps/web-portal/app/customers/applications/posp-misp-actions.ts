...
export async function approvePospMispApplication(formData:FormData){
 ...
 const reviewer=await requirePospMispManager();if(!reviewer?.id)redirect(`${path(applicationId)}?error=unauthorized`);
 ...
 if(applicationError||!application)redirect(`${path(applicationId)}?error=application_not_found`);
 ...
 if(onboardingError||!onboarding)redirect(`${path(applicationId)}?error=posp_misp_profile_missing`);if(onboarding.workflow_stage!=="completed")redirect(`${path(applicationId)}?error=application_not_ready`);
 const finalType=onboarding.final_account_type??onboarding.partner_type;const normalRoute=onboarding.iib_remarks==="No Data Found In POS System"&&["posp","misp"].includes(finalType);const partnerRoute=onboarding.iib_remarks==="Matching Record Found In DataBase"&&onboarding.partner_decision==="convert_to_partner"&&finalType==="partner";if(!normalRoute&&!partnerRoute)redirect(`${path(applicationId)}?error=application_not_ready`);
 const now=new Date().toISOString();const {data:intermediary,error:intermediaryError}=await admin.from("intermediaries").update({intermediary_type:finalType,account_status:"active",compliance_status:finalType==="partner"?"restricted_partner":"approved",visibility_level:finalType==="partner"?"restricted":"standard",activated_at:now,updated_by:reviewer.id,updated_at:now}).eq("onboarding_profile_id",onboarding.id).select("id").maybeSingle<{id:string}>();if(intermediaryError||!intermediary?.id)redirect(`${path(applicationId)}?error=intermediary_activation_failed`);
 ...
 revalidatePath("/intermediaries");revalidatePath("/customers/posp-misp");revalidatePath(path(applicationId));redirect(`${path(applicationId)}?success=intermediary_activated`);
}
