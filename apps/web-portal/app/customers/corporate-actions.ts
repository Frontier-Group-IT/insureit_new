...
export async function createCorporateOnboarding(_state: CorporateOnboardingState, data: FormData): Promise<CorporateOnboardingState> {
  ...
  const companyPanCopy = upload(data, "company_pan_copy");
  const gstCopy = upload(data, "gst_copy");
  const panFileError = checkFile(companyPanCopy, "Company PAN copy", true);
  if (panFileError) return fail(panFileError, "company_pan_copy");
  ...
  const { data: customer, error: customerError } = await admin.from("customers").insert({
    ...
    is_gst_registered: Boolean(gstNumber),
    gst_number: gstNumber,
    pan_number: companyPan,
    fleet_size_band: fleetSize,
  ...
  if (customerError || !customer) {
    await correction(customerError?.message ?? "Corporate customer could not be saved.");
    return fail(`Corporate customer could not be saved: ${customerError?.message ?? "Unknown error"}`);
  }

  const cleanup = async () => { await admin.from("customers").delete().eq("id", customer.id); };

  const { error: contactError } = await admin.from("customer_onboarding_contacts").insert(contacts.map((contact) => ({
  ...
  const { error: membershipError } = await admin.from("customer_memberships").insert(contacts.map((contact) => ({
    customer_id: customer.id,
    invited_phone: contact.phone,
    invited_email: contact.email,
    membership_role: contact.role,
    is_primary: contact.role === "dedicated_spoc",
    status: "pending",
    created_by: profile.id
  })));
