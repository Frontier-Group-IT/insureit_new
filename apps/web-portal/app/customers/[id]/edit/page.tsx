...
type Customer = {
  ...
  india_location_id: string | null; city: string | null; state: string | null; postal_code: string | null; pan_number: string | null;
  aadhaar_last_four: string | null; legal_trade_name: string | null; is_gst_registered: boolean; gst_number: string | null;
  fleet_size_band: string | null; onboarding_status: string; assigned_agent_id: string | null; created_at: string; updated_at: string;
};
...
export default async function EditCustomerPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; field?: string }> }) {
  ...
  if (customer.partner_type === "dealership") {
    const [{ data: dealership }, { data: representative }, { data: contacts }, { data: manufacturers }] = await Promise.all([
      ...
      admin.from("dealership_contacts").select("contact_role, contact_name, mobile, email").eq("customer_id", id).returns<Contact[]>(),
    ...
    return <AppShell title="Dealership Profile"><DealershipProfileEditor
      action={updateDealershipProfile.bind(null, id)}
      values={{
        ...
        address_locality: customer.address_locality,
        city: customer.city,
        state: customer.state,
        postal_code: customer.postal_code,
        india_location_id: customer.india_location_id,
        oem_name: dealership.oem_name,
        yearly_sales_band: dealership.yearly_sales_band,
        ...
        representative_pan: representative.pan_number,
        aadhaar_last_four: representative.aadhaar_last_four
      }}
      contacts={contacts ?? []}
  ...
  return <AppShell title="Customer Profile"><CustomerProfileEditor
    customer={customer}
    documents={documentsWithUrls}
    vehicles={vehicles ?? []}
    agents={agents ?? []}
