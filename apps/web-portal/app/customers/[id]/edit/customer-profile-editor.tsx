      ...
      <form ref={formRef} action={action} encType="multipart/form-data" onSubmit={handleSubmit} className="space-y-2 pb-5">
        ...
        <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-[var(--shadow-panel)]">
          <div className="flex items-center justify-between gap-2 px-4 py-3">
            <Link href={`/vehicles/new?customer_id=${encodeURIComponent(customer.id)}`} className="group inline-flex h-8 items-center gap-1.5 rounded-lg border border-[#D6E1EE] bg-[#F4F7FB] px-2.5 text-[11px] font-semibold text-[#2563EB] transition-all hover:border-[#B8C9DE] hover:bg-[#EDF3F9] hover:text-[#1D4ED8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#B9CCE3]" aria-label="Add Vehicle"><span className="relative inline-flex h-4 w-5 items-center justify-center"><CarFront className="h-4 w-4" strokeWidth={2.1} /><CirclePlus className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 fill-white text-[#2563EB] transition group-hover:text-[#1D4ED8]" strokeWidth={2.3} /></span><span>Add Vehicle</span></Link>
            <div className="flex items-center justify-end gap-2">
              ...
              <FormSubmitButton label="Save changes" className="h-9 rounded-md border border-[#315FEA] bg-[#315FEA] px-4 text-[10.5px] font-semibold text-white shadow-sm hover:bg-[#2851D9]" />
            </div>
          </div>
        </section>
      </form>
