...
export function CustomerOnboardingForm({ action, partnerType }: Props) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsDirty(true);
    if (missingDocuments.length && !warningAccepted) { event.preventDefault(); setShowWarning(true); }
  }
        ...
        {!supported ? <section className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-center"><p className="text-[14px] font-semibold text-amber-900">{partnerLabels[partnerType]} onboarding is coming soon.</p><Link href="/customers?choose_partner=1" className="mt-4 inline-flex rounded-md bg-[#4F46E5] px-4 py-2 text-[11px] font-semibold text-white">Choose another partner type</Link></section> : (
          <form ref={formRef} action={submitWithFiles} onSubmit={handleSubmit} onChange={() => setIsDirty(true)} className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
            ...
            <FormSection title="Address Details" columns="five">
              ...
              <Field label="PIN Code" name="postal_code" required value={postalCode} onChange={(event) => setPostalCode(event.target.value)} placeholder="PIN" inputMode="numeric" maxLength={6} />
...
function FormSection({ title, action, children, compact = false, columns }: { title: string; action?: React.ReactNode; children: React.ReactNode; compact?: boolean; columns: GridColumns }) {
  const gridClass = columns === "five" ? "md:grid-cols-2 xl:grid-cols-[minmax(0,2.1fr)_minmax(0,1.25fr)_minmax(0,1fr)_minmax(0,1.2fr)_minmax(90px,.75fr)]" : columns === "four" ? "md:grid-cols-2 xl:grid-cols-4" : "md:grid-cols-2 xl:grid-cols-3";
