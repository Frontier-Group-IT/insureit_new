    ...
    <>
      ...
      <div className="mx-auto max-w-[1240px] space-y-2 pb-20">
        <div className="flex items-center justify-between">
          <span className="rounded-full border border-[#D8DEE8] bg-white px-2.5 py-1 text-[10.5px] font-semibold text-[#475569]">
            {partnerLabels[partnerType] ?? partnerType}
          </span>
        ...
        {!supported ? (
          ...
          <form ref={formRef} action={submitWithFiles} onSubmit={handleSubmit} onChange={() => setIsDirty(true)} className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
            ...
            <FormSection title="Personal Information" columns="three">
            ...
            <FormSection title="Address Details" columns="five">
              <div className="xl:col-span-2"><Field label="Street" name="address_street" required placeholder="House, building or street" /></div>
              <Field label="Locality" name="address_locality" placeholder="Area or locality" />
              <div className="relative xl:max-w-[180px]">
                <label className={labelClass} htmlFor="city_search">City *</label>
                <input id="city_search" name="city_search" value={cityQuery} required autoComplete="off" className={fieldClass(formState.field === "city_search")} placeholder="City" onChange={(event) => { setCityQuery(event.target.value); setSelectedLocation(null); }} />
              ...
              <div className="xl:max-w-[130px]"><Field label="PIN Code" name="postal_code" required value={postalCode} onChange={(event) => setPostalCode(event.target.value)} placeholder="PIN" inputMode="numeric" maxLength={6} /></div>
              <div className="xl:col-span-2"><Field label="State" name="state" required value={stateValue} onChange={(event) => setStateValue(event.target.value)} placeholder="State" /></div>
            ...
            >
              ...
              {gstRegistered ? <Field label="GST Number" name="gst_number" required error={formState.field === "gst_number"} placeholder="22AAAAA0000A1Z5" maxLength={15} /> : <div />}
            </FormSection>

            <FormSection title="Documents" columns="four" compact>
              ...
              {gstRegistered ? <DocumentUpload label="GST Copy" name="gst_copy" file={documentFiles.gst_copy} onChange={(file) => updateDocument("gst_copy", file)} /> : <div />}
            </FormSection>

            <FormSection title="Fleet Details" columns="four">
              <div><label className={labelClass} htmlFor="fleet_size_band">Fleet Size *</label><select id="fleet_size_band" name="fleet_size_band" required className={fieldClass(formState.field === "fleet_size_band")}><option value="">Select fleet size</option><option value="less_than_5">Less than 5</option><option value="5_to_20">5–20</option><option value="20_to_50">20–50</option><option value="more_than_50">More than 50</option></select></div>
            </FormSection>
...
function FormSection({ title, action, children, compact = false, columns }: { title: string; action?: React.ReactNode; children: React.ReactNode; compact?: boolean; columns: GridColumns }) {
  const gridClass = columns === "five"
    ? "md:grid-cols-2 xl:grid-cols-5"
    : columns === "four"
      ? "md:grid-cols-2 xl:grid-cols-4"
      : "md:grid-cols-2 xl:grid-cols-3";
  return <section className={`border-b border-[#E2E8F0] px-5 ${compact ? "py-3" : "py-4"} last:border-b-0`}><div className="mb-3 flex items-center justify-between gap-3"><h3 className="text-[13px] font-semibold text-[#0F172A]">{title}</h3>{action}</div><div className={`grid gap-x-3 gap-y-3 ${gridClass}`}>{children}</div></section>;
}

function fieldClass(error = false) {
  return `${inputClass} ${error ? "border-red-400 bg-red-50/40 focus:border-red-500 focus:ring-red-100" : "border-[#CBD5E1] focus:border-[#4F46E5] focus:ring-[#E0E7FF]"}`;
}

function Field({ label, name, required = false, error = false, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string; name: string; error?: boolean }) {
  return <div><label className={labelClass} htmlFor={name}>{label}{required ? " *" : ""}</label><input id={name} name={name} required={required} aria-invalid={error || undefined} className={fieldClass(error)} {...props} /></div>;
}
