...
export function CustomerOnboardingForm({ action, partnerType, returnToVehicle = false }: Props) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const aadhaarInput = form.elements.namedItem("aadhaar_number");
    const aadhaar = aadhaarInput instanceof HTMLInputElement ? aadhaarInput.value.replace(/\D/g, "") : "";
    if (aadhaar && !/^[0-9]{12}$/.test(aadhaar)) {
      event.preventDefault();
      setErrorPopup({ message: "Enter a valid 12-digit Aadhaar number.", field: "aadhaar_number" });
      return;
    }
    if (missingDocuments.length) {
      event.preventDefault();
      const firstMissing = missingDocuments[0];
      const label = firstMissing === "gst_copy" ? "GST certificate" : "Document";
...
function DocumentsSection({ children }: { children: React.ReactNode }) { return <section className="mx-3 my-3 rounded-xl border border-[#DDE3F8] bg-[#F6F7FF] px-4 py-3.5 sm:mx-4 sm:px-5"><div className="mb-3 flex items-center"><h3 className="text-[13px] font-bold text-[#0F172A]">Documents</h3></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{children}</div></section>; }
...
function DocumentUpload({ label, name, file, required = false, onChange }: { label: string; name: DocumentField; file?: File; required?: boolean; onChange: (file: File | null) => void }) { return <div className="rounded-lg border border-[#E3E7F0] bg-white p-2.5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]"><span className={labelClass}>{label}{required ? " *" : ""}</span><label htmlFor={name} className={`flex h-9 cursor-pointer items-center gap-2 rounded-md border px-2.5 text-[10.5px] ${file ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-dashed border-[#CBD5E1] bg-white text-[#64748B] hover:border-[#818CF8]"}`}><span>{file ? "✓" : "↥"}</span><span className="min-w-0 flex-1 truncate">{file?.name ?? "Choose file"}</span>{file ? <span className="text-[9px] font-semibold">Ready</span> : required ? <Paperclip className="h-3.5 w-3.5 shrink-0" aria-label="Attachment required" /> : <span className="text-[9px] font-semibold">Optional</span>}</label><input id={name} name={name} type="file" required={required && !file} accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" className="sr-only" onChange={(event) => onChange(event.target.files?.[0] ?? null)} /></div>; }