...
type Props = { action: (previousState: CustomerOnboardingState, formData: FormData) => Promise<CustomerOnboardingState>; partnerType: string; groups: Option[] };
...
export function CustomerOnboardingForm({ action, partnerType, groups }: Props) {
  ...
  useEffect(() => {
    ...
    if (locationId) setSelectedLocation({ id: locationId, city_name: cityName, district: null, state_name: String(snapshot.state ?? ""), pincode: String(snapshot.postal_code ?? "") });
    requestAnimationFrame(() => {
    ...
    <div className="mx-auto max-w-[1240px] space-y-2 pb-20">
      ...
      {!supported ? <section className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-center"><p className="text-[14px] font-semibold text-amber-900">{partnerLabels[partnerType]} onboarding is coming soon.</p><Link href="/customers?choose_partner=1" className="mt-4 inline-flex rounded-md bg-[#4F46E5] px-4 py-2 text-[11px] font-semibold text-white">Choose another partner type</Link></section> : (
        <form ref={formRef} action={submitWithFiles} onSubmit={handleSubmit} onChange={() => setIsDirty(true)} className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <FormSection title="Group Affiliation" columns="four"><div><label className={labelClass} htmlFor="group_customer_id">Parent Group</label><select id="group_customer_id" name="group_customer_id" className={fieldClass(formState.field === "group_customer_id")}><option value="">No parent Group</option>{groups.map((group) => <option key={group.value} value={group.value}>{group.label}</option>)}</select></div></FormSection>
...
function DocumentUpload({ label, name, file, required = false, onChange }: { label: string; name: DocumentField; file?: File; required?: boolean; onChange: (file: File | null) => void }) { return <div><span className={labelClass}>{label}{required ? " *" : ""}</span><label htmlFor={name} className={`flex h-9 cursor-pointer items-center gap-2 rounded-md border px-2.5 text-[10.5px] ${file ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-dashed border-[#CBD5E1] bg-[#F8FAFC] text-[#64748B] hover:border-[#818CF8]"}`}><span>{file ? "✓" : "↥"}</span><span className="min-w-0 flex-1 truncate">{file?.name ?? "Choose file"}</span><span className="text-[9px] font-semibold">{file ? "Ready" : required ? "Required" : "Optional"}</span></label><input id={name} name={name} type="file" required={required && !file} accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" className="sr-only" onChange={(event) => onChange(event.target.files?.[0] ?? null)} /></div>; }
