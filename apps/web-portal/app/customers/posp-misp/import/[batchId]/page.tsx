...
export default async function PospMispImportBatchPage({params,searchParams}:{params:Promise<{batchId:string}>;searchParams:Promise<{error?:string;success?:string}>}){
  ...
  const rowsWithWorkflow=(rows??[]).map(row=>({
    ...row,
    normalized_data:{...row.normalized_data,aadhaar_number:decryptSensitiveValue(typeof row.normalized_data.aadhaar_number_encrypted==="string"?row.normalized_data.aadhaar_number_encrypted:null)},
    documents:documentsByRow.get(row.id)??[],
    pan_job:row.application_id?jobsByApplication.get(row.application_id)??null:null,
    route_profile:row.application_id?profilesByApplication.get(row.application_id)??null:null
  }));
