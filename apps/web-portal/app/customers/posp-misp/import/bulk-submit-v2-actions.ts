...
export async function submitPospMispImportBatchV2(data:FormData){
  ...
  for(const row of rows){
    ...
    try{
      ...
      const onboardingId=string(n.external_onboarding_id)!;
      const{data:duplicate}=await admin.from("posp_misp_onboarding_profiles").select("application_id").eq("external_onboarding_id",onboardingId).maybeSingle<{application_id:string}>();
      ...
      if(applicationError||!application)throw stage("Create application",applicationError);applicationId=application.id;
      ...
      const{data:profile,error:profileError}=await admin.from("posp_misp_onboarding_profiles").insert(profilePayload).select("id").single<{id:string}>();
      if(profileError||!profile)throw stage("Create profile",profileError);
      ...
      if(applicationId)await admin.from("intermediary_onboarding_applications").delete().eq("id",applicationId);
      const ref=randomUUID().slice(0,8);console.error(`POSP/MISP v2 import row ${row.row_number} failed [${ref}]`,error);
