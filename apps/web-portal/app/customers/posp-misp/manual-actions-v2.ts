...
export async function createManualPospMispOnboardingV2(_state: PospMispState, data: FormData): Promise<CreateResult> {
  ...
  if (partnerType === "misp") {
    ...
    const result = oem ? await admin.from("vehicle_manufacturers").select("name").eq("name", oem).eq("is_active", true).maybeSingle<{ name: string }>() : { data: null };
