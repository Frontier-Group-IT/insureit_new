...
export async function createManualPospMispOnboardingV2(_state: PospMispState, data: FormData): Promise<CreateResult> {
  ...
  let oemName: string | null = null;
  if (partnerType === "misp") {
    const oem = value(data, "oem_name");
    const manufacturerBrand = oem ? await loadActiveManufacturerBrand(admin, oem) : null;
    if (!manufacturerBrand) return fail("Select a valid OEM Name.", "oem_name");
    oemName = manufacturerBrand.brand_name;
  }
  const onboardingId = null;
  ...
  if (!address) return fail("Address is required.", "address"); if (!city) return fail("City is required.", "city"); if (!state) return fail("State is required.", "state");
  ...
  if (partnerType === "misp" && !gst) return fail("GST Number is required.", "gst_number");
  ...
  if (partnerType === "misp") { if (!dpFirst) return fail("DP First Name is required.", "dp_first_name"); if (!dpPhone) return fail("Enter a valid DP Contact.", "dp_phone"); if (!dpEmail || !EMAIL.test(dpEmail)) return fail("Enter a valid DP Email.", "dp_email"); if (!PAN.test(dpPan ?? "")) return fail("DP PAN No is invalid.", "dp_pan_number"); if (!dob || Number.isNaN(Date.parse(dob))) return fail("Enter a valid DP Date of Birth.", "date_of_birth"); if (!/^[0-9]{12}$/.test(aadhaarDigits ?? "")) return fail("DP Aadhaar Number must contain exactly 12 digits.", "aadhaar_number"); }
  const applicantPhone = partnerType === "misp" ? dpPhone : normalizePhone(value(data, "applicant_phone")); const applicantEmail = partnerType === "misp" ? dpEmail : value(data, "applicant_email")?.toLowerCase() ?? null;
  if (!applicantPhone) return fail("Enter a valid Mobile Number.", partnerType === "misp" ? "dp_phone" : "applicant_phone");
  if (!applicantEmail || !EMAIL.test(applicantEmail)) return fail("Enter a valid Email Address.", partnerType === "misp" ? "dp_email" : "applicant_email");
  ...
  try {
    ...
    const pendingOnboardingId = `PENDING-${partnerType.toUpperCase()}-${application.id}`;
    const { error: profileError } = await admin.from("posp_misp_onboarding_profiles").insert({ application_id: application.id, partner_type: partnerType, requested_account_type: partnerType, final_account_type: null, partner_decision: "not_applicable", associate_employee_id: associate.id, associate_profile_id: associate.profile_id, associate_name: associate.full_name, associate_id: associate.employee_code, external_onboarding_id: pendingOnboardingId, document_received_at: value(data, "document_received_at"), pos_name: posName, misp_name: mispName, applicant_phone: applicantPhone, applicant_email: applicantEmail, pan_number: businessPan, gst_number: gst, address, city, state, postal_code: postalCode, bank_id: bank.id, bank_name: bank.name, bank_account_number: accountNumber, bank_ifsc_code: ifsc, oem_name: oemName, dp_first_name: dpFirst, dp_middle_name: dpMiddle, dp_last_name: dpLast, dp_name: dpName, dp_phone: dpPhone, dp_email: dpEmail, dp_pan_number: dpPan, dp_date_of_birth: partnerType === "misp" ? dob : null, dp_aadhaar_last_four: partnerType === "misp" ? aadhaar.lastFour : null, dp_aadhaar_hash: partnerType === "misp" ? aadhaar.hash : null, dp_aadhaar_number_encrypted: partnerType === "misp" ? aadhaar.encrypted : null, date_of_birth: partnerType === "posp" ? dob : null, aadhaar_last_four: partnerType === "posp" ? aadhaar.lastFour : null, aadhaar_hash: partnerType === "posp" ? aadhaar.hash : null, aadhaar_number_encrypted: partnerType === "posp" ? aadhaar.encrypted : null, education_status: "not_received", iib_remarks: null, iib_upload_status: "pending", iib_uploaded: false, workflow_stage: "iib_processing", pre_iib_submitted_at: now, source: "manual", raw_data: { gst_registered: Boolean(gst), onboarding_id_pending: true }, created_by: manager.id, updated_by: manager.id });
    ...
    for (const field of DOCUMENT_FIELDS) { const selected = upload(data, field); if (!selected) continue; const path = `${application.id}/intermediary/${field}/${randomUUID()}.${fileExtension(selected)}`; const { error: uploadError } = await admin.storage.from(DOCUMENT_BUCKET).upload(path, new Uint8Array(await selected.arrayBuffer()), { contentType: selected.type, upsert: false }); if (uploadError) throw uploadError; uploaded.push(path); const { error: documentError } = await admin.from("intermediary_onboarding_documents").upsert({ application_id: application.id, document_type: field, file_name: selected.name, storage_bucket: DOCUMENT_BUCKET, storage_path: path, mime_type: selected.type, file_size: selected.size, verification_status: "pending", uploaded_by: manager.id }, { onConflict: "application_id,document_type" }); if (documentError) throw documentError; }
  } catch (error) {
...
async function loadActiveManufacturerBrand(admin:ReturnType<typeof createSupabaseAdminClient>,brandName:string){const{data:brands,error}=await admin.from("vehicle_manufacturer_brands").select("manufacturer_id,brand_name").eq("brand_name",brandName).eq("is_active",true).limit(20).returns<Array<{manufacturer_id:string;brand_name:string}>>();if(error||!brands?.length)return null;const manufacturerIds=Array.from(new Set(brands.map((brand)=>brand.manufacturer_id)));const{data:manufacturer,error:manufacturerError}=await admin.from("vehicle_manufacturers").select("id").in("id",manufacturerIds).eq("is_active",true).limit(1).maybeSingle<{id:string}>();if(manufacturerError||!manufacturer)return null;return brands.find((brand)=>brand.manufacturer_id===manufacturer.id)??null}
...
function message(error:unknown,fallback:string){if(error instanceof Error&&error.message)return error.message;if(error&&typeof error==="object"){const item=error as{message?:unknown;details?:unknown;hint?:unknown;code?:unknown};const text=[item.message,item.details,item.hint].filter((part):part is string=>typeof part==="string"&&Boolean(part.trim()));if(text.length)return text.join(" ");if(typeof item.code==="string")return`Database error ${item.code}`}return fallback}
