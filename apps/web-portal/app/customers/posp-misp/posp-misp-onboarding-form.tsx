  ...
  return <>
    <div className="w-full space-y-3 pb-20">
      ...
      <form ref={formRef} action={submitPath ?? formAction} method={submitPath ? "post" : undefined} onSubmitCapture={handleSubmit} noValidate className="w-full overflow-hidden rounded-2xl border border-[#DCE5EF] bg-white shadow-sm">
        ...
        <Section title={isMisp ? "Bank & GST detail" : "Bank details"}><SelectField label="Bank Name" name="bank_id" required manualValidation={manualValidation} options={banks} placeholder="Select bank" defaultValue={initialValues.bank_id} /><Field label="Account Number" name="bank_account_number" required manualValidation={manualValidation} inputMode="numeric" pattern="[0-9]{6,20}" defaultValue={initialValues.bank_account_number} /><Field label="IFSC Code" name="bank_ifsc_code" required manualValidation={manualValidation} maxLength={11} minLength={11} pattern="[A-Za-z]{4}0[A-Za-z0-9]{6}" transform="uppercase" defaultValue={initialValues.bank_ifsc_code} />{isMisp ? <Field label="GST Number" name="gst_number" required manualValidation={manualValidation} maxLength={15} minLength={15} pattern="[0-9]{2}[A-Za-z]{5}[0-9]{4}[A-Za-z][1-9A-Za-z]Z[0-9A-Za-z]" transform="uppercase" defaultValue={initialValues.gst_number} /> : null}</Section>
