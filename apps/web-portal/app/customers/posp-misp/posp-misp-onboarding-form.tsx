    ...
    <div className="w-full space-y-3 pb-20">
      ...
      <form ref={formRef} action={submitPath ?? formAction} method={submitPath ? "post" : undefined} onSubmitCapture={handleSubmit} noValidate className="w-full overflow-hidden rounded-2xl border border-[#DCE5EF] bg-white shadow-sm">
        ...
        <Section title={isMisp ? "MISP details" : "POSP details"}>
          <div className={`grid min-w-0 gap-3 md:grid-cols-2 xl:col-span-4 ${isMisp ? "xl:grid-cols-4" : "xl:grid-cols-3"}`}><SelectField label="RM Name" name="associate_employee_id" required manualValidation={manualValidation} options={salesManagers} placeholder="Select RM" value={rmValue} onChange={event => { setRmValue(event.target.value); setClientError(null); }} />{isMisp ? <Field label="MISP Name" name="misp_name" required manualValidation={manualValidation} /> : null}<PanInput label={isMisp ? "MISP PAN" : "PAN Number"} name="pan_number" compact manualValidation={manualValidation} /><IndianDateField label="Document Received Date" name="document_received_at" inputClassName={dateInputClass} /></div>
          {!isMisp ? <div className="grid min-w-0 gap-3 md:grid-cols-3 xl:col-span-4"><Field label="POS First Name" name="pos_first_name" required manualValidation={manualValidation} /><Field label="POS Middle Name" name="pos_middle_name" manualValidation={manualValidation} /><Field label="POS Last Name" name="pos_last_name" required manualValidation={manualValidation} /></div> : null}
          {isMisp ? <SelectField label="OEM Name" name="oem_name" required manualValidation={manualValidation} options={oems} placeholder="Select OEM" /> : null}
          <Field label="Address" name="address" required manualValidation={manualValidation} /><Field label="City" name="city" required manualValidation={manualValidation} /><Field label="State" name="state" required manualValidation={manualValidation} /><Field label="PIN Code" name="postal_code" required manualValidation={manualValidation} inputMode="numeric" pattern="[0-9]{6}" maxLength={6} minLength={6} />
        </Section>
        {!isMisp ? <Section title="POSP contact"><Field label="Mobile Number" name="applicant_phone" required manualValidation={manualValidation} inputMode="tel" pattern="(?:\+91)?[6-9][0-9]{9}" /><Field label="Email" name="applicant_email" type="email" required manualValidation={manualValidation} /><IndianDateField label="Date of Birth" name="date_of_birth" required={!manualValidation} inputClassName={dateInputClass} /><Field label="Aadhaar Number" name="aadhaar_number" required manualValidation={manualValidation} inputMode="numeric" pattern="[0-9]{12}" maxLength={12} minLength={12} /></Section> : null}
        {isMisp ? <Section title="Designated Person (DP)"><Field label="DP First Name" name="dp_first_name" required manualValidation={manualValidation} /><Field label="DP Middle Name" name="dp_middle_name" manualValidation={manualValidation} /><Field label="DP Last Name" name="dp_last_name" required manualValidation={manualValidation} /><Field label="DP Contact" name="dp_phone" required manualValidation={manualValidation} inputMode="tel" pattern="(?:\+91)?[6-9][0-9]{9}" /><Field label="DP Email" name="dp_email" required type="email" manualValidation={manualValidation} /><PanInput label="DP PAN No" name="dp_pan_number" manualValidation={manualValidation} /><IndianDateField label="DP Date of Birth" name="date_of_birth" required={!manualValidation} inputClassName={dateInputClass} /><Field label="DP Aadhaar Number" name="aadhaar_number" required manualValidation={manualValidation} inputMode="numeric" pattern="[0-9]{12}" maxLength={12} minLength={12} /></Section> : null}
        <Section title="Bank details"><SelectField label="Bank Name" name="bank_id" required manualValidation={manualValidation} options={banks} placeholder="Select bank" /><Field label="Account Number" name="bank_account_number" required manualValidation={manualValidation} inputMode="numeric" pattern="[0-9]{6,20}" /><Field label="IFSC Code" name="bank_ifsc_code" required manualValidation={manualValidation} maxLength={11} minLength={11} pattern="[A-Za-z]{4}0[A-Za-z0-9]{6}" transform="uppercase" /></Section>
...
function Field({ label, name, required = false, transform, manualValidation = false, onInvalid, onInput, pattern, minLength, maxLength, type, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string; name: string; transform?: "uppercase"; manualValidation?: boolean }) { return <div className="min-w-0"><label className={labelClass} htmlFor={name}>{label}{required ? " *" : ""}</label><input id={name} name={name} type={type} required={!manualValidation && required} data-required={manualValidation && required || undefined} data-pattern={manualValidation ? pattern : undefined} data-min-length={manualValidation ? minLength : undefined} data-label={label} pattern={manualValidation ? undefined : pattern} minLength={manualValidation ? undefined : minLength} maxLength={maxLength} className={inputClass} onInvalid={event => { const input = event.currentTarget; if (input.validity.valueMissing) input.setCustomValidity(`${label} is required.`); else if (input.validity.typeMismatch || input.validity.patternMismatch || input.validity.tooShort) input.setCustomValidity(`Enter a valid ${label.toLowerCase()}.`); onInvalid?.(event); }} onInput={event => { if (transform === "uppercase") event.currentTarget.value = event.currentTarget.value.toUpperCase().replace(/\s/g, ""); event.currentTarget.setCustomValidity(""); onInput?.(event); }} {...props} /></div>; }
...
function firstInvalidControl(form: HTMLFormElement) { for (const control of Array.from(form.elements)) { if (!(control instanceof HTMLInputElement || control instanceof HTMLSelectElement || control instanceof HTMLTextAreaElement)) continue; if (control.disabled || control.type === "hidden") continue; control.setCustomValidity(""); if (control.dataset.required && !control.value.trim()) return control; if (control instanceof HTMLInputElement && control.type === "email" && control.value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(control.value)) return control; if (control.dataset.minLength && control.value.length < Number(control.dataset.minLength)) return control; if (control.dataset.pattern && control.value && !new RegExp(`^(?:${control.dataset.pattern})$`).test(control.value)) return control; if (!control.validity.valid) return control; } return null; }
