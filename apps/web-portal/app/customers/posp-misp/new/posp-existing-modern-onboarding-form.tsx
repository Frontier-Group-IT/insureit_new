...
export function PospExistingModernOnboardingForm({ initialError = null, initialField = null, initialValues = {}, salesManagers, banks }: Props) {
  ...
  return (
    <div className="w-full pb-24">
      ...
      <form action="/customers/posp-misp/new/legacy-submit" method="post" onSubmitCapture={handleSubmit} data-posp-misp-onboarding-form="true" data-validation-mode="route-post-native-v7" className="w-full">
        <input type="hidden" name="partner_type" value="posp" />
        ...
        <nav className="sticky top-[66px] z-30 mb-4 grid grid-cols-4 overflow-hidden rounded-b-2xl border border-t-0 border-[#D9E2F0] bg-white/95 shadow-[0_7px_18px_rgba(15,23,42,.08)] backdrop-blur" aria-label="Existing POSP onboarding sections">
          <PospNavItem href="#posp-section-1" number="01" label="Identity & contact" />
          <PospNavItem href="#posp-section-2" number="02" label="Address" />
          <PospNavItem href="#posp-section-3" number="03" label="Bank & tax" />
          <PospNavItem href="#posp-section-4" number="04" label="Existing account" last />
        </nav>
        ...
        <div className="space-y-4">
            ...
            <div className="grid min-w-0 gap-3 md:col-span-2 md:grid-cols-2 xl:col-span-4 xl:grid-cols-5">
              ...
              <div className="md:col-span-2 xl:col-span-3">
                ...
                <div className="grid min-w-0 gap-2 md:grid-cols-3">
                  <Field label="First Name" name="pos_first_name" required hideLabel placeholder="First name" defaultValue={initialValues.pos_first_name} error={errorFor("pos_first_name")} />
                  <Field label="Middle Name" name="pos_middle_name" hideLabel placeholder="Middle name" defaultValue={initialValues.pos_middle_name} error={errorFor("pos_middle_name")} />
                  <Field label="Last Name" name="pos_last_name" hideLabel placeholder="Last name" defaultValue={initialValues.pos_last_name} error={errorFor("pos_last_name")} />
                </div>
              </div>
            </div>
            <div className="grid min-w-0 gap-3 md:col-span-2 md:grid-cols-2 xl:col-span-4 xl:grid-cols-5">
              <PanField label="PAN" name="pan_number" defaultValue={initialValues.pan_number} error={errorFor("pan_number")} />
              <Field label="Aadhaar" name="aadhaar_number" required inputMode="numeric" pattern="[0-9]{12}" maxLength={12} minLength={12} defaultValue={initialValues.aadhaar_number} error={errorFor("aadhaar_number")} />
              <IndianDateField label="Date of Birth" name="date_of_birth" required defaultValue={initialValues.date_of_birth} inputClassName={inputClass} />
              <Field label="Mobile" name="applicant_phone" required inputMode="tel" pattern="(?:\+91)?[6-9][0-9]{9}" defaultValue={initialValues.applicant_phone} error={errorFor("applicant_phone")} />
              <Field label="Email" name="applicant_email" type="email" required defaultValue={initialValues.applicant_email} error={errorFor("applicant_email")} />
            </div>
          </PospSection>
          ...
          <PospSection id="posp-section-3" number="03" title="Bank & tax">
            ...
            <Field label="GST Number" name="gst_number" maxLength={15} pattern="[0-9]{2}[A-Za-z]{5}[0-9]{4}[A-Za-z][1-9A-Za-z]Z[0-9A-Za-z]" transform="uppercase" defaultValue={initialValues.gst_number} error={errorFor("gst_number")} />
          </PospSection>

          <PospSection id="posp-section-4" number="04" title="Existing account details">
            <div className="grid min-w-0 gap-3 md:col-span-2 md:grid-cols-2 xl:col-span-4 xl:grid-cols-4">
              ...
              <Field label="Existing POSP ID" name="legacy_registration_code" required placeholder="POSP-2024-00481" defaultValue={initialValues.legacy_registration_code} error={errorFor("legacy_registration_code")} />
...
function SelectField({ label, name, required = false, options, placeholder, error, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string; name: string; options: SelectOption[]; placeholder: string; error?: string | null }) {
  return <div className="min-w-0"><label className={labelClass} htmlFor={name}>{label}{required ? " *" : ""}</label><select id={name} name={name} required={required} aria-invalid={Boolean(error)} aria-describedby={error ? `${name}-error` : undefined} className={`${inputClass} ${error ? "border-red-400 bg-red-50/40" : ""}`} {...props}><option value="">{placeholder}</option>{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{error ? <p id={`${name}-error`} className="mt-1.5 text-[9.5px] font-semibold text-red-600">{error}</p> : null}</div>;
}
...
function StatusSelect({ label, name, value, options, onChange }: { label: string; name: string; value: string; options: readonly StatusOption[]; onChange: (value: string) => void }) {
  return <label className="min-w-0 rounded-xl border border-[#E2E8F0] bg-white p-3"><span className="mb-1.5 block text-[10px] font-semibold text-[#344054]">{label} *</span><select name={name} value={value} required onChange={(event) => onChange(event.currentTarget.value)} className="h-10 w-full rounded-lg border border-[#CBD5E1] bg-white px-2.5 text-[10.5px] font-medium text-[#17203A] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#E0E7FF]">{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>;
}
