...
type Props = { action: (state: DealershipOnboardingState, formData: FormData) => Promise<DealershipOnboardingState>; dealershipType: "posp" | "misp"; oems: Option[]; groups: Option[] };
...
export function DealershipOnboardingForm({ action, dealershipType, oems, groups }: Props) {
    ...
    <>
      ...
      <div className="mx-auto max-w-[1240px] space-y-2 pb-20">
        ...
        <form ref={formRef} action={submit} className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <Section title="Dealership Details" columns="four"><Field label="Dealership Name" name="dealership_name" required placeholder="Registered dealership name" /><Field label="Owner Name" name="owner_name" required placeholder="Dealership owner" /><Field label="Mobile Number" name="phone" required inputMode="tel" maxLength={10} placeholder="10-digit mobile" /><Field label="Email ID" name="email" type="email" placeholder="Optional email" /></Section>
          <Section title="Group Affiliation" columns="four"><Select label="Parent Group" name="group_customer_id" options={groups} emptyLabel="No parent Group" /></Section>
          <Section title="Address" columns="five"><Field label="Street" name="address_street" required placeholder="House, building or street" /><Field label="Locality" name="address_locality" placeholder="Area or locality" /><div className="relative"><label className={labelClass} htmlFor="city_search">City *</label><input id="city_search" name="city_search" required autoComplete="off" value={cityQuery} onChange={(event) => { setCityQuery(event.target.value); setSelectedLocation(null); }} className={inputClass} placeholder="Search city" />{locations.length ? <div className="absolute z-30 mt-1 max-h-52 w-72 overflow-auto rounded-lg border border-[#D8DEE8] bg-white p-1 shadow-xl">{locations.map((location) => <button key={location.id} type="button" className="block w-full rounded-md px-2.5 py-2 text-left hover:bg-[#F8FAFC]" onClick={() => { setSelectedLocation(location); setCityQuery(location.city_name); setStateValue(location.state_name); setPostalCode(location.pincode); setLocations([]); }}><span className="block text-[11px] font-semibold">{location.city_name}</span><span className="text-[10px] text-[#64748B]">{location.district ? `${location.district}, ` : ""}{location.state_name} · {location.pincode}</span></button>)}</div> : null}</div><Field label="State" name="state" required value={stateValue} onChange={(event) => setStateValue(event.target.value)} placeholder="State" /><Field label="PIN Code" name="postal_code" required value={postalCode} onChange={(event) => setPostalCode(event.target.value)} inputMode="numeric" maxLength={6} placeholder="PIN" /><input type="hidden" name="india_location_id" value={selectedLocation?.id ?? ""} /><input type="hidden" name="city" value={selectedLocation?.city_name ?? cityQuery} /></Section>
          <Section title="Business Profile" columns="four"><Select label="Dealership OEM" name="oem_name" required options={oems} emptyLabel="Select OEM" /><Select label="Yearly Sales" name="yearly_sales_band" required options={[{ value: "less_than_500", label: "Less than 500" }, { value: "500_to_1000", label: "500–1000" }, { value: "more_than_1000", label: "More than 1000" }]} emptyLabel="Select yearly sales" /></Section>
          <Section title="GST Details" columns="four" action={<label className="inline-flex cursor-pointer items-center gap-2 text-[11px] font-semibold text-[#334155]"><input type="checkbox" name="is_gst_registered" value="true" checked={gstRegistered} onChange={(event) => setGstRegistered(event.target.checked)} className="h-3.5 w-3.5" /> GST Registered</label>}><Field label="GST Number" name="gst_number" required={gstRegistered} maxLength={15} placeholder="22AAAAA0000A1Z5" /><FileField label="GST Certificate" name="gst_copy" file={files.gst_copy} required={gstRegistered} onChange={(selected) => setFile("gst_copy", selected)} /></Section>
          <Section title={`${representativeLabel} Information`} columns="four"><Field label={`${representativeLabel} Name`} name="representative_name" required placeholder={`${representativeLabel} name`} /><Field label="Mobile Number" name="representative_mobile" required inputMode="tel" maxLength={10} placeholder="10-digit mobile" /><Field label="Email ID" name="representative_email" type="email" placeholder="Optional email" /><Field label="Aadhaar Number" name="representative_aadhaar" required inputMode="numeric" maxLength={12} placeholder="12-digit Aadhaar" /><FileField label="Aadhaar Front" name="representative_aadhaar_front" file={files.representative_aadhaar_front} required onChange={(selected) => setFile("representative_aadhaar_front", selected)} /><FileField label="Aadhaar Back" name="representative_aadhaar_back" file={files.representative_aadhaar_back} required onChange={(selected) => setFile("representative_aadhaar_back", selected)} /><Field label="PAN Card Number" name="representative_pan" required maxLength={10} placeholder="ABCDE1234F" /><FileField label="PAN Card Copy" name="representative_pan_copy" file={files.representative_pan_copy} required onChange={(selected) => setFile("representative_pan_copy", selected)} /></Section>
