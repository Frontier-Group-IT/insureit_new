...
const partnerOptions = [
  ...
  { value: "dealership", label: "Dealership", description: "Vehicle dealer or service partner", available: false },
  { value: "corporate", label: "Corporate", description: "Registered company or enterprise fleet", available: false },
  { value: "group", label: "Group", description: "Multiple linked entities under one group", available: false }
];
    ...
    <>
      ...
      <div className="mx-auto max-w-[1440px] space-y-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-[20px] font-semibold tracking-tight text-[#0F172A]">Customers</h2>
            <p className="mt-1 text-[12px] text-[#64748B]">Manage customer profiles, KYC status and linked commercial vehicles.</p>
          </div>
...
function PartnerTypeModal({ onClose }: { onClose: () => void }) {
  return <div className="fixed inset-0 z-[120] grid place-items-center bg-[#0F172A]/35 px-4 backdrop-blur-[2px]" role="dialog" aria-modal="true"><div className="w-full max-w-[720px] rounded-2xl border border-white/60 bg-white p-5 shadow-[0_28px_80px_rgba(15,23,42,0.28)]"><div className="flex items-start justify-between gap-4"><div><h3 className="text-[18px] font-semibold text-[#0F172A]">Select partner type</h3><p className="mt-1 text-[11.5px] text-[#64748B]">Choose the customer relationship before starting onboarding.</p></div><button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-md border border-[#E2E8F0] text-[#64748B]">×</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{partnerOptions.map((option) => option.available ? <Link key={option.value} href={`/customers/new?partner_type=${option.value}`} className="group rounded-xl border border-[#CBD5E1] p-4 transition hover:border-[#6366F1] hover:bg-[#F8FAFF]"><div className="flex items-center justify-between"><p className="text-[13px] font-semibold text-[#0F172A]">{option.label}</p><span className="text-[#4F46E5]">→</span></div><p className="mt-1 text-[10.5px] leading-4 text-[#64748B]">{option.description}</p></Link> : <button key={option.value} type="button" disabled className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-4 text-left opacity-75"><div className="flex items-center justify-between"><p className="text-[13px] font-semibold text-[#475569]">{option.label}</p><span className="rounded-full bg-amber-100 px-2 py-0.5 text-[9px] font-semibold text-amber-700">Coming soon</span></div><p className="mt-1 text-[10.5px] leading-4 text-[#94A3B8]">{option.description}</p></button>)}</div></div></div>;
}

function StatusPill({ status }: { status: string }) {
  const active = status === "active";
  return <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[9.5px] font-semibold ${active ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}><span className={`h-1.5 w-1.5 rounded-full ${active ? "bg-emerald-500" : "bg-amber-500"}`} />{active ? "Active" : "KYC incomplete"}</span>;
}
