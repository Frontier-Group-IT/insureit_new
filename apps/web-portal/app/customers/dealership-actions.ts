...
export async function createDealershipOnboarding(_state: DealershipOnboardingState, formData: FormData): Promise<DealershipOnboardingState> {
  ...
  const groupCustomerId = text(formData, "group_customer_id");
  ...
  const requiredFiles: Array<[string, string, boolean]> = [["gst_copy", "GST certificate", isGstRegistered], ["representative_aadhaar_front", "Aadhaar front", true], ["representative_aadhaar_back", "Aadhaar back", true], ["representative_pan_copy", "PAN copy", true]];
  ...
  if (groupCustomerId) {
    ...
    if (!group) return fail("Select a valid active Group customer.", "group_customer_id");
  }
  ...
  try {
    application = await beginPortalOnboardingApplication(admin, { initiatedBy: profile.id, partnerType: "dealership", phone, email, draftData: { dealership_type: dealershipType, dealership_name: dealershipName, owner_name: ownerName, city, state, postal_code: postalCode, oem_name: oemName, yearly_sales_band: yearlySalesBand, is_gst_registered: isGstRegistered, group_customer_id: groupCustomerId } });
  ...
  const { data: customer, error: customerError } = await admin.from("customers").insert({ customer_code: `CUST-${Date.now().toString().slice(-9)}`, partner_type: "dealership", contact_name: ownerName, company_name: dealershipName, legal_trade_name: dealershipName, phone, email, address, address_street: addressStreet, address_locality: addressLocality, india_location_id: locationId, city, state, postal_code: postalCode, is_gst_registered: isGstRegistered, gst_number: isGstRegistered ? gstNumber : null, onboarding_status: "active", created_by: profile.id, updated_by: profile.id }).select("id").single<{ id: string }>();
  ...
  const { error: representativeError } = await admin.from("dealership_representatives").insert({ customer_id: customer.id, representative_type: dealershipType, representative_name: representativeName, mobile: representativePhone, email: representativeEmail, aadhaar_last_four: aadhaarNumber.slice(-4), aadhaar_hash: createHash("sha256").update(aadhaarNumber).digest("hex"), pan_number: panNumber });
  ...
  const documentMap: Array<[string, string]> = [["gst_copy", "gst_copy"], ["representative_aadhaar_front", "aadhaar_front"], ["representative_aadhaar_back", "aadhaar_back"], ["representative_pan_copy", "pan_copy"]];
