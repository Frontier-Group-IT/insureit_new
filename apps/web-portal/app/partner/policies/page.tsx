        ...
        <section>
            ...
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {lifecycles.map((item) => {
                  ...
                  return <Link key={item.value} href={hrefFor({ lifecycle: item.value, page: 1 })} className={"rounded-xl px-3 py-2 text-[10px] font-bold " + (active ? "bg-[#3156B8] text-white" : "border border-[#D8E0EA] bg-white text-[#4D617D]")}>{item.label}</Link>;
                })}
              </div>
            </div>
          </div>
          ...
          {rows.length ? (
            <div className="divide-y divide-[#E8EDF4]">
              {rows.map((row) => (
                <Link key={row.policy_id} href={"/partner/policies/" + encodeURIComponent(row.policy_id)} prefetch={false} className="group grid gap-3 px-5 py-4 transition hover:bg-[#F8FAFD] sm:px-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)_minmax(140px,.65fr)_minmax(120px,.55fr)_auto] xl:items-center">
