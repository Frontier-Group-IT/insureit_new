...
function validMode(value?: string): PartnerExternalRenewalMode {
  return value === "expired" || value === "future" ? value : "due";
}
...
export default async function PartnerExternalRenewalsPage({
  ...
  searchParams: Promise<{ q?: string; mode?: string; window?: string; page?: string }>;
}) {
  ...
  const [summary, rows] = await Promise.all([
    getPartnerExternalRenewalSummary(),
    listPartnerExternalRenewals({ limit: PAGE_SIZE, offset, search: q, mode, window }),
  ]);
  ...
  const hrefFor = (next: { mode?: PartnerExternalRenewalMode; window?: PartnerExternalRenewalWindow; page?: number }) => {
    ...
    <PartnerPortalShell title="External Renewal Opportunities">
      <div className="space-y-7">
        <PartnerPageHeader
          ...
          description="External policies are kept separate from verified INSUREIT business."
          action={
        ...
        <PartnerMetricStrip
          items={[
            { label: "Due 0–7 Days", value: summary.due_0_7_count, meta: "External only" },
            { label: "Due 8–15 Days", value: summary.due_8_15_count, meta: "External only" },
            { label: "Due 16–30 Days", value: summary.due_16_30_count, meta: "External only" },
            { label: "Expired 30 Days", value: summary.expired_30_count, meta: "Recovery opportunities" },
          ]}
        />
        ...
        <section>
          <div className="border-y border-[#DCE4ED] py-3">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              <div className="flex flex-wrap gap-2">
                ...
                <Link href={hrefFor({ mode: "expired", window: "all", page: 1 })} className={"rounded-lg px-3 py-2 text-[10px] font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3156B8]/20 " + (mode === "expired" ? "bg-[#3156B8] text-white" : "border border-[#D8E0EA] bg-white text-[#4D617D]")}>Recently Expired</Link>
                <Link href={hrefFor({ mode: "future", window: "all", page: 1 })} className={"rounded-lg px-3 py-2 text-[10px] font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3156B8]/20 " + (mode === "future" ? "bg-[#3156B8] text-white" : "border border-[#D8E0EA] bg-white text-[#4D617D]")}>Future</Link>
              </div>
          ...
          <div className="mt-5">
            <PartnerSectionHeading
              title={mode === "expired" ? "Recently expired opportunities" : mode === "future" ? "Future opportunities" : "30-day opportunity worklist"}
              description={rows.length + " shown · " + total + " matched"}
            />
          </div>
