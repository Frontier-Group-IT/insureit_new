...
export function PartnerNetworkStructure({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-[#DFE7F2] bg-white shadow-[0_8px_24px_rgba(49,86,184,0.05)]">
      <div className="flex flex-col gap-3 border-b border-[#E3EAF4] bg-[#F1F6FD] px-4 py-3.5 lg:flex-row lg:items-center lg:justify-between">
        ...
        {sections.length ? <div className="space-y-2">{sections.map((section) => {
          ...
          return <div key={section.key} className="overflow-hidden rounded-lg border border-[#E1E8F2] bg-white">
            ...
            <div className="divide-y divide-[#E8EDF4]">{section.rows.map((row) => {
              ...
              return <details key={row.partner_id} className="group bg-white">
                ...
                <div className="border-t border-[#EDF1F6] bg-[#FBFDFF] px-3.5 pb-3 pt-2.5">
                  ...
                  {branches.length ? <div className="mt-2.5 rounded-lg border border-[#DCE7F5] bg-[#F5F9FF] p-2">
                    ...
                    <div className="space-y-1.5">{branches.map((branch) => <details key={branch.partner_id} className="group/branch overflow-hidden rounded-lg border border-[#DFE8F3] bg-white">
                      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 marker:content-none [&::-webkit-details-marker]:hidden">
                        ...
                        <div className="flex shrink-0 items-center gap-4">
                          <div className="hidden text-right sm:block"><p className="text-[7px] font-black uppercase tracking-[0.06em] text-[#8795A8]">Premium This Month</p><p className="mt-1 text-[10px] font-extrabold text-[#2563EB]">{currency(branch.metrics.premium_this_month)}</p></div>
                          <ChevronDown className="h-3.5 w-3.5 text-[#617895] transition-transform group-open/branch:rotate-180" />
                        </div>
                      </summary>
                      <div className="border-t border-[#EDF1F6] bg-[#FBFDFF] px-3 py-2.5">
                        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
                          <InlineMetric label="Policies" value={branch.metrics.total_policies} Icon={FileText} tone="blue" />
                          <InlineMetric label="Customers" value={branch.metrics.total_customers} Icon={UsersRound} tone="green" />
                          <InlineMetric label="This Month" value={branch.metrics.policies_this_month} Icon={BarChart3} tone="purple" />
                          <InlineMetric label="Renewals 30d" value={branch.metrics.renewals_30_days} Icon={RefreshCcw} tone="orange" />
                          <InlineMetric label="Active Claims" value={branch.metrics.active_claims} Icon={ShieldCheck} tone="pink" />
                        </div>
                      </div>
                    </details>)}</div>
                  </div> : null}
