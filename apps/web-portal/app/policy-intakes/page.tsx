...
export default async function PolicyIntakesPage() {
  ...
  return <AppShell title={reviewer ? "Policy Intakes" : "My Policy Intakes"}>
    {error ? <div className="mx-auto max-w-[1480px] rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[10px] font-semibold text-red-700">Policy Intakes are temporarily unavailable.</div> : <PolicyIntakeWorkspace rows={data ?? []} reviewer={reviewer} creator={creator} currentProfileId={profile.id} />}
  </AppShell>;
}
