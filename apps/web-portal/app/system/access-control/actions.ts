...
import { permissionDefinitions, type PermissionAccess, type PermissionScope } from "@/lib/permission-management";

const editableRoles = new Set(["it_super_user", "super_admin"]);
const allowedAccess = new Set<PermissionAccess>(["inherit", "none", "view", "edit", "approve"] as PermissionAccess[]);
const allowedScope = new Set<PermissionScope>(["inherit", "self", "hierarchy", "organization"] as PermissionScope[]);
...
export async function saveEmployeePermissionOverride(formData: FormData) {
  ...
  const access = String(formData.get("access_level") ?? "inherit") as PermissionAccess;
  ...
  if (!profileId || !permissionDefinitions.some((item) => item.capability === capability)) redirect(`${returnTo}?error=Invalid permission selection`);
  if (!allowedAccess.has(access) || !allowedScope.has(scope)) redirect(`${returnTo}?error=Invalid access configuration`);
  if (!reason || reason.length < 5) redirect(`${returnTo}?error=Please enter a clear reason for the change`);
  if (profileId === actor.id && (capability === "manage_users" || capability === "manage_system") && access === "none") redirect(`${returnTo}?error=You cannot remove your own critical administration access`);

  const admin = createSupabaseAdminClient();
  const { data: target } = await admin.from("profiles").select("id,role,is_active").eq("id", profileId).maybeSingle();
  if (!target?.id || !target.is_active || !isAppRole(target.role)) redirect(`${returnTo}?error=The selected active portal user could not be found`);
  ...
  redirect(`${returnTo}?success=Permission updated`);
}
...
export async function resetEmployeePermissionOverrides(formData: FormData) {
  ...
  const profileId = String(formData.get("profile_id") ?? "");
  const reason = String(formData.get("reason") ?? "").trim();
  if (!profileId || reason.length < 5) redirect(`/system/access-control/employees/${profileId}?error=Please enter a clear reset reason`);
  ...
  revalidatePath("/system/access-control");
  revalidatePath(`/system/access-control/employees/${profileId}`);
  redirect(`/system/access-control/employees/${profileId}?success=Custom permissions reset`);
}
