...
Large campaign exports must page campaign members and call attempts instead of relying on the Supabase default row cap, and must chunk UUID `.in(...)` lookups for opportunities and attempt events. Never reintroduce a fixed 100-member export ceiling: campaign upload capacity and report capacity must remain aligned. The export remains IT-Super-User-only and privacy-safe.
