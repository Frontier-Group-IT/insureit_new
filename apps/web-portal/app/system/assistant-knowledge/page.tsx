...
function LifecycleForm({ action, id, label }: { action: (data: FormData) => Promise<void>; id: string; label: string }) {
  return <form action={action}><input type="hidden" name="entry_id" value={id} /><button className={`rounded-lg px-3 py-2 text-[9px] font-semibold ${label === "Publish" ? "bg-emerald-600 text-white" : "border border-red-200 bg-red-50 text-red-700"}`}>{label}</button></form>;
}
