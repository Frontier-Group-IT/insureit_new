...
export function TrainingReviewQueue({ rows, canTrain }: { rows: TrainingQueueRow[]; canTrain: boolean }) {
  ...
  const visibleRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesFilter = filter === "all" || (filter === "failed" ? ["failed", "exhausted"].includes(row.processingStatus) : filter === "exact_match" ? isExactDatabaseMatch(row) : row.status === filter);
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex flex-wrap gap-2">
          {FILTERS.map((item) => {
            const count = rows.filter((row) => item === "all" || (item === "failed" ? ["failed", "exhausted"].includes(row.processingStatus) : item === "exact_match" ? isExactDatabaseMatch(row) : row.status === item)).length;
            return (
      ...
      {row.proposal?.warnings.length ? (
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3">
          ...
          <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-amber-900">
            {row.proposal.warnings.map((warning) => (
...
function statusLabel(row: TrainingQueueRow) {
  ...
  return row.status === "reviewed" ? "Ready to approve" : row.status.replaceAll("_", " ");
}
