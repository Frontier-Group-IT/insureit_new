...
export default async function EditPolicyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isNonMotor) {
    ...
    const nonMotorInitialValues: NonMotorUnifiedInitialValues = {
      ...
      odPremium: stringValue((premium?.od_premium ?? 0) > 0 ? premium?.od_premium : premium?.net_premium),
      tpPremium: "0",
      netPremium: stringValue((premium?.od_premium ?? 0) > 0 ? premium?.od_premium : premium?.net_premium),
  ...
  const initialValues: PolicyUnifiedInitialValues = {
    ...
    validFrom: policy.start_date,
    validUpto: policy.end_date,
    payoutBasis: commercialAccess ? (payout?.payout_basis === "FIXED_AMOUNT" ? "FLAT" : (payin?.payout_basis ?? "NET")) : "",
    ...
    flatPayoutAmount: commercialAccess && payout?.payout_basis === "FIXED_AMOUNT" ? stringValue(payout?.partner_payout_amount ?? payout?.gross_payout) : "",
