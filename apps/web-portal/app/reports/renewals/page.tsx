...
export default async function RenewalReportsPage({searchParams}:Props){
  ...
  return <AppShell title="Reports"><ReportPageShell
    ...
    controls={<>
      <ReportQueryShortcuts label="Horizon" param="horizon" activeValue={String(filters.horizonDays)} options={HORIZONS}/>
      <form action="/reports/renewals" method="get" className="grid gap-2 md:grid-cols-2 xl:grid-cols-[120px_minmax(190px,1fr)_minmax(170px,1fr)_minmax(190px,1fr)_160px_auto_auto]">
        ...
        <ReportFilterField label="Relationship manager"><select name="rm" defaultValue={filters.rmName??""} className={reportInputClass}><option value="">All RMs</option>{report.filters.rms.map(x=><option key={x} value={x}>{x}</option>)}</select></ReportFilterField>
        ...
        <ReportFilterField label="Expiry bucket"><select name="bucket" defaultValue={filters.bucket??""} className={reportInputClass}><option value="">All buckets</option>{bucketOptions().map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select></ReportFilterField>
        <ReportApplyButton/>
        <ReportResetLink href="/reports/renewals"/>
      </form>
    </>}
  >
