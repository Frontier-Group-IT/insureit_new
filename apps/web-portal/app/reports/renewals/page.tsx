...
function href(base:string,f:RenewalFilters,bucket?:RenewalBucket|null,page?:number){const p=new URLSearchParams();p.set("horizon",String(f.horizonDays));if(f.insurerId)p.set("insurer",f.insurerId);if(f.rmEmployeeId)p.set("rm",f.rmEmployeeId);if(f.intermediaryCode)p.set("intermediary",f.intermediaryCode);if(bucket)p.set("bucket",bucket);if(page&&page>1)p.set("page",String(page));return `${base}?${p.toString()}`}
...
function fallbackFilters():RenewalFilters{return{horizonDays:365,insurerId:null,rmEmployeeId:null,intermediaryCode:null,bucket:null,page:1}}
