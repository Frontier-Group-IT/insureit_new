  ...
  return (
    <AppShell title="Reports">
      ...
      >
        ...
        <section className="grid gap-4 xl:grid-cols-2 print:grid-cols-2">
          <article className="portal-card overflow-hidden print:break-inside-avoid"><Header title={archived ? "Renewal exposure" : "Current renewal exposure"} /><div className="grid gap-3 p-5 sm:grid-cols-3"><Mini label="Due ≤ 30d" value={integer(pack.renewals.summary.due_30_count)} /><Mini label="Due ≤ 90d" value={integer(pack.renewals.summary.due_90_count)} /><Mini label="Net premium at risk" value={currency(pack.renewals.summary.premium_at_risk)} /></div><RenewalTable rows={pack.renewals.buckets} /></article>
...
function TopBusiness({ insurers, rms }: { insurers: Array<{ id: string; name: string; policy_count: number; net_premium: number; gross_premium?: number }>; rms: Array<{ name: string; policy_count: number; net_premium: number; gross_premium?: number }> }) { return <div className="grid border-t border-[#edf0f4] lg:grid-cols-2"><SimpleRows title="Insurance company" rows={insurers.map(x => [x.name, `${integer(x.policy_count)} · ${currency(preferredPremium(x.net_premium, x.gross_premium))}`])} /><SimpleRows title="Relationship manager" rows={rms.map(x => [x.name, `${integer(x.policy_count)} · ${currency(preferredPremium(x.net_premium, x.gross_premium))}`])} /></div>; }
function DistributionTable({ rows }: { rows: Array<{ name: string; intermediary_count: number; policy_count: number; customer_count: number; net_premium: number; gross_premium?: number }> }) { return <SimpleRows title="RM performance" rows={rows.map(x => [x.name, `${integer(x.intermediary_count)} partners · ${integer(x.policy_count)} policies · ${currency(preferredPremium(x.net_premium, x.gross_premium))}`])} />; }
...
function RenewalTable({ rows }: { rows: Array<{ label: string; policy_count: number; net_premium: number; gross_premium?: number }> }) { return <SimpleRows title="Renewal buckets" rows={rows.map(x => [x.label, `${integer(x.policy_count)} · ${currency(preferredPremium(x.net_premium, x.gross_premium))}`])} />; }
...
function preferredPremium(netPremium: number | undefined, legacyGross?: number) { return Number.isFinite(netPremium) ? Number(netPremium) : Number(legacyGross ?? 0); }
function legacyNumber(value: unknown, key: string) { if (!value || typeof value !== "object") return undefined; const raw = (value as Record<string, unknown>)[key]; const parsed = typeof raw === "number" ? raw : Number(raw); return Number.isFinite(parsed) ? parsed : undefined; }
