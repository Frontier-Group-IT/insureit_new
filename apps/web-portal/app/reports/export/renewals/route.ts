...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    const headers=["Expiry Date","Days to Expiry","Bucket","Policy No","Business Line","Category","Customer","Customer Code","Risk / Asset","Insurance Company","Relationship Manager","Intermediary Type","Intermediary Code","Gross Premium","Status"];
    const lines=[headers.map(csv).join(",")];
    for(const r of rows)lines.push([r.end_date,r.days_to_expiry,r.renewal_bucket,r.policy_no,r.business_line,r.category,r.customer_name,r.customer_code,r.risk_reference||r.vehicle_no,r.insurer_name,r.rm_name,r.intermediary_type??"",r.intermediary_code??"",r.gross_premium,r.status].map(csv).join(","));
...
function toQuery(s:URLSearchParams):RenewalQuery{return{horizon:s.get("horizon")??undefined,insurer:s.get("insurer")??undefined,rm:s.get("rm")??undefined,intermediary:s.get("intermediary")??undefined,business:s.get("business")??undefined,category:s.get("category")??undefined,bucket:s.get("bucket")??undefined}}
