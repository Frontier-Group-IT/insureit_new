...
function InsurerTable({ rows }: { rows: FinanceReport["insurers"] }) {
  ...
  return <div className="overflow-x-auto"><table className="w-full min-w-[760px]"><thead><tr className="bg-[#f8fafc] text-[8px] font-black uppercase tracking-[.07em] text-[#7c899b]"><th className="px-5 py-3 text-left">Insurer</th><th className="px-3 py-3 text-right">Policies</th><th className="px-3 py-3 text-right">Projected Pay-in</th><th className="px-3 py-3 text-right">Partner Payout</th><th className="px-5 py-3 text-right">Projected Margin</th></tr></thead><tbody className="divide-y divide-[#edf0f4]">{rows.map((x, i) => {const logo=getInsurerLogo(x.insurer_name);return <tr key={`${x.id ?? "none"}-${i}`} className="text-[10px]"><td className="px-5 py-3.5 font-semibold"><div className="flex min-w-0 items-center gap-2">{logo?<Image src={logo} alt={x.insurer_name?`${x.insurer_name} logo`:"Insurance company"} width={24} height={24} className="max-h-6 max-w-6 shrink-0 object-contain"/>:null}<span className="min-w-0 truncate">{x.insurer_name}</span></div></td><td className="px-3 py-3.5 text-right">{integer(x.policy_count)}</td><td className="px-3 py-3.5 text-right">{currency(x.projected_payin)}</td><td className="px-3 py-3.5 text-right">{currency(x.gross_payout)}</td><td className="px-5 py-3.5 text-right font-bold">{currency(x.projected_payin - x.gross_payout)}</td></tr>})}</tbody></table></div>;
}
...
function Register({ rows }: { rows: FinanceReport["register"]["rows"] }) {
  if (!rows.length) return <Empty />;
  return <div className="overflow-x-auto"><table className="w-full min-w-[1160px]"><thead><tr className="bg-[#f8fafc] text-[8px] font-black uppercase tracking-[.07em] text-[#7c899b]"><th className="px-5 py-3 text-left">Business date</th><th className="px-3 py-3 text-left">Policy</th><th className="px-3 py-3 text-left">Customer / risk</th><th className="px-3 py-3 text-left">Insurer</th><th className="px-3 py-3 text-left">RM / intermediary</th><th className="px-3 py-3 text-right">Net Premium</th><th className="px-3 py-3 text-right">Projected Pay-in</th><th className="px-3 py-3 text-right">Partner Payout</th><th className="px-3 py-3 text-right">Projected Margin</th><th className="px-5 py-3 text-center">Open</th></tr></thead><tbody className="divide-y divide-[#edf0f4]">{rows.map((x) => {const logo=getInsurerLogo(x.insurer_name);return <tr key={x.id} className="text-[9.5px] hover:bg-[#fbfcfe]"><td className="px-5 py-3.5 font-semibold"><div className="flex items-center gap-2">{logo?<Image src={logo} alt={x.insurer_name?`${x.insurer_name} logo`:"Insurance company"} width={22} height={22} className="max-h-[22px] max-w-[22px] shrink-0 object-contain"/>:null}<span>{date(x.business_date)}</span></div></td><td className="px-3 py-3.5"><p className="font-bold">{x.policy_no}</p><p className="text-[8px] text-[#8490a1]">{x.business_line} · {x.category}</p></td><td className="px-3 py-3.5"><p className="font-semibold">{x.customer_name}</p><p className="text-[8px] text-[#8490a1]">{x.risk_reference || x.vehicle_no || "—"}</p></td><td className="px-3 py-3.5">{x.insurer_name}</td><td className="px-3 py-3.5"><p className="font-semibold">{x.rm_name ?? "Unassigned"}</p><p className="text-[8px] text-[#8490a1]">{x.intermediary_code ?? "—"}</p></td><td className="px-3 py-3.5 text-right">{currency(x.net_premium)}</td><td className="px-3 py-3.5 text-right">{currency(x.projected_payin)}</td><td className="px-3 py-3.5 text-right">{currency(x.gross_payout)}</td><td className="px-3 py-3.5 text-right font-bold">{currency(x.projected_payin - x.gross_payout)}</td><td className="px-5 py-3.5 text-center"><Link href={`/policies/${x.id}`} className="inline-grid h-8 w-8 place-items-center rounded-lg border border-[#d9e1ec] text-[#425b8f]"><ExternalLink className="h-3.5 w-3.5" /></Link></td></tr>})}</tbody></table></div>;
}
