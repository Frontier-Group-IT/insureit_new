...
export default async function ReportsPage({ searchParams }: Props) {
  ...
  return <AppShell title="Reports"><ReportPageShell title="Policy production & portfolio" loadError={loadError} actions={<ReportExportLink href={exportHref} />} controls={
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Policies" value={integer(report.summary.policy_count)} detail={`Motor ${integer(report.summary.motor_policy_count)} · Non-Motor ${integer(report.summary.non_motor_policy_count)}`} /><Metric label="Gross premium" value={currency(report.summary.gross_premium)} detail={`Motor ${compactCurrency(report.summary.motor_gross_premium)} · Non-Motor ${compactCurrency(report.summary.non_motor_gross_premium)}`} /><Metric label="Net premium" value={currency(report.summary.net_premium)} /><Metric label="Average premium" value={currency(report.summary.average_premium)} /><Metric label="Intermediaries" value={integer(report.summary.intermediary_count)} /></section>
...
function Composition({report,businessLine}:{report:PolicyBusinessReport;businessLine:"Motor"|"Non Motor"|null}){const rows=businessLine==="Motor"?[{label:"Own damage",value:report.summary.od_premium},{label:"Third party",value:report.summary.tp_premium},{label:"CPA",value:report.summary.cpa_amount}]:businessLine==="Non Motor"?report.category_mix.map(x=>({label:x.category,value:x.gross_premium})): [{label:"Motor",value:report.summary.motor_gross_premium},{label:"Non Motor",value:report.summary.non_motor_gross_premium}];const total=Math.max(rows.reduce((sum,row)=>sum+row.value,0),1);if(!rows.length)return <Empty/>;return <div className="mt-5 space-y-4">{rows.map(x=><div key={x.label}><div className="flex items-end justify-between gap-3"><p className="truncate text-[10.5px] font-semibold text-[#536174]">{x.label}</p><p className="text-[13px] font-bold tabular-nums text-[#21324f]">{currency(x.value)}</p></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#edf1f6]"><div className="h-full rounded-full bg-[#516dab]" style={{width:`${Math.min((x.value/total)*100,100)}%`}}/></div></div>)}</div>}
