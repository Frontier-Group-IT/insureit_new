...
export function EmployeeDirectoryWorkspace({ employees, managers, canManage, initialQuery, initialStatus, loadError }: { employees: EmployeeRow[]; managers: ManagerOption[]; canManage: boolean; initialQuery: string; initialStatus: string; loadError: string | null }) {
  ...
  const portalCount = filtered.filter((employee) => employee.profile_id).length;
    ...
    <section className="rounded-lg border border-[#D7E6F5] bg-white p-4 shadow-[0_3px_12px_rgba(7,29,73,0.05)]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center"><Summary value={filtered.length} label="Listed" /><Summary value={activeCount} label="Active" /><Summary value={portalCount} label="Portal" /></div>
        </div>
      ...
      {loadError ? <p className="mt-4 rounded-md border border-red-200 bg-red-50 p-3 text-[11px] text-red-700">{loadError}</p> : filtered.length ? <EmployeeTable employees={filtered} managers={managers} canManage={canManage} /> : <div className="mt-4 rounded-md border border-dashed border-[#CBD8E8] bg-[#F8FBFF] p-8 text-center text-[11px] text-[#667085]">No employees match these filters.</div>}
    </section>
  );
}

function EmployeeTable({ employees, managers, canManage }: { employees: EmployeeRow[]; managers: ManagerOption[]; canManage: boolean }) {
