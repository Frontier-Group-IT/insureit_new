...
export default async function DashboardPage() {
  ...
  const dashboardPromise = getOperationsDashboardData(supabase);
  ...
  const [
    ...
    dashboardPromise,
    profile?.id ? getAccessibleIntermediaryApplicationIds(profile.id, profile.role) : Promise.resolve([]),
    hasEffectiveCapability(profile, "manage_customers", "edit"),
    hasEffectiveCapability(profile, "review_kyc", "edit"),
  ]);
  ...
  return (
    <ClaimManagerShell title="Operations Dashboard" activeNav="dashboard">
      <div className="mx-auto max-w-[1540px] space-y-5 pb-8">
        <section className="relative overflow-hidden rounded-[28px] bg-[#111a35] px-4 py-5 text-white shadow-[0_28px_80px_rgba(17,26,53,.24)] sm:px-7 sm:py-6 lg:px-8">
...
function MetricCard({ metric, delay }: { metric: Metric; delay: number }) { const Icon = metric.icon; return <Link href={metric.href} prefetch={false} style={{ animationDelay: `${delay}ms` }} className="portal-card portal-card-hover group animate-portal-enter p-4 sm:p-5"><div className={`pointer-events-none absolute -right-12 -top-14 h-36 w-36 rounded-full ${metric.glow} blur-3xl`} /><div className="relative flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#7a879a]">{metric.label}</p><p className="portal-display mt-3 text-[32px] font-semibold leading-none text-[#13203b]">{metric.value.toLocaleString("en-IN")}</p></div><span className={`grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br ${metric.gradient} text-white shadow-[0_14px_28px_rgba(37,48,78,.18)] transition group-hover:-rotate-3 group-hover:scale-105`}><Icon className="h-5 w-5" /></span></div><div className="relative mt-5 flex items-end justify-between gap-3 border-t border-[#edf0f5] pt-3"><p className="text-[9.8px] leading-4 text-[#718096]">{metric.supporting}</p><ArrowUpRight className="h-4 w-4 shrink-0 text-[#9aa5b6] group-hover:text-[#6759ff]" /></div></Link>; }
function FocusCard({ item }: { item: FocusItem }) { const Icon = item.icon; return <Link href={item.href} prefetch={false} className="group flex items-center gap-3 rounded-2xl border border-[#e5e9f0] bg-[#fafbfe] p-3.5 hover:-translate-y-0.5 hover:border-[#d4cffb] hover:bg-white hover:shadow-[0_14px_32px_rgba(29,40,70,.08)]"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${item.tone}`}><Icon className="h-[18px] w-[18px]" /></span><div className="min-w-0 flex-1"><div className="flex items-baseline justify-between gap-2"><p className="truncate text-[10.5px] font-bold text-[#1b2943]">{item.label}</p><p className="portal-display text-[22px] font-semibold text-[#14213c]">{item.value}</p></div><p className="mt-0.5 truncate text-[9.5px] text-[#7b879a]">{item.detail}</p></div></Link>; }
function PortfolioCard({ item }: { item: OperationsDashboardData["portfolio"][number] }) { const Icon = partnerIcons[item.key] ?? Building2; return <div className="group rounded-2xl border border-[#e5e9f0] bg-[#fafbfe] p-3 hover:-translate-y-0.5 hover:bg-white hover:shadow-[0_12px_28px_rgba(29,40,70,.07)]"><div className="flex items-center justify-between"><span className="grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-[#eeeaff] to-[#e5fbfa] text-[#6759ff]"><Icon className="h-4 w-4" /></span><p className="portal-display text-[21px] font-semibold text-[#14213c]">{item.value}</p></div><p className="mt-3 truncate text-[9.5px] font-semibold text-[#718096]">{item.label}</p></div>; }
function ActivityList({ title, eyebrow, href, rows }: { title: string; eyebrow: string; href: string; rows: Array<{ id: string; href: string; title: string; subtitle: string; status: string; updatedAt: string }> }) { return <section className="portal-card overflow-hidden"><div className="p-4 sm:p-5"><SectionTitle eyebrow={eyebrow} title={title} href={href} /></div><div className="divide-y divide-[#edf0f5] border-t border-[#edf0f5]">{rows.length ? rows.map((row) => <Link key={row.id} href={row.href} prefetch={false} className="group grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3.5 hover:bg-[#fafbff] sm:px-5"><div className="min-w-0"><p className="truncate text-[11px] font-bold text-[#1b2943]">{row.title}</p><p className="mt-0.5 truncate text-[9.5px] text-[#7b879a]">{row.subtitle}</p></div><div className="text-right"><StatusPill status={row.status} /><p className="mt-1 text-[8.8px] font-medium text-[#9aa5b6]">{relativeTime(row.updatedAt)}</p></div></Link>) : <div className="px-5 py-12 text-center text-[10.5px] text-[#7b879a]">No records available.</div>}</div></section>; }
function SectionTitle({ eyebrow, title, href }: { eyebrow: string; title: string; href: string }) { return <div className="flex items-start justify-between gap-4"><div><p className="text-[8.5px] font-black uppercase tracking-[0.16em] text-[#6759ff]">{eyebrow}</p><h2 className="mt-1 text-[16px] font-semibold text-[#14213c]">{title}</h2></div><Link href={href} prefetch={false} className="inline-flex items-center gap-1 rounded-xl border border-[#e1e5ed] bg-white px-2.5 py-1.5 text-[9px] font-bold text-[#5f6d82] hover:border-[#cfc8ff] hover:text-[#6759ff]">Open<ArrowUpRight className="h-3.5 w-3.5" /></Link></div>; }
