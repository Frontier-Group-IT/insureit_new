...
function ActivityList({ title, eyebrow, href, rows }: { title: string; eyebrow: string; href: string; rows: Array<{ id: string; href: string; title: string; subtitle: string; status: string; updatedAt: string }> }) { return <section className="portal-card overflow-hidden"><div className="p-4 sm:p-5"><SectionTitle eyebrow={eyebrow} title={title} href={href} /></div><div className="divide-y divide-[#edf0f5] border-t border-[#edf0f5]">{rows.length ? rows.map((row) => <Link key={row.id} href={row.href} className="group grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3.5 hover:bg-[#fafbff] sm:px-5"><div className="min-w-0"><p className="truncate text-[11px] font-bold text-[#1b2943]">{row.title}</p><p className="mt-0.5 truncate text-[9.5px] text-[#7b879a]">{row.subtitle}</p></div><div className="text-right"><StatusPill status={row.status} /><p className="mt-1 text-[8.8px] font-medium text-[#9aa5b6]">{relativeTime(row.updatedAt)}</p></div></Link>) : <div className="px-5 py-12 text-center text-[10.5px] text-[#7b879a]">No records available.</div>}</div></section>; }
