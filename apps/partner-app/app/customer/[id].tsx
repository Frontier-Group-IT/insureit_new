...
export default function CustomerDetailScreen() {
  ...
  return (
    <PartnerScreen eyebrow="BUSINESS" title="Customer" onBack={() => router.back()}>
      {loading ? (
        ...
        <>
          <View style={styles.hero}>
            <View style={styles.heroTop}>
              ...
              <View style={styles.heroBody}><Text style={styles.heroName}>{data.customer.customer_name}</Text><Text style={styles.heroMeta}>{[data.customer.city, data.customer.state].filter(Boolean).join(', ') || 'Location not recorded'}{data.customer.customer_code ? ` · ${data.customer.customer_code}` : ''}</Text></View>
          ...
          <View style={styles.summary}><Summary value={data.summary.policies} label="Policies" /><Summary value={data.summary.vehicles} label="Vehicles" /><Summary value={data.summary.claims} label="Claims" /><Summary value={data.summary.renewals_30_days} label="Renewals" /></View>

          <PartnerSectionHeader title="Relationship" />
          <View style={styles.relationshipCard}><Info label="Customer type" value={humanize(data.customer.customer_type || 'not recorded')} /><Info label="Fleet" value={humanize(data.customer.fleet_size_band || 'not recorded')} /><Info label="Intermediary" value={data.customer.intermediary_code || 'Organization / unassigned'} /><Info label="Status" value={humanize(data.customer.status || 'not recorded')} /></View>
          ...
          {data.policies.length ? (
            <View style={styles.stack}>{visiblePolicies.map((policy) => (
              <Pressable accessibilityRole="button" accessibilityLabel={`Open policy ${policy.policy_no || policy.policy_code || ''}`} key={policy.policy_id} onPress={() => router.push(`/policy/${policy.policy_id}` as never)} style={({ pressed }) => [styles.itemCard, pressed && styles.pressed]}>
                <View style={styles.itemArtworkWrap}><Image source={policyArtwork(policy)} style={styles.itemArtwork} resizeMode="contain" /></View>
                <View style={styles.itemBody}><View style={styles.itemHeading}><Text style={styles.itemTitle}>{policy.policy_no || policy.policy_code || 'Policy'}</Text><PartnerStatusBadge label={policyCategory(policy)} tone="brand" /></View><Text style={styles.itemText}>{[policy.insurer_name, policy.vehicle_no].filter(Boolean).join(' · ') || 'Policy details'}</Text><Text style={styles.itemMeta}>Ends {formatDate(policy.end_date)} · {formatIndianCurrency(policy.premium_amount)}</Text></View>
          ...
          {data.vehicles.length ? (
            <View style={styles.stack}>{visibleVehicles.map((vehicle) => (
              <View key={vehicle.vehicle_id} style={styles.vehicleCard}>
                <View style={styles.itemArtworkWrap}><Image source={PartnerAssets.products.motorInsurance} style={styles.itemArtwork} resizeMode="contain" /></View>
                <View style={styles.itemBody}><Text style={styles.itemTitle}>{vehicle.vehicle_no || 'Vehicle'}</Text><Text style={styles.itemText}>{displayParts(vehicle.make, vehicle.model, vehicle.year) || humanize(vehicle.vehicle_type || 'vehicle')}</Text><View style={styles.expiryRow}><Expiry label="PUC" date={vehicle.puc_expiry_date} /><Expiry label="Fitness" date={vehicle.fitness_expiry_date} /><Expiry label="Road tax" date={vehicle.road_tax_expiry_date} /><Expiry label="National permit" date={vehicle.national_permit_expiry_date} /><Expiry label="Local permit" date={vehicle.local_permit_expiry_date} /></View></View>
              </View>
            ))}</View>
          ...
          {data.claims.length ? (
            <View style={styles.stack}>{visibleClaims.map((claim) => (
              <Pressable accessibilityRole="button" accessibilityLabel={`Open claim ${claim.claim_no || ''}`} key={claim.claim_id} onPress={() => router.push(`/claim/${claim.claim_id}` as never)} style={({ pressed }) => [styles.itemCard, pressed && styles.pressed]}>
                <View style={styles.itemArtworkWrap}><Image source={claimArtwork(claim.current_status)} style={styles.itemArtwork} resizeMode="contain" /></View>
                <View style={styles.itemBody}><View style={styles.itemHeading}><Text style={styles.itemTitle}>{claim.claim_no || 'Claim'}</Text><PartnerStatusBadge label={humanize(claim.current_status || 'active')} tone={claimTone(claim.current_status)} /></View><Text style={styles.itemText}>{[claim.vehicle_no, claim.insurer_name].filter(Boolean).join(' · ') || 'Claim details'}</Text></View>
                <Ionicons name="chevron-forward" size={16} color="#9AA3B2" />
              </Pressable>
            ))}</View>
...
function claimArtwork(value: string | null): ImageSourcePropType { const normalized = (value || '').toLowerCase(); if (normalized.includes('complete') || normalized.includes('settled') || normalized.includes('closed')) return PartnerAssets.status.verified; if (normalized.includes('pending') || normalized.includes('attention')) return PartnerAssets.status.claimAttention; return PartnerAssets.navigation.claims; }
function policyArtwork(policy: PartnerCustomerDetail['policies'][number]): ImageSourcePropType { const category = policyCategory(policy); if (category === 'Motor') return PartnerAssets.products.motorInsurance; if (category === 'Health') return PartnerAssets.products.healthInsurance; if (category === 'Life') return PartnerAssets.products.familyInsurance; return PartnerAssets.products.commercialInsurance; }
...
const styles = StyleSheet.create({
  hero: { paddingVertical: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: partnerTheme.colors.line }, heroTop: { flexDirection: 'row', alignItems: 'center', gap: 12 }, avatar: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: partnerTheme.colors.brandSoft }, avatarText: { color: partnerTheme.colors.brandStrong, ...partnerTheme.typography.bodyStrong }, heroBody: { flex: 1 }, heroName: { color: partnerTheme.colors.ink, ...partnerTheme.typography.sectionTitle }, heroMeta: { marginTop: 3, color: partnerTheme.colors.inkMuted, ...partnerTheme.typography.caption }, heroSince: { marginTop: 7, color: partnerTheme.colors.inkSubtle, ...partnerTheme.typography.meta }, contactActions: { marginTop: 8 }, attention: { marginTop: 8 }, summary: { marginTop: 8, flexDirection: 'row', backgroundColor: partnerTheme.colors.surface, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: partnerTheme.colors.line }, summaryItem: { flex: 1, minHeight: 62, alignItems: 'center', justifyContent: 'center' }, summaryValue: { color: partnerTheme.colors.ink, fontSize: 18, lineHeight: 23, fontWeight: '800' }, summaryLabel: { marginTop: 3, color: partnerTheme.colors.inkMuted, ...partnerTheme.typography.meta }, relationshipCard: { flexDirection: 'row', flexWrap: 'wrap', rowGap: 9, paddingVertical: 8, backgroundColor: partnerTheme.colors.surface }, info: { width: '50%', paddingRight: 8 }, infoLabel: { color: '#8A94A6', textTransform: 'uppercase', letterSpacing: 0.5, ...partnerTheme.typography.meta }, infoValue: { marginTop: 3, color: partnerTheme.colors.ink, ...partnerTheme.typography.caption }, stack: { gap: 0 }, itemCard: { minHeight: 64, flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, paddingHorizontal: 2, backgroundColor: partnerTheme.colors.surface, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: partnerTheme.colors.line }, vehicleCard: { minHeight: 64, flexDirection: 'row', alignItems: 'flex-start', gap: 10, paddingVertical: 10, paddingHorizontal: 2, backgroundColor: partnerTheme.colors.surface, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: partnerTheme.colors.line }, itemArtworkWrap: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' }, itemArtwork: { width: 38, height: 38 }, itemBody: { flex: 1 }, itemHeading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 7 }, itemTitle: { flex: 1, color: partnerTheme.colors.ink, ...partnerTheme.typography.bodyStrong }, itemText: { marginTop: 3, color: partnerTheme.colors.inkMuted, ...partnerTheme.typography.caption }, itemMeta: { marginTop: 3, color: partnerTheme.colors.brandStrong, ...partnerTheme.typography.meta }, expiryRow: { marginTop: 5, flexDirection: 'row', flexWrap: 'wrap', gap: 5 }, expiry: { borderRadius: partnerTheme.radius.pill, paddingHorizontal: 7, paddingVertical: 4 }, expiryGood: { backgroundColor: partnerTheme.colors.successSoft }, expiryWarn: { backgroundColor: partnerTheme.colors.warningSoft }, expiryBad: { backgroundColor: partnerTheme.colors.dangerSoft }, expiryText: { color: partnerTheme.colors.inkMuted, ...partnerTheme.typography.meta }, expandToggle: { minHeight: partnerTheme.control.minTouchTarget, marginTop: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5 }, expandToggleText: { color: partnerTheme.colors.brandStrong, ...partnerTheme.typography.caption }, emptyLine: { minHeight: 54, alignItems: 'center', justifyContent: 'center', borderRadius: partnerTheme.radius.lg, backgroundColor: partnerTheme.colors.surface }, emptyLineText: { color: partnerTheme.colors.inkMuted, ...partnerTheme.typography.caption }, pressed: { backgroundColor: partnerTheme.colors.pressed },
});
