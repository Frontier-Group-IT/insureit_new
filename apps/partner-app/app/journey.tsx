...
export default function JourneyScreen() {
  ...
  return (
    <PartnerScreen eyebrow="MY JOURNEY" title="My progress" onBack={() => router.back()}>
      {loading ? (
        <PartnerStateView state="loading" title="Loading your journey" />
      ) : error || !data ? (
        <PartnerStateView state="error" title="Your journey is temporarily unavailable" message={error || 'Your journey could not be loaded.'} actionLabel="Try again" onAction={() => void load()} />
      ) : (
        <>
          <View style={styles.hero}>
            ...
            <Text style={styles.heroText}>
              {data.next_milestone.remaining > 0
                ? `${data.next_milestone.remaining} more customer${data.next_milestone.remaining === 1 ? '' : 's'} to reach the next milestone.`
                : 'Milestone reached.'}
            </Text>
            <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${progress * 100}%` }]} /></View>
            <View style={styles.progressMeta}><Text style={styles.progressText}>{data.next_milestone.current}</Text><Text style={styles.progressText}>{data.next_milestone.target}</Text></View>
          </View>
          ...
          {data.milestones.length ? (
            ...
            <PartnerStateView state="empty" title="Your journey is just beginning" message="Business and service milestones will appear as real events are recorded." asset={PartnerAssets.status.journey} />
          )}
        </>
      )}
    </PartnerScreen>
...
const styles = StyleSheet.create({
  ...
  heroTitle: { marginTop: 4, color: '#FFFFFF', fontSize: 22, lineHeight: 28, fontWeight: '800' },
  heroText: { marginTop: 4, color: '#C9D0DE', ...partnerTheme.typography.caption },
  ...
  timelineTitle: { marginTop: 3, color: partnerTheme.colors.ink, ...partnerTheme.typography.bodyStrong },
  timelineText: { marginTop: 3, color: partnerTheme.colors.inkMuted, ...partnerTheme.typography.caption },
});
