  ...
  return (
    <PartnerScreen eyebrow="RECOGNITION" title="Progress worth noticing" onBack={() => router.back()}>
      {loading ? (
        ...
        <PartnerStateView
          state="error"
          title="Recognition is temporarily unavailable"
          message={error || 'Recognition could not be loaded.'}
        ...
        <>
          ...
          <View style={styles.list}>
            {data.items.length ? data.items.map((item)=><View key={item.code} style={styles.card}>
              ...
              <PartnerStateView
                state="empty"
                title="More highlights will appear here"
                message="Recognition appears when a recorded milestone is reached."
                asset={PartnerAssets.status.achievement}
              />
            )}
          </View>
...
const styles=StyleSheet.create({
  hero:{minHeight:76,flexDirection:'row',alignItems:'center',gap:12,borderRadius:partnerTheme.radius.xl,paddingHorizontal:15,paddingVertical:12,backgroundColor:partnerTheme.colors.nav},heroArtwork:{width:40,height:40},heroTitle:{flex:1,color:'#FFFFFF',fontSize:18,fontWeight:'800'},
