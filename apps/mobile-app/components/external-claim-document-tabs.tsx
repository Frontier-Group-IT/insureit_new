    ...
    <View style={styles.card}>
      ...
      <View style={styles.grid}>
        {activeGroup.group.documents.map((document) => {
          const latestMatchingDocument = documents.find((item) => matchesClaimIntimationDocument(item.document_type, document.type));
          const replacementRequested = isManaged && latestMatchingDocument?.verification_status === 'rejected';
          const uploaded = replacementRequested ? undefined : latestMatchingDocument;
          ...
          return <View key={document.type} style={[styles.documentCard, uploaded && styles.documentCardUploaded, replacementRequested && styles.documentCardReuploadRequested, !canModify && styles.documentCardReadOnly]}>
            ...
            <Text style={[styles.documentBody, replacementRequested && styles.documentBodyReuploadRequested]} numberOfLines={2}>{replacementRequested ? 'REQUEST UPLOAD' : uploaded?.verification_status === 'verified' ? 'Verified by claims desk' : uploaded ? 'Uploaded' : document.body}</Text>
            <Pressable accessibilityRole="button" disabled={Boolean(uploadingType) || !canModify} onPress={() => void pickAndUpload(document)} style={[styles.uploadButton, uploaded && styles.replaceButton, replacementRequested && styles.reuploadButton, (!canModify || (Boolean(uploadingType) && !uploading)) && styles.uploadButtonDisabled]}>
              <MaterialCommunityIcons name={uploaded?.verification_status === 'verified' ? 'check-decagram' : uploaded ? 'refresh' : 'upload-outline'} size={15} color="#FFFFFF" />
              <Text style={styles.uploadButtonText}>{uploading ? 'Uploading…' : uploaded?.verification_status === 'verified' ? 'Verified' : uploaded ? 'Replace' : 'Upload'}</Text>
            </Pressable>
          </View>;
        })}
      </View>
