...
export function GroupAddAccountScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const [nextProfile, context, manufacturerResult] = await Promise.all([
  ...
  function setValue(name: string, value: string) {
    ...
    if (errorField === name && value.trim()) setErrorField('');
  }
  ...
  async function chooseFile(name: string) {
    ...
    if (asset.size && asset.size > maxFileSize) {
      setErrorField(name);
      return setMessage('Each file must be 5 MB or smaller.');
    }
  ...
  function validateCurrent() {
    if (!current) return { message: '', field: '' };
    for (const field of current.fields) {
      if (field.name === 'city' && field.required && !values.india_location_id) return { message: 'Enter at least 2 letters and select the city from the matching locations.', field: 'city' };
      if (field.kind === 'file' && field.required && !files[field.name]) return { message: `Attach ${field.label}.`, field: field.name };
      if (field.kind !== 'file' && field.required && !values[field.name]?.trim()) return { message: `Enter ${field.label}.`, field: field.name };
    }
  ...
  async function next() {
    ...
    if (validation.message) {
      ...
      return setMessage(validation.message);
    }
  ...
  async function submit() {
    ...
    if (!values.india_location_id) {
      setErrorField('city');
      return setMessage('Select the city from the matching locations before submitting.');
    }
    setSubmitting(true);
    try {
      const session = await getCurrentSession();
  ...
  return <GroupPageShell title="Add Associated Customer" subtitle={type ? `${partnerTypeLabel(type)} onboarding` : 'Choose the partner type to begin'} icon="account-plus-outline">
    {!type ? <PartnerTypePicker parentType={groupContext?.partner_type ?? 'group'} onSelect={selectPartnerType} /> : <>
      ...
      {current ? <View style={styles.section}><Text style={styles.sectionTitle}>{current.title}</Text><Text style={styles.sectionText}>{current.subtitle}</Text>
        {current.fields.length === 0 ? <ReviewPanel type={type} dealershipType={dealershipType} groupName={parentTitle} values={values} files={files} gstRegistered={gstRegistered} /> : current.title === 'Login Contacts' ? <CorporateLoginContactGroups values={values} errorField={errorField} onChange={(name, value) => setValue(name, normalize(name, value))} /> : current.fields.map((field) => {
          ...
          if (field.kind === 'file') return <DocumentField key={field.name} label={field.label} file={files[field.name]} required={field.required} error={errorField === field.name} onPress={() => void chooseFile(field.name)} />;
          if (field.name === 'city') return <LocationSearchField key={field.name} query={locationQuery} selected={selectedLocation} options={locationOptions} searching={locationSearching} searched={locationSearched} error={errorField === 'city'} onChange={changeLocationQuery} onSelect={chooseLocation} />;
          if (field.name === 'oem_name') return <OemSelector key={field.name} manufacturers={manufacturers} selectedOem={values.oem_name || ''} query={oemQuery || values.oem_name || ''} open={oemOpen} onOpenChange={setOemOpen} onQueryChange={(value) => { setOemQuery(value); setValue('oem_name', value); setOemOpen(true); }} onSelect={(value) => { setValue('oem_name', value); setOemQuery(value); setOemOpen(false); }} />;
          return <Field key={field.name} label={field.label} required={field.required} value={values[field.name] || ''} error={errorField === field.name} onChangeText={(value) => setValue(field.name, normalize(field.name, value))} keyboardType={field.keyboardType} editable={!field.locked} />;
        })}
      </View> : null}
...
function stepDefinitions(type: AccountType, dealershipType: 'posp'|'misp', gst: boolean): StepDef[] {
  if (type === 'corporate') return [
    ...
    { title:'Company Address & Fleet', subtitle:'Search and select a verified city; State and PIN Code will be filled automatically.', fields:[f('address_street','Street',true),f('address_locality','Locality'),f('city','City',true),choice('fleet_size_band','Fleet Size',fleetOptions,true)] },
  ...
  if (type === 'individual_proprietor') return [
    ...
    { title:'Address Details', subtitle:'Search and select a verified city; State and PIN Code will be filled automatically.', fields:[f('address_street','Street',true),f('address_locality','Locality'),f('city','City',true)] },
  ...
  return [
    ...
    { title:'Address & Business Profile', subtitle:'Search and select a verified city; State and PIN Code will be filled automatically.', fields:[f('address_street','Street',true),f('address_locality','Locality'),f('city','City',true),f('oem_name','Dealership OEM',true),choice('yearly_sales_band','Yearly Sales',salesOptions,true)] },
...
function normalize(name:string,value:string){
  ...
  if (name === 'company_pan' || name === 'pan_number' || name === 'representative_pan') return normalizePan(value);
...
function displayValue(key:string,value:string){if(!value)return 'Not provided';const options=[...fleetOptions,...salesOptions,['posp','POSP'],['misp','MISP']] as const;return options.find(([option])=>option===value)?.[1]||value;}
...
function Field({ label, required, error, ...props }: React.ComponentProps<typeof TextInput> & { label:string; required?:boolean; error?: boolean }) { return <View style={styles.field}><Text style={styles.fieldLabel}>{label}{required?<Text style={styles.required}> *</Text>:null}</Text><TextInput placeholderTextColor="#9AA7B8" style={[styles.input,props.editable===false&&styles.inputLocked,error&&styles.inputError]} {...props} /></View>; }
function CorporateLoginContactGroups({ values, errorField, onChange }: { values: Values; errorField: string; onChange: (name: string, value: string) => void }) { const groups=[['corporate_creator','Corporate Creator'],['ceo_head','CEO / Head'],['admin_head','Admin Head'],['dedicated_spoc','Dedicated SPOC']] as const;return <View style={styles.contactGroups}>{groups.map(([prefix,title])=><View key={prefix} style={styles.contactGroup}><View style={styles.contactGroupHeader}><View style={styles.contactGroupIcon}><MaterialCommunityIcons name="account-key-outline" size={17} color="#0A43A3"/></View><Text style={styles.contactGroupTitle}>{title}</Text></View><Field label="Name" required value={values[`${prefix}_name`]||''} error={errorField===`${prefix}_name`} onChangeText={(value)=>onChange(`${prefix}_name`,value)}/><Field label="Mobile" required value={values[`${prefix}_mobile`]||''} error={errorField===`${prefix}_mobile`} onChangeText={(value)=>onChange(`${prefix}_mobile`,value)} keyboardType="phone-pad"/><Field label="Email" value={values[`${prefix}_email`]||''} error={errorField===`${prefix}_email`} onChangeText={(value)=>onChange(`${prefix}_email`,value)} keyboardType="email-address" autoCapitalize="none"/></View>)}</View>;}
function OemSelector({ manufacturers, selectedOem, query, open, onOpenChange, onQueryChange, onSelect }: { manufacturers: string[]; selectedOem: string; query: string; open: boolean; onOpenChange: (open: boolean) => void; onQueryChange: (value: string) => void; onSelect: (value: string) => void }) { const visible=manufacturers.filter((manufacturer)=>!query.trim()||manufacturer.toLowerCase().includes(query.trim().toLowerCase())).slice(0,12);return <View style={styles.field}><Text style={styles.fieldLabel}>Dealership OEM <Text style={styles.required}>*</Text></Text><View style={styles.oemBox}><Pressable onPress={()=>onOpenChange(!open)} style={styles.oemHeader}><View style={styles.flex}><Text style={styles.oemValue} numberOfLines={1}>{selectedOem||'Select OEM'}</Text><Text style={styles.oemHint}>Search approved vehicle manufacturers</Text></View><MaterialCommunityIcons name={open?'chevron-up':'chevron-down'} size={20} color={palette.navy}/></Pressable>{open?<><View style={styles.oemSearch}><MaterialCommunityIcons name="magnify" size={18} color="#7A8799"/><TextInput value={query} onChangeText={onQueryChange} placeholder="Search OEM" placeholderTextColor="#8A94A6" style={styles.oemSearchInput}/></View>{manufacturers.length?<View style={styles.oemGrid}>{visible.map((manufacturer)=><Pressable key={manufacturer} onPress={()=>onSelect(manufacturer)} style={[styles.oemChip,selectedOem===manufacturer&&styles.oemChipActive]}><Text style={[styles.oemChipText,selectedOem===manufacturer&&styles.oemChipTextActive]} numberOfLines={1}>{manufacturer}</Text></Pressable>)}</View>:<TextInput value={selectedOem} onChangeText={onSelect} placeholder="Enter OEM" placeholderTextColor="#9AA7B8" style={styles.input}/>}</>:null}</View></View>;}
function ReviewPanel({ type, dealershipType, groupName, values, files, gstRegistered }: { type: AccountType; dealershipType: 'posp'|'misp'; groupName: string; values: Values; files: FileMap; gstRegistered: boolean }) { const detailKeys=type==='corporate'?['company_name','company_pan','gst_number','address_street','address_locality','city','state','postal_code','fleet_size_band','corporate_creator_name','corporate_creator_mobile','ceo_head_name','ceo_head_mobile','admin_head_name','admin_head_mobile','dedicated_spoc_name','dedicated_spoc_mobile']:type==='individual_proprietor'?['contact_name','phone','email','address_street','address_locality','city','state','postal_code','pan_number','aadhaar_number','legal_trade_name','gst_number','fleet_size_band']:['dealership_type','dealership_name','owner_name','phone','email','address_street','address_locality','city','state','postal_code','oem_name','yearly_sales_band','gst_number','representative_name','representative_mobile','representative_email','representative_pan'];const docs=Object.entries(files);return <View style={styles.reviewWrap}><View style={styles.reviewHeader}><View style={styles.reviewIcon}><MaterialCommunityIcons name="clipboard-check-outline" size={24} color="#0A43A3"/></View><View style={styles.flex}><Text style={styles.reviewTitle}>{partnerTypeLabel(type)} Application</Text><Text style={styles.reviewMeta}>Associated with {groupName}</Text></View><View style={styles.reviewBadge}><Text style={styles.reviewBadgeText}>{type==='dealership'?dealershipType.toUpperCase():'KYC'}</Text></View></View><View style={styles.reviewSection}><Text style={styles.reviewSectionTitle}>Application details</Text>{detailKeys.filter((key)=>values[key]).map((key)=><View key={key} style={styles.reviewRow}><Text style={styles.reviewLabel}>{fieldLabels[key]||key.replace(/_/g,' ')}</Text><Text style={styles.reviewValue} numberOfLines={2}>{displayValue(key,values[key])}</Text></View>)}<View style={styles.reviewRow}><Text style={styles.reviewLabel}>GST Registered</Text><Text style={styles.reviewValue}>{gstRegistered?'Yes':'No'}</Text></View></View><View style={styles.reviewSection}><Text style={styles.reviewSectionTitle}>Documents</Text>{docs.length?docs.map(([key,file])=><View key={key} style={styles.reviewDoc}><MaterialCommunityIcons name="file-check-outline" size={18} color="#12805C"/><View style={styles.flex}><Text style={styles.reviewDocTitle}>{fieldLabels[key]||key.replace(/_/g,' ')}</Text><Text style={styles.reviewDocName} numberOfLines={1}>{file?.name}</Text></View></View>):<Text style={styles.reviewEmpty}>No new documents selected on this device.</Text>}</View><Text style={styles.reviewNote}>Please confirm the information carefully. After submission, the application will move to verification.</Text></View>;}
function LocationSearchField({query,selected,options,searching,searched,error,onChange,onSelect}:{query:string;selected:IndiaLocation|null;options:IndiaLocation[];searching:boolean;searched:boolean;error?:boolean;onChange:(value:string)=>void;onSelect:(location:IndiaLocation)=>void}){return <View style={styles.field}><Text style={styles.fieldLabel}>City <Text style={styles.required}>*</Text></Text><View style={[styles.locationInputShell,selected&&styles.locationInputSelected,error&&styles.inputError]}><TextInput value={query} onChangeText={onChange} placeholder="Enter at least 2 letters" placeholderTextColor="#9AA7B8" autoCapitalize="words" style={styles.locationInput}/>{searching?<ActivityIndicator size="small" color="#0A43A3"/>:<MaterialCommunityIcons name={selected?'check-circle':'magnify'} size={19} color={selected?'#12805C':'#607089'}/>}</View>{query.trim().length===1?<Text style={styles.locationHint}>Enter one more letter to search.</Text>:null}{options.length?<View style={styles.locationList}>{options.map((option)=><Pressable key={option.id} onPress={()=>onSelect(option)} style={styles.locationOption}><View style={styles.flex}><Text style={styles.locationCity}>{option.city_name}</Text><Text style={styles.locationMeta}>{[option.district,option.state_name,option.pincode].filter(Boolean).join(' · ')}</Text></View><MaterialCommunityIcons name="chevron-right" size={19} color="#728197"/></Pressable>)}</View>:null}{searched&&!searching&&!selected&&query.trim().length>=2&&!options.length?<Text style={styles.locationEmpty}>No matching city found. Check the spelling and try again.</Text>:null}{selected?<View style={styles.selectedLocation}><MaterialCommunityIcons name="map-marker-check-outline" size={18} color="#12805C"/><View style={styles.flex}><Text style={styles.selectedLocationTitle}>{selected.city_name}, {selected.state_name}</Text><Text style={styles.selectedLocationMeta}>PIN {selected.pincode}{selected.district?` · ${selected.district}`:''}</Text></View></View>:null}</View>;}
