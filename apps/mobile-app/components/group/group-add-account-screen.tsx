...
import type { Json, PartnerType, Profile } from '@/lib/types';
  ...
  return <GroupPageShell title="Add Associated Customer" subtitle={type ? `${partnerTypeLabel(type)} onboarding` : 'Choose the partner type to begin'} icon="account-plus-outline">
    {!type ? <PartnerTypePicker onSelect={(next) => { setType(next); setStep(0); setMessage(''); }} /> : <>
      ...
      {current ? <View style={styles.section}><Text style={styles.sectionTitle}>{current.title}</Text><Text style={styles.sectionText}>{current.subtitle}</Text>
        {current.fields.map((field) => field.kind === 'switch' ? <View key={field.name} style={styles.switchRow}><View style={styles.flex}><Text style={styles.fieldLabel}>{field.label}</Text><Text style={styles.switchHint}>Enable when applicable.</Text></View><Switch value={gstRegistered} onValueChange={setGstRegistered} /></View>
          : field.kind === 'choice' ? <ChoiceField key={field.name} label={field.label} value={values[field.name] || ''} options={field.options || []} onChange={(value) => setValue(field.name, value)} />
          : field.kind === 'file' ? <DocumentField key={field.name} label={field.label} file={files[field.name]} required={field.required} onPress={() => void chooseFile(field.name)} />
          : <Field key={field.name} label={field.label} required={field.required} value={values[field.name] || ''} onChangeText={(value) => setValue(field.name, normalize(field.name, value))} keyboardType={field.keyboardType} editable={!field.locked} />)}
      </View> : null}
      <View style={styles.footerActions}><Pressable disabled={submitting} onPress={() => step > 0 ? setStep((value) => value - 1) : setType(null)} style={styles.secondaryButton}><Text style={styles.secondaryButtonText}>Back</Text></Pressable><Pressable disabled={submitting} onPress={() => void next()} style={[styles.primaryButton, submitting && styles.disabled]}>{submitting ? <ActivityIndicator color="#FFFFFF" /> : <><Text style={styles.primaryButtonText}>{step === steps.length - 1 ? 'Submit for Review' : 'Continue'}</Text><MaterialCommunityIcons name="arrow-right" size={17} color="#FFFFFF" /></>}</Pressable></View>
    </>}
...
function stepDefinitions(type: AccountType, dealershipType: 'posp'|'misp', gst: boolean): StepDef[] {
  if (type === 'corporate') return [
    ...
    { title:'Company Address & Fleet', subtitle:'Enter the registered address and fleet profile.', fields:[f('address_street','Street',true),f('address_locality','Locality'),f('city','City',true),f('state','State',true),f('postal_code','PIN Code',true,'number-pad'),choice('fleet_size_band','Fleet Size',fleetOptions,true)] },
  ...
  if (type === 'individual_proprietor') return [
    ...
    { title:'Address Details', subtitle:'Enter the complete customer address.', fields:[f('address_street','Street',true),f('address_locality','Locality'),f('city','City',true),f('state','State',true),f('postal_code','PIN Code',true,'number-pad')] },
  ...
  return [
    ...
    { title:'Address & Business Profile', subtitle:'Match the website Dealership address and business profile.', fields:[f('address_street','Street',true),f('address_locality','Locality'),f('city','City',true),f('state','State',true),f('postal_code','PIN Code',true,'number-pad'),f('oem_name','Dealership OEM',true),choice('yearly_sales_band','Yearly Sales',salesOptions,true)] },
