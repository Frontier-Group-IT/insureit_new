...
import { customerAccountTitle, getAccessibleCustomerContexts, getGroupChildAccountOverview, getSelectedCustomerContext, membershipRoleLabel, type CustomerAccountContext } from '@/lib/customer-context';
...
export function GroupProfileScreen() {
  ...
  const [associatedBreakdown, setAssociatedBreakdown] = useState({ corporate: 0, individual: 0, dealership: 0, inReview: 0 });
  ...
  useEffect(() => { let active = true; setLoading(true); void (async () => {
    try {
      ...
      if (!active) return;
      const associated = contexts.filter((item) => item.access_source === 'group_child' && item.group_customer_id === group?.customer_id);
      const overview = group ? await getGroupChildAccountOverview(group.customer_id) : [];
      const accountIds = Array.from(new Set([group?.customer_id, ...overview.map((item) => item.customer_id), ...associated.map((item) => item.customer_id)].filter((id): id is string => Boolean(id))));
      ...
      setProfile(nextProfile); setContext(group); setCustomer(customerResult.data); setAssociatedCount(visibleAssociated.length);
  ...
  return <GroupPageShell title="Group Profile" subtitle={groupName} icon="account-circle-outline" loading={loading}>
    {loading ? <LoadingState /> : <>
      ...
      <View style={styles.associatedCard}>
        <View style={styles.associatedTop}><View style={styles.associatedIcon}><MaterialCommunityIcons name="account-group-outline" size={24} color="#0A43A3" /></View><View style={styles.associatedCopy}><Text style={styles.associatedTitle}>{associatedCount} associated customer{associatedCount === 1 ? '' : 's'}</Text><Text style={styles.associatedText}>Manage linked corporate, individual and dealership accounts from your profile.</Text></View></View>
        <View style={styles.associatedStats}><MiniStat label="Corporate" value={associatedBreakdown.corporate} /><MiniStat label="Individual" value={associatedBreakdown.individual} /><MiniStat label="Dealership" value={associatedBreakdown.dealership} /><MiniStat label="In review" value={associatedBreakdown.inReview} /></View>
      ...
      <View style={styles.activityCard}>
        <ActivityRow icon="account-check-outline" title={`${associatedCount} associated account${associatedCount === 1 ? '' : 's'}`} body="Group access and portfolio links are available here." />
