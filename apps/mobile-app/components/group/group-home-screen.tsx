...
import { customerAccountTitle, getAccessibleCustomerContexts, membershipRoleLabel, partnerTypeLabel, type CustomerAccountContext } from '@/lib/customer-context';
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.attentionRow}>
        ...
        <AttentionCard icon="account-clock-outline" value={underReview ? 1 : 0} label="Accounts in review" tone="#0A43A3" soft="#EEF5FF" onPress={() => router.push('/customer/group/accounts')} disabled={underReview} />
      </View>

      <AnimatedCard disabled={underReview} onPress={() => router.push('/customer/group/accounts')}>
        <View style={styles.accountsHeader}>
          <View><Text style={styles.cardTitle}>Associated Accounts</Text><Text style={styles.cardSubtitle}>{underReview ? 'Available after verification' : `${data.associated.length} active businesses`}</Text></View>
        ...
        {recentAccounts.length ? <View style={styles.accountPreviewList}>{recentAccounts.map((account) => <Pressable key={account.customer_id} onPress={() => router.push({ pathname: '/customer/group/account-detail', params: { customerId: account.customer_id } })} style={styles.accountPreview}><View style={styles.accountAvatar}><Text style={styles.accountAvatarText}>{initialFor(customerAccountTitle(account))}</Text></View><View style={styles.accountPreviewCopy}><Text style={styles.accountName} numberOfLines={1}>{customerAccountTitle(account)}</Text><Text style={styles.accountMeta}>{partnerTypeLabel(account.partner_type)}</Text></View><MaterialCommunityIcons name="chevron-right" size={20} color="#667085" /></Pressable>)}</View> : <View style={styles.accountEmpty}><MaterialCommunityIcons name="account-multiple-plus-outline" size={24} color="#0A43A3" /><Text style={styles.accountEmptyText}>{underReview ? 'Associated accounts will appear after approval.' : 'Add your first associated business to build the Group portfolio.'}</Text></View>}
      </AnimatedCard>
