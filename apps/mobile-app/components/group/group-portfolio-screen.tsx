...
import { customerAccountTitle, getAccessibleCustomerContexts, getGroupChildAccountOverview, getSelectedCustomerContext, groupChildAccountTitle, partnerTypeLabel, selectCustomerContext, type CustomerAccountContext, type GroupChildAccountOverview } from '@/lib/customer-context';
...
export function GroupAccountsScreen() {
  ...
  const filtered = useMemo(() => rows.filter((row) => {
    ...
    const haystack = `${groupChildAccountTitle(row)} ${row.customer_code ?? ''} ${row.city ?? ''} ${row.state ?? ''} ${row.contact_name ?? ''} ${row.onboarding_status}`.toLowerCase();
  ...
  return <GroupPageShell title="Associated Customers" subtitle={`${rows.length} account${rows.length === 1 ? '' : 's'} in the Group portfolio`} icon="account-multiple-outline" rightAction={addAction}>
    {loading ? <LoadingState /> : <>
      ...
      {!filtered.length ? <EmptyState title="No associated customers" body="Accounts associated with this Group will appear here with their onboarding status." /> : filtered.map((row) => {
        const canOpen = row.account_source === 'linked_customer' && Boolean(row.customer_id);
        return <Pressable key={row.row_id} disabled={!canOpen} onPress={() => row.customer_id && router.push({ pathname: '/customer/group/account-detail', params: { id: row.customer_id } })} style={[styles.customerCard, !canOpen && styles.customerCardPending]}>
          ...
          <View style={styles.cardFooter}><Text style={styles.contactText}>Contact: {row.contact_name || 'Pending contact'}</Text>{canOpen ? <MaterialCommunityIcons name="chevron-right" size={20} color="#0A43A3" /> : <Text style={styles.pendingText}>Waiting for verification</Text>}</View>
        </Pressable>;
      })}
    </>}
  </GroupPageShell>;
...
export function GroupAccountDetailScreen() {
  ...
  useEffect(() => { let active = true; void (async () => {
    try {
      ...
      if (!next) return;
      const [customerResult, vehicleResult, policyResult, claimResult] = await Promise.all([supabase.from('customers').select('*').eq('id', id).maybeSingle(), supabase.from('vehicles').select('*').eq('customer_id', id), supabase.from('policies').select('*').eq('customer_id', id), supabase.from('claims').select('*').eq('customer_id', id)]);
      if (!active) return;
