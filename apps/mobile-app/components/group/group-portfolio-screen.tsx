...
export function GroupAccountDetailScreen() {
  ...
  function openAsSelectedAccount() {
    if (!currentDetail.customer_id) return;
    void runWithLoader(async () => { await selectCustomerContext(currentDetail.customer_id as string); router.replace('/customer/home'); }, 'Opening associated account');
  }

  return <GroupPageShell title={currentDetail.account_title} subtitle="Associated with Group Account" icon="office-building-outline">
