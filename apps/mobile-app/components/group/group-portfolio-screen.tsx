...
export function GroupAccountsScreen() {
  ...
  useEffect(() => { let active = true; setLoading(true); void (async () => {
    try {
      ...
      setRows(contexts.map((context) => ({ ...context, vehicles: (vehicleResult.data ?? []).filter((item) => item.customer_id === context.customer_id).length, policies: (policyResult.data ?? []).filter((item) => item.customer_id === context.customer_id && new Date(item.end_date).getTime() >= Date.now()).length, claims: (claimResult.data ?? []).filter((item) => item.customer_id === context.customer_id && !['Closed', 'Settled', 'Rejected', 'Claim Complete'].includes(item.current_status)).length, city: customers.get(context.customer_id)?.city ?? null, status: customers.get(context.customer_id)?.onboarding_status ?? 'active' })));
    } finally { if (active) setLoading(false); }
  })(); return () => { active = false; }; }, []);

  const filtered = useMemo(() => rows.filter((row) => { if (filter !== 'all' && row.partner_type !== filter) return false; const haystack = `${customerAccountTitle(row)} ${row.customer_code} ${row.city ?? ''} ${row.contact_name}`.toLowerCase(); return !query.trim() || haystack.includes(query.trim().toLowerCase()); }), [filter, query, rows]);
