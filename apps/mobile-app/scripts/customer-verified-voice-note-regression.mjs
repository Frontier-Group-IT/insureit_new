...
const selfManagedPath = path.join(root, 'apps/mobile-app/app/customer/self-managed-claim.tsx');

const component = fs.readFileSync(componentPath, 'utf8');
const stage = fs.readFileSync(stagePath, 'utf8');
const selfManaged = fs.readFileSync(selfManagedPath, 'utf8');
...
const checks = [
  ...
  ['Verified voice note uses the green locked card treatment', component.includes('locked = saved && !onRemoveSaved') && component.includes('locked && styles.cardLocked') && component.includes("cardLocked: { borderColor: '#52B57F', backgroundColor: '#EFFAF4' }")],
  ['Persisted self-managed voice notes remain removable rather than being mistaken for verified locks', selfManaged.includes('saved={savedDocuments.some((item) => item.document_type === VOICE_NOTE_DOCUMENT_TYPE)}') && selfManaged.includes('onRemoveSaved={async () => {') && component.includes('locked = saved && !onRemoveSaved')],
