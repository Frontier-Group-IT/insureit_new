    ...
    <View style={styles.profileRoot}>
      ...
      {customer ? <View style={styles.kycVaultCard}>
        ...
        <View style={styles.kycVaultSummary}>
          <Pressable accessibilityRole="button" accessibilityLabel={documentsOpen ? "Close uploaded documents" : "Open uploaded documents"} accessibilityState={{ expanded: documentsOpen }} onPress={() => setDocumentsOpen((current) => !current)} style={styles.kycMiniStat}><Text style={styles.kycMiniStatValue}>{documents.length}</Text><Text style={styles.kycMiniStatLabel}>Uploaded</Text></Pressable>
          <Pressable accessibilityRole="button" accessibilityLabel={documentsOpen ? "Close private document vault" : "Open private document vault"} accessibilityState={{ expanded: documentsOpen }} onPress={() => setDocumentsOpen((current) => !current)} style={styles.kycMiniStat}><MaterialCommunityIcons name="lock-check-outline" size={17} color="#12805C" /><Text style={styles.kycMiniStatLabel}>Private vault</Text></Pressable>
          <Pressable accessibilityRole="button" accessibilityLabel={documentsOpen ? "Close document manager" : "Manage documents"} accessibilityState={{ expanded: documentsOpen }} onPress={() => setDocumentsOpen((current) => !current)} style={styles.kycMiniStat}><MaterialCommunityIcons name="trash-can-outline" size={17} color="#B7791F" /><Text style={styles.kycMiniStatLabel}>Manage documents</Text></Pressable>
        </View>
