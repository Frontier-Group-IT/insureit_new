...
const accountPrivacyIcon = require('../../assets/custom-icons/profile/account-privacy.png');
    ...
    <View style={styles.profileRoot}>
      ...
      <View style={styles.hero}>
        ...
        <Animated.View style={[styles.avatarStage, { transform: [{ translateY: avatarLift }, { scale: avatarScale }] }]}>
          <View style={[styles.avatarShell, !avatarUri && styles.avatarFallback]}>
            {avatarUri ? (
              <Image source={{ uri: avatarUri }} style={styles.avatarImage} resizeMode="cover" />
            ) : (
              <Text style={styles.avatarInitials}>{profileInitials(displayName)}</Text>
            )}
          </View>
          {customer ? (
            <Pressable
      ...
      {customer ? <View style={styles.kycVaultCard}>
        <Pressable accessibilityRole="button" onPress={() => setDocumentsOpen((current) => !current)} style={styles.kycVaultHeader}>
          <View style={styles.kycVaultIcon}><Image source={documentsKycIcon} style={styles.cardHeaderArtworkLarge} resizeMode="contain" /></View>
...
function Section({ title, icon, iconSource, action, onAction, children }: { title: string; icon: keyof typeof MaterialCommunityIcons.glyphMap; iconSource?: number; action?: string; onAction?: () => void; children: React.ReactNode }) { return <View style={styles.section}><View style={styles.sectionHeader}><View style={styles.sectionTitleWrap}><View style={styles.sectionIcon}>{iconSource ? <Image source={iconSource} style={styles.cardHeaderArtwork} resizeMode="contain" /> : <MaterialCommunityIcons name={icon} size={19} color="#0B63CE" />}</View><Text style={styles.sectionTitle}>{title}</Text></View>{action ? <Pressable accessibilityRole="button" onPress={onAction} style={styles.sectionAction}><Text style={styles.sectionActionText}>{action}</Text></Pressable> : null}</View>{children}</View>; }
