    ...
    <Screen title="Policy details" subtitle={vehicle?.vehicle_no ?? policy.policy_no} showLogout showTitleHeader={false}>
      <View style={styles.pageTitleRow}>
        ...
        {renewalState.action ? (
          ...
          >
            <MaterialCommunityIcons name="refresh" size={15} color="#0B57B7" />
      ...
      <View style={styles.contentStack}>
        ...
        <Card style={styles.vehicleCard}>
          <Pressable
            accessibilityRole="button"
            disabled={!vehicle}
            onPress={() => vehicle && router.push({ pathname: '/customer/vehicle-detail', params: { id: vehicle.id } } as any)}
            style={({ pressed }) => [styles.vehicleCardPressable, pressed && vehicle && styles.vehicleCardPressed]}
          >
            ...
            <View style={styles.vehicleCardCopy}>
              <Text style={styles.vehicleCardTitle}>Linked vehicle</Text>
              <Text style={styles.vehicleCardNumber} numberOfLines={1}>{vehicle?.vehicle_no || 'Vehicle record'}</Text>
            </View>
            {vehicle ? <MaterialCommunityIcons name="arrow-right" size={22} color={palette.navy} /> : null}
          </Pressable>
        </Card>
      </View>
    </Screen>
...
const styles = StyleSheet.create({
  ...
  pageRenewAction: { minHeight: 32, borderRadius: 999, borderWidth: 1.25, borderColor: '#2F73D9', paddingHorizontal: 11, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, backgroundColor: '#FFFFFF' },
  pageRenewActionPressed: { opacity: 0.82, transform: [{ scale: 0.985 }] },
  pageRenewActionText: { color: '#0B57B7', fontSize: 10.5, lineHeight: 13, fontWeight: '900' },
  ...
  vehicleCard: { backgroundColor: '#FFFFFF', borderColor: '#DCE8F4', padding: 0, overflow: 'hidden' },
  vehicleCardPressable: { minHeight: 78, paddingHorizontal: 14, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', gap: 11 },
  vehicleCardPressed: { backgroundColor: '#F7FAFF' },
  vehicleBrandIcon: { width: 46, height: 46, borderRadius: 23, backgroundColor: '#F4F7FB', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  vehicleBrandIconImage: { width: 34, height: 34 },
  vehicleCardCopy: { flex: 1, minWidth: 0 },
  vehicleCardTitle: { color: palette.navy, fontSize: 14, lineHeight: 17, fontWeight: '900' },
  vehicleCardNumber: { color: palette.slate, fontSize: 12, lineHeight: 15, fontWeight: '700', marginTop: 2 },
});
