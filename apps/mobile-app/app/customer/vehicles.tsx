...
import { Animated, Image, Modal, NativeModules, Platform, Pressable, StyleSheet, Text, TextInput, View } from 'react-native';
...
{vehicles.length === 0 ? <EmptyState title="No vehicles yet" body="Vehicle records will appear here." /> : null}
      ...
      {filteredVehicles.map((vehicle) => {
        ...
        const vehicleModel = [vehicle.model, vehicle.year ? String(vehicle.year) : null].filter(Boolean).join(' - ');
          ...
          <Pressable key={vehicle.id} onPress={() => router.push({ pathname: '/customer/vehicle-detail', params: { id: vehicle.id } })} style={({ pressed }) => [styles.vehicleCard, pressed && styles.vehicleCardPressed]}>
            ...
            <View style={styles.cardMain}>
              <View style={styles.leftPane}>
                ...
                <View style={styles.vehicleDescriptorRow}>
                  <Text style={styles.vehicleMake} numberOfLines={1}>{vehicleMake}</Text>
                  {vehicleModel ? <Text style={styles.vehicleModel} numberOfLines={1}> · {vehicleModel}</Text> : null}
                </View>
              </View>
...
async function copyVehicleNumber(value: string) {
  const nativeClipboard =
    (NativeModules as Record<string, any>).ExpoClipboard ??
    (NativeModules as Record<string, any>).RNCClipboard ??
    (NativeModules as Record<string, any>).Clipboard;

  if (typeof nativeClipboard?.setStringAsync === 'function') {
    await nativeClipboard.setStringAsync(value);
    return;
  }
...
const styles = StyleSheet.create({
  ...
  vehicleNoChip: { alignSelf: 'flex-start', minHeight: 31, borderRadius: 8, backgroundColor: '#EAF3FF', paddingHorizontal: 8, paddingVertical: 5, flexDirection: 'row', alignItems: 'center', gap: 5 },
  vehicleNoChipPressed: { opacity: 0.75 },
  ...
  vehicleDescriptorRow: { flexDirection: 'row', alignItems: 'center', marginTop: 5, minWidth: 0, flexWrap: 'wrap' },
  vehicleMake: { color: palette.navy, fontSize: 12.4, lineHeight: 15, fontWeight: '900', flexShrink: 1 },
  vehicleModel: { color: '#8A94A6', fontSize: 12.2, lineHeight: 15, fontWeight: '700', flexShrink: 1 },
