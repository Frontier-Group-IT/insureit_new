...
export default function SelfManagedClaimDetailScreen(){
 ...
 return <Screen title="Claim Tracker" subtitle={`${vehicle?.vehicle_no??claim.claim_no} • ${policy?.policy_no??'Policy linked'}`} showLogout>
  <Card title={settled?'Claim Settled':'Claim Progress'} subtitle={settled?'All 9 milestones are recorded':`${count} of 9 milestones completed`}><Text style={s.progress}>{Math.round(count/9*100)}%</Text>{!settled&&current?<Text style={s.note}>Current stage: {current.label}</Text>:<Text style={s.note}>This claim remains available as settled claim history.</Text>}</Card>
