...
export default function ClaimDetailScreen() {
  ...
  return <Screen title="Claim Detail" showLogout showTitleHeader={false}>
    ...
    <View style={[styles.quickActions, selfManaged && styles.quickActionsExternal]}>
      {selfManaged && !settled ? <QuickAction icon="pencil-circle-outline" label="Update Current Stage" primary external onPress={openCurrentSelfStage} /> : null}
      ...
      {selfManaged && !settled && claim.assistance_status !== 'requested' ? <QuickAction icon="account-tie-voice-outline" label="Request Assistance" external onPress={() => router.push({ pathname: '/customer/request-claim-assistance', params: { id: claim.id } })} /> : null}
      {!selfManaged ? <QuickAction icon="headset" label="Claims Desk" onPress={() => router.push('/customer/support')} /> : null}
    </View>
...
function QuickAction({ icon, label, onPress, primary, external }: { icon: keyof typeof MaterialCommunityIcons.glyphMap; label: string; onPress: () => void; primary?: boolean; external?: boolean }) { return <Pressable onPress={onPress} style={[styles.quickAction, external && styles.quickActionExternal, primary && styles.quickActionPrimary]}><MaterialCommunityIcons name={icon} size={20} color={primary ? '#FFF' : palette.navy} /><Text numberOfLines={2} style={[styles.quickActionText, external && styles.quickActionTextExternal, primary && { color: '#FFF' }]}>{label}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  nextActionCard:{borderWidth:1,borderRadius:17,padding:12,flexDirection:'row',gap:10,backgroundColor:'#FFF',marginBottom:10},nextActionIcon:{width:42,height:42,borderRadius:13,alignItems:'center',justifyContent:'center'},nextLabel:{fontSize:8.5,fontWeight:'900',letterSpacing:.4},nextTitle:{color:palette.navy,fontSize:13,fontWeight:'900',marginTop:2},nextBody:{color:'#667085',fontSize:10.3,lineHeight:14,fontWeight:'600',marginTop:3},quickActions:{flexDirection:'row',flexWrap:'wrap',gap:8,marginBottom:10},quickActionsExternal:{flexWrap:'nowrap'},quickAction:{minHeight:44,borderRadius:14,borderWidth:1,borderColor:'#DCE6F0',backgroundColor:'#FFF',paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:7},quickActionExternal:{flex:1,minWidth:0,minHeight:52,justifyContent:'center',paddingHorizontal:8},quickActionPrimary:{backgroundColor:palette.navy,borderColor:palette.navy},quickActionText:{color:palette.navy,fontSize:10,fontWeight:'900'},quickActionTextExternal:{flexShrink:1,textAlign:'center',lineHeight:13},
