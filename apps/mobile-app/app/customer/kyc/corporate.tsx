...
const fleetOptions: { value: FleetBand; label: string }[] = [
  { value: 'less_than_5', label: 'Less than 5' }, { value: '5_to_20', label: '5 to 20' },
  { value: '20_to_50', label: '20 to 50' }, { value: 'more_than_50', label: 'More than 50' },
];

export default function CorporateKycScreen() {
  ...
  const [companyName, setCompanyName] = useState(''); const [companyPan, setCompanyPan] = useState(''); const [gstNumber, setGstNumber] = useState('');
  const [street, setStreet] = useState(''); const [locality, setLocality] = useState('');
  const [locationQuery, setLocationQuery] = useState('');
  const [location, setLocation] = useState<IndiaLocation | null>(null); const [locationOptions, setLocationOptions] = useState<IndiaLocation[]>([]);
  const [locationSearching, setLocationSearching] = useState(false); const [locationSearched, setLocationSearched] = useState(false);
  const [fleetBand, setFleetBand] = useState<FleetBand | null>(null);
  const [fleetOpen, setFleetOpen] = useState(false);
  const [ceo, setCeo] = useState<Contact>({ name: '', phone: '', email: '' });
  const [admin, setAdmin] = useState<Contact>({ name: '', phone: '', email: '' });
  const [spoc, setSpoc] = useState<Contact>({ name: '', phone: '', email: '' });
  const [creatorPhone, setCreatorPhone] = useState('');
  const [panFile, setPanFile] = useState<PickedFile | null>(null); const [gstFile, setGstFile] = useState<PickedFile | null>(null);
  const [loading, setLoading] = useState(true); const [submitting, setSubmitting] = useState(false); const [error, setError] = useState(''); const [successVisible, setSuccessVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    if (query.length < 2 || (location && query.toLowerCase() === location.city_name.toLowerCase())) {
      setLocationOptions([]);
  ...
  function chooseLocation(nextLocation: IndiaLocation) {
    setLocation(nextLocation);
    setLocationQuery(nextLocation.city_name);
    setLocationOptions([]);
    setLocationSearched(false);
  }

  async function chooseFile(kind: 'company_pan_copy'|'gst_copy') { const result = await DocumentPicker.getDocumentAsync({ type:['application/pdf','image/jpeg','image/png'], copyToCacheDirectory:true }); if (result.canceled || !result.assets[0]) return; const asset = result.assets[0]; if (asset.size && asset.size > maxFileSize) return setError('Document must be 5 MB or smaller.'); const file = { uri:asset.uri,name:asset.name,mimeType:asset.mimeType ?? null,size:asset.size ?? null }; if (kind === 'company_pan_copy') setPanFile(file); else setGstFile(file); }
  function validate() { if (companyName.trim().length < 2) return 'Enter company name.'; if (!panPattern.test(companyPan)) return 'Enter a valid company PAN.'; if (gstNumber && !gstPattern.test(gstNumber)) return 'Enter a valid GSTIN.'; if (!street.trim() || !location) return 'Complete the company address and select city.'; if (!fleetBand) return 'Select fleet size.'; if (!/^\d{10}$/.test(creatorPhone)) return 'Sign in with a valid mobile number for Corporate Creator access.'; for (const [label,contact] of [['CEO / Head',ceo],['Admin Head',admin],['Dedicated SPOC',spoc]] as const) { if (contact.name.trim().length < 2) return `Enter ${label} name.`; if (!/^\d{10}$/.test(contact.phone)) return `Enter valid ${label} mobile.`; if (contact.email && !/^\S+@\S+\.\S+$/.test(contact.email)) return `Enter valid ${label} email.`; } if (new Set([creatorPhone,ceo.phone,admin.phone,spoc.phone]).size !== 4) return 'Corporate Creator, CEO, Admin Head and Dedicated SPOC must use four different mobile numbers.'; const existing = new Set(documents.filter((item) => item.verification_status !== 'rejected').map((item) => item.document_type)); if (!panFile && !existing.has('company_pan_copy' as never)) return 'Attach company PAN copy.'; if (gstNumber && !gstFile && !existing.has('gst_copy')) return 'Attach GST certificate.'; return ''; }
...
function ContactSection({title,value,onChange}:{title:string;value:Contact;onChange:(value:Contact)=>void}){return <Section title={`${title} login`}><Field label="Full name" value={value.name} onChangeText={(name)=>onChange({...value,name})}/><Field label="Mobile number" value={value.phone} onChangeText={(phone)=>onChange({...value,phone:phone.replace(/\D/g,'').slice(0,10)})} keyboardType="number-pad"/><Field label="Email optional" value={value.email} onChangeText={(email)=>onChange({...value,email})} keyboardType="email-address" autoCapitalize="none"/></Section>}
function LocationField({query,selected,options,searching,searched,onChange,onSelect}:{query:string;selected:IndiaLocation|null;options:IndiaLocation[];searching:boolean;searched:boolean;onChange:(value:string)=>void;onSelect:(location:IndiaLocation)=>void}){return <View style={styles.field}><Text style={styles.label}>City</Text><View style={[styles.locationShell,selected&&styles.locationSelected]}><TextInput value={query} onChangeText={onChange} placeholder="Enter at least 2 letters" placeholderTextColor="#9AA7B8" style={styles.locationInput}/>{searching?<ActivityIndicator size="small" color="#0A43A3"/>:<MaterialCommunityIcons name={selected?'check-circle':'magnify'} size={19} color={selected?'#12805C':'#607089'}/>}</View>{options.length?options.map((item)=><Pressable key={item.id} onPress={()=>onSelect(item)} style={styles.location}><Text style={styles.locationTitle}>{item.city_name}</Text><Text style={styles.locationText}>{[item.district,item.state_name,item.pincode].filter(Boolean).join(' - ')}</Text></Pressable>):null}{searched&&!searching&&!selected&&query.trim().length>=2&&!options.length?<Text style={styles.locationEmpty}>No matching city found.</Text>:null}{selected?<Text style={styles.selected}>{selected.city_name}, {selected.state_name} - {selected.pincode}</Text>:null}</View>}
function OptionDropdown({label,value,options,open,onToggle,onSelect}:{label:string;value:string|null;options:readonly {value:string;label:string}[];open:boolean;onToggle:()=>void;onSelect:(value:string)=>void}){const selected=options.find((item)=>item.value===value);return <View style={styles.dropdownWrap}><Text style={styles.label}>{label}</Text><Pressable onPress={onToggle} style={styles.dropdownButton}><Text style={styles.dropdownValue}>{selected?.label||'Select'}</Text><MaterialCommunityIcons name={open?'chevron-up':'chevron-down'} size={20} color={palette.navy}/></Pressable>{open?<View style={styles.dropdownMenu}>{options.map((item)=><Pressable key={item.value} onPress={()=>onSelect(item.value)} style={[styles.dropdownItem,item.value===value&&styles.dropdownItemActive]}><Text style={[styles.dropdownItemText,item.value===value&&styles.dropdownItemTextActive]}>{item.label}</Text>{item.value===value?<MaterialCommunityIcons name="check-circle" size={18} color={palette.navy}/>:null}</Pressable>)}</View>:null}</View>}
function asDraft(value:Json|null):Record<string,Json|undefined>{return value&&typeof value==='object'&&!Array.isArray(value)?value as Record<string,Json|undefined>:{};} function textDraft(draft:Record<string,Json|undefined>,key:string){const value=draft[key];return typeof value==='string'?value:'';}
function normalizePan(value:string){const cleaned=value.replace(/[^a-z0-9]/gi,'').toUpperCase();return collapseDoubledTyping(cleaned).slice(0,10);} function collapseDoubledTyping(value:string){if(value.length>=4&&value.length%2===0){let out='';for(let i=0;i<value.length;i+=2){if(value[i]!==value[i+1])return value;out+=value[i];}return out;}return value;}
const styles=StyleSheet.create({safe:{flex:1,backgroundColor:'#F6F8FC'},keyboard:{flex:1},header:{height:66,paddingHorizontal:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between',backgroundColor:'#fff',borderBottomWidth:1,borderBottomColor:'#E2E8F0'},back:{width:42,height:42,borderRadius:12,borderWidth:1,borderColor:'#D8E2EE',alignItems:'center',justifyContent:'center'},content:{padding:16,paddingBottom:110,gap:14},title:{color:palette.navy,fontSize:22,fontWeight:'800'},error:{backgroundColor:'#FEF3F2',borderRadius:11,padding:11},errorText:{color:'#B42318'},card:{backgroundColor:'#fff',borderWidth:1,borderColor:'#D9E2ED',borderRadius:16,padding:15},sectionTitle:{color:palette.navy,fontSize:15,fontWeight:'800',marginBottom:12},field:{marginBottom:12},label:{fontSize:12,fontWeight:'700',color:palette.navy,marginBottom:6},input:{minHeight:50,borderWidth:1,borderColor:'#D4DDE8',borderRadius:11,paddingHorizontal:12,color:'#17202F'},file:{minHeight:48,borderWidth:1,borderStyle:'dashed',borderColor:'#BFD0E4',borderRadius:11,flexDirection:'row',alignItems:'center',gap:8,paddingHorizontal:12,marginBottom:10},fileText:{color:'#0A43A3',fontWeight:'700'},options:{gap:8},option:{minHeight:44,borderWidth:1,borderColor:'#D4DDE8',borderRadius:10,justifyContent:'center',paddingHorizontal:12},optionActive:{backgroundColor:'#0A43A3',borderColor:'#0A43A3'},optionTextActive:{color:'#fff',fontWeight:'800'},locationShell:{minHeight:48,borderRadius:12,borderWidth:1,borderColor:'#D7E0EA',backgroundColor:'#fff',paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:8},locationSelected:{borderColor:'#8ED7B7',backgroundColor:'#F7FFFB'},locationInput:{flex:1,minHeight:46,color:palette.navy,fontSize:13,fontWeight:'600'},location:{padding:10,borderWidth:1,borderColor:'#D8E2EE',borderRadius:10,marginTop:7},locationTitle:{fontWeight:'700',color:palette.navy},locationText:{fontSize:11,color:'#64748B'},locationEmpty:{color:'#B42318',fontSize:10,marginTop:7},selected:{color:'#16845B',fontWeight:'700',marginTop:7},dropdownWrap:{gap:7},dropdownButton:{minHeight:50,borderRadius:12,borderWidth:1,borderColor:'#D4DDE8',backgroundColor:'#FFFFFF',paddingHorizontal:12,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},dropdownValue:{color:palette.navy,fontSize:13,fontWeight:'800'},dropdownMenu:{borderRadius:12,borderWidth:1,borderColor:'#DCE8F4',backgroundColor:'#FFFFFF',overflow:'hidden'},dropdownItem:{minHeight:44,paddingHorizontal:12,flexDirection:'row',alignItems:'center',justifyContent:'space-between',borderBottomWidth:1,borderBottomColor:'#EEF2F6'},dropdownItemActive:{backgroundColor:'#EEF5FF'},dropdownItemText:{color:'#65758B',fontSize:12,fontWeight:'700'},dropdownItemTextActive:{color:palette.navy,fontWeight:'900'},footer:{padding:12,backgroundColor:'#fff',borderTopWidth:1,borderTopColor:'#E2E8F0'},submit:{minHeight:54,borderRadius:12,backgroundColor:'#0A3B8F',alignItems:'center',justifyContent:'center'},submitText:{color:'#fff',fontWeight:'800'},loading:{flex:1,alignItems:'center',justifyContent:'center',gap:12},modalBackdrop:{flex:1,backgroundColor:'rgba(15,23,42,.45)',alignItems:'center',justifyContent:'center',padding:24},modal:{backgroundColor:'#fff',borderRadius:20,padding:24,width:'100%',maxWidth:390,alignItems:'center'},modalTitle:{fontSize:20,fontWeight:'800',color:palette.navy},modalText:{marginTop:8,textAlign:'center',color:'#59687A',lineHeight:20},modalButton:{marginTop:20,minHeight:48,borderRadius:12,backgroundColor:'#0A3B8F',paddingHorizontal:20,justifyContent:'center'}});
