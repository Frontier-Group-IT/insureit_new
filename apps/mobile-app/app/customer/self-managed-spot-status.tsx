...
export default function SelfManagedSpotStatusScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (claimResult.data.claim_service_mode !== 'self_managed') {
        router.replace({ pathname: '/customer/claim-detail', params: { id } });
        return;
      }
      setClaimNo(claimResult.data.claim_no);
      const details = milestoneResult.data?.details;
  ...
  async function submit() {
    ...
    try {
      ...
      const { error } = await supabase.from('claim_milestones').upsert({
        claim_id: id,
  ...
  return (
    <Screen title="Spot Status" showTitleHeader={false}>
      <View style={styles.topRow}>
        ...
        <View style={styles.topCopy}><Text style={styles.eyebrow}>SELF-MANAGED • STEP 2 OF 9</Text><Text style={styles.title}>Spot Status</Text><Text style={styles.subtitle}>{claimNo || 'External claim'} • Record the completed spot survey.</Text></View>
      </View>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeading}><View style={styles.iconBox}><MaterialCommunityIcons name="account-tie-outline" size={20} color="#0A43A3" /></View><View><Text style={styles.cardTitle}>Surveyor Details</Text><Text style={styles.cardSub}>Optional for self-managed external claims</Text></View></View>
      ...
      <View style={styles.helpBox}><MaterialCommunityIcons name="information-outline" size={18} color="#0A43A3" /><Text style={styles.helpText}>For broker-managed Sankalp claims these details can be provided by the claim desk. Here, you record them yourself only if available.</Text></View>
