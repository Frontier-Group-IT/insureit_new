...
old = '''          <div className={`grid border-t border-white/15 sm:grid-cols-2 ${isPartner ? "xl:grid-cols-6" : "xl:grid-cols-5"}`}>{stats.map((stat) => <HeaderStat key={stat.label} {...stat} />)}</div>'''
new = '''          <div className="grid border-t border-white/15 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{stats.map((stat) => <HeaderStat key={stat.label} {...stat} />)}</div>'''
if old not in text:
    raise SystemExit('summary grid not found')
text = text.replace(old, new, 1)

old = '''    <form action={portalAction === "create_user" ? createIntermediaryPortalLogin : resendIntermediaryPortalInvite} className="mt-1">'''
new = '''    <form action={portalAction === "create_user" ? createIntermediaryPortalLogin : resendIntermediaryPortalInvite} className="mt-0.5 leading-none">'''
if old not in text:
    raise SystemExit('portal form spacing not found')
text = text.replace(old, new, 1)

old = '''      <FormSubmitButton label={value} pendingLabel={portalAction === "create_user" ? "Creating…" : "Sending…"} className="h-auto min-h-0 p-0 text-[10.5px] font-semibold text-white underline-offset-2 transition hover:text-[#C7D2FE] hover:underline" />'''
new = '''      <FormSubmitButton label={value} pendingLabel={portalAction === "create_user" ? "Creating…" : "Sending…"} className="h-auto min-h-0 p-0 text-[10.5px] font-semibold leading-[1.15] text-white underline-offset-2 transition hover:text-[#C7D2FE] hover:underline" />'''
if old not in text:
    raise SystemExit('portal button class not found')
text = text.replace(old, new, 1)

old = '''  return <div className="flex min-w-0 items-center gap-3 border-white/15 px-4 py-4 xl:border-r xl:last:border-r-0"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/20 bg-white/10 text-white"><Icon name={icon} className="h-4 w-4" /></span><div className="min-w-0"><div className="flex items-center gap-1.5"><p className="text-[8px] font-semibold uppercase tracking-[.05em] text-white/60">{label}</p>{href && showViewIcon ? <Link href={href} aria-label={`View ${label} application`} className="inline-flex shrink-0 text-white/70 transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60"><EyeIcon /></Link> : null}</div>{portalActionNode ?? (href ? <Link href={href} className="mt-1 block truncate text-[10.5px] font-semibold text-white underline-offset-2 transition hover:text-[#C7D2FE] hover:underline">{value}</Link> : <p className="mt-1 truncate text-[10.5px] font-semibold text-white">{value}</p>)}</div></div>;'''
...
if old not in text:
    raise SystemExit('HeaderStat render not found')
text = text.replace(old, new, 1)
