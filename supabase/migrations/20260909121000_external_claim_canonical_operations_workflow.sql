-- Align External Claims with the canonical Operations-managed nine-stage workflow.
-- Policy source remains external; only workflow ownership changes when Operations proceeds.
  ...
  where cm.claim_id = p_claim_id
    ...
    and not exists (
      ...
      where csd.claim_id = cm.claim_id
        and csd.details->>'milestone_key' = cm.milestone_key
...
-- Prevent duplicate active claims for an External policy even after Operations
-- takes workflow ownership and claim_service_mode changes to broker_managed.
create or replace function public.prevent_duplicate_active_external_claim_insert()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if new.external_policy_id is null then
    return new;
  end if;

  if coalesce(new.current_status::text, '') in ('Settled', 'Closed', 'Claim Complete') then
    return new;
  end if;

  perform pg_advisory_xact_lock(hashtextextended(new.external_policy_id::text, 0));

  if exists (
    select 1
    from public.claims existing
    where existing.external_policy_id = new.external_policy_id
      and existing.id is distinct from new.id
      and coalesce(existing.current_status::text, '') not in ('Settled', 'Closed', 'Claim Complete')
      and (
        existing.claim_service_mode is distinct from 'self_managed'::public.claim_service_mode
        or (
          select count(distinct cm.milestone_key)
          from public.claim_milestones cm
          where cm.claim_id = existing.id
            and cm.milestone_key in (
    ...
    raise exception 'Claim already in progress for this external policy.' using errcode = 'P0001';
  end if;
...
drop trigger if exists prevent_duplicate_active_external_claim_insert on public.claims;
create trigger prevent_duplicate_active_external_claim_insert
before insert on public.claims
for each row
execute function public.prevent_duplicate_active_external_claim_insert();
