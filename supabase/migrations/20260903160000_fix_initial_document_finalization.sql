-- Include customer-action and legacy initial-document states in the protected
-- reconciliation path. The application verifies all six categories first.
...
begin
  ...
  if not exists (select 1 from public.claim_documents where claim_id = p_claim_id and verification_status = 'verified' and lower(document_type) like any (array['%gr / load bill%', '%gr copy / load challan%', '%gr copy / road challan%', '%gr / load challan%', '%road challan%', '%load challan%'])) then raise exception 'GR / Load Bill is not verified.'; end if;
