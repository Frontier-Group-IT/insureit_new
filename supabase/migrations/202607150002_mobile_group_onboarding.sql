-- Mobile Group onboarding submission. Group applications intentionally collect only
-- the umbrella name and owner contact; child entities are linked after approval.
...
begin
  ...
  values (
    ...
    coalesce(result.applicant_phone, ''),
    normalized_email,
    true,
    auth.uid(),
    'pending'
...
revoke all on function public.submit_group_onboarding_application(uuid, text, text, text) from public;
grant execute on function public.submit_group_onboarding_application(uuid, text, text, text) to authenticated;
