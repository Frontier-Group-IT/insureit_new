-- Fix External Claim Operations takeover on persisted enum milestone keys.
-- This follow-up replaces only the takeover RPC; duplicate-claim guards remain unchanged.
...
begin
  ...
  -- The enum milestone key is explicitly cast to text before comparing it with
  -- the persisted JSON text value in claim_stage_details.
