...
begin
  ...
  select jsonb_build_object(
    ...
    'in_policy_intake_count', count(*) filter (
      where o.opportunity_status not in ('won','lost','invalid_contact','do_not_contact','renewed_elsewhere','duplicate')
        and exists (
          ...
          where l.opportunity_id=o.id and l.partner_id=o.partner_id
        )
    ),
...
drop function if exists public.partner_app_list_external_renewals(integer,integer,text,text,text,text,text,text);
...
create function public.partner_app_list_external_renewals(
  ...
  p_intake_state text default 'all'
)
returns table (
  opportunity_id uuid,
  batch_id uuid,
...
declare
  ...
  v_intake_state text := lower(coalesce(nullif(btrim(p_intake_state),''),'all'));
begin
  ...
  if v_intake_state not in ('all','not_started','in_progress') then
    raise exception 'Invalid Policy Intake state filter';
  end if;

  if v_mode <> 'due' then v_window := 'all'; end if;
  if v_mode <> 'follow_up' then v_follow_up := 'all'; end if;
  ...
  with filtered as (
    select
      ...
      case when l.id is null then 'not_started' else 'in_progress' end as intake_state,
      r.id as intake_id,
      r.intake_number,
      r.status::text as intake_status,
      o.created_at
    from public.external_renewal_opportunities o
    join public.external_renewal_import_batches b on b.id=o.batch_id
    left join public.external_renewal_policy_intake_links l
      on l.opportunity_id=o.id and l.partner_id=o.partner_id
    left join public.policy_intake_requests r
      on r.id=l.intake_id
      and (
    ...
    where o.partner_id=any(v_partner_ids)
      ...
      and (
        v_intake_state='all'
        or (v_intake_state='not_started' and l.id is null)
        or (v_intake_state='in_progress' and l.id is not null)
      )
      and (
        v_search is null
...
revoke all on function public.partner_app_list_external_renewals(integer,integer,text,text,text,text,text,text) from public, anon;
grant execute on function public.partner_app_list_external_renewals(integer,integer,text,text,text,text,text,text) to authenticated, service_role;
