drop function if exists public.create_customer_vehicle(uuid,text,text,text,text,integer,text,text,text,numeric,date,date,date,date,date,date);

create function public.create_customer_vehicle(
  p_customer_id uuid,
  p_vehicle_no text,
...
begin
  ...
  if p_customer_id is null or not public.can_access_customer(p_customer_id) then raise exception 'You do not have access to add vehicles for this customer.'; end if;
  ...
  if p_year is null or p_year < 1950 or p_year > extract(year from now())::integer + 1 then raise exception 'Enter a valid manufacturing year.'; end if;
...
grant execute on function public.create_customer_vehicle(uuid,text,text,text,text,integer,text,text,text,numeric,date,date,date,date,date,date,text) to authenticated;
revoke execute on function public.create_customer_vehicle(uuid,text,text,text,text,integer,text,text,text,numeric,date,date,date,date,date,date,text) from anon, public;
