...
set dp_first_name = coalesce(dp_first_name, nullif(split_part(trim(coalesce(dp_name, '')), ' ', 1), '')),
    dp_last_name = coalesce(dp_last_name, case when array_length(regexp_split_to_array(trim(coalesce(dp_name, '')), '\s+'), 1) > 1 then (regexp_split_to_array(trim(dp_name), '\s+'))[array_length(regexp_split_to_array(trim(dp_name), '\s+'), 1)] else null end),
    dp_middle_name = coalesce(dp_middle_name, case when array_length(regexp_split_to_array(trim(coalesce(dp_name, '')), '\s+'), 1) > 2 then array_to_string((regexp_split_to_array(trim(dp_name), '\s+'))[2:array_length(regexp_split_to_array(trim(dp_name), '\s+'), 1)-1], ' ') else null end),
...
begin
  if new.partner_type = 'misp' then
    if nullif(trim(coalesce(new.dp_name,'')),'') is not null and (new.dp_first_name is null or new.dp_middle_name is null or new.dp_last_name is null) then
      v_parts := regexp_split_to_array(trim(new.dp_name), '\s+');
      new.dp_first_name := coalesce(new.dp_first_name, v_parts[1]);
      new.dp_last_name := coalesce(new.dp_last_name, case when array_length(v_parts,1) > 1 then v_parts[array_length(v_parts,1)] else null end);
      new.dp_middle_name := coalesce(new.dp_middle_name, case when array_length(v_parts,1) > 2 then array_to_string(v_parts[2:array_length(v_parts,1)-1], ' ') else null end);
    end if;
...
create trigger sync_misp_dp_identity_fields before insert or update on public.posp_misp_onboarding_profiles for each row execute function public.sync_misp_dp_identity_fields();
...
create trigger validate_posp_misp_required_fields before insert or update of external_onboarding_id,address,city,state,postal_code,pan_number,bank_id,bank_account_number,bank_ifsc_code,dp_first_name,dp_middle_name,dp_last_name,dp_phone,dp_email,dp_pan_number,dp_date_of_birth,dp_aadhaar_hash,date_of_birth,aadhaar_hash on public.posp_misp_onboarding_profiles for each row execute function public.validate_posp_misp_required_fields();
...
begin
  ...
  if tg_op = 'UPDATE' and v_old_pan is distinct from v_pan then
    update public.posp_misp_onboarding_profiles set iib_remarks=null,final_account_type=null,partner_decision='not_applicable',partner_decision_at=null,partner_decision_by=null,partner_decision_remark=null,updated_at=now() where id=new.id;
  end if;
  if new.workflow_stage='pre_iib' and new.bank_id is not null and v_pan ~ '^[A-Z]{5}[0-9]{4}[A-Z]$' and exists(select 1 from public.customer_onboarding_applications app where app.id=new.application_id and app.status in('submitted','under_review','changes_requested')) then
...
create trigger queue_posp_misp_pan_when_ready after insert or update of pan_number,dp_pan_number,bank_id,workflow_stage on public.posp_misp_onboarding_profiles for each row execute function public.queue_posp_misp_pan_on_profile_ready();

update public.pan_verification_jobs job
set pan_number=case when profile.partner_type='misp' then profile.dp_pan_number else profile.pan_number end,status='pending',result_code=null,result_message=null,last_error=null,checked_by_device=null,requested_at=now(),started_at=null,completed_at=null,attempt_count=0,updated_at=now()
from public.posp_misp_onboarding_profiles profile
where job.application_id=profile.application_id and case when profile.partner_type='misp' then profile.dp_pan_number else profile.pan_number end ~ '^[A-Z]{5}[0-9]{4}[A-Z]$' and job.pan_number is distinct from case when profile.partner_type='misp' then profile.dp_pan_number else profile.pan_number end;
