...
-- Hardening for Group/Branch portal access introduced in 20260915123000..125000.
-- This is a forward-only corrective migration: it does not rewrite existing
-- Partner credentials, hierarchy rows, memberships, or portal mappings.
...
begin
  if new.entity_type = 'group' then
    if not exists (
      ...
      raise exception 'Portal Group target must be an active Group';
    end if;
    ...
    if not exists (
      ...
      raise exception 'Portal Branch target must be an active Branch';
    end if;
  else
    raise exception 'Unsupported portal access entity type: %', new.entity_type;
  end if;
  return new;
end;
$$;
...
begin
  ...
  if v_identity.entity_type = 'branch' then
    if not exists (
      ...
      from public.partners p
      join public.partner_branch_profiles b on b.partner_id = p.id
      where p.id = v_identity.entity_id
        and p.partner_status = 'active_partner'
    ) then
      return;
    end if;
  ...
  if not exists (
    select 1
    from public.intermediary_groups g
    where g.id = v_identity.entity_id
      and g.status = 'active'
  ) then
    return;
  end if;
  ...
  with member_partners as (
    ...
    where m.group_id = v_identity.entity_id
      ...
      and p.partner_status = 'active_partner'
      and p.parent_partner_id is null
  ), scoped as (
    select mp.id from member_partners mp
    union
    select p.id
    from public.partners p
    join member_partners mp on mp.id = p.parent_partner_id
    where p.partner_status = 'active_partner'
      and exists (select 1 from public.partner_branch_profiles b where b.partner_id = p.id)
  )
  select
    'group'::text,
    v_identity.entity_id,
...
-- Policy Intake uses its long-standing profile submitter columns for employee
-- identities and portal-account columns for legacy Partner identities. For an
-- explicit Group/Branch portal user, present a Policy-Intake-specific identity
-- that uses the authenticated profile as submitter while retaining the normal
-- canonical identity for every other Partner portal feature.
create or replace function public.partner_policy_intake_current_identity()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  if coalesce(v_identity ->> 'portal_access_type', '') in ('group', 'branch') then
...
revoke all on function public.partner_policy_intake_current_identity() from public, anon;
grant execute on function public.partner_policy_intake_current_identity() to authenticated, service_role;

-- Recreate commercial scope with two corrections:
-- 1) explicit Branch access inherits Group membership through its root Partner;
-- 2) legacy Branch Partner logins preserve the same parent-Partner inheritance.
...
declare
  ...
  v_actor_kind text;
  v_partner_id uuid;
  v_group_membership_partner_id uuid;
...
begin
  ...
  if found then
    ...
    if v_access_context.account_type = 'group' then
      ...
      into v_group_membership_partner_id
      from public.partners p
      ...
      where m.partner_id = v_group_membership_partner_id
        and m.effective_to is null;
    end if;
  ...
  -- Preserve the existing Partner/Employee fallback contract.
  v_identity := public.partner_app_current_identity();
  if v_identity is null then
    return null;
  end if;
