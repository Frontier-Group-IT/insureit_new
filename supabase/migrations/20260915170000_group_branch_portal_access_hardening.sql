...
-- Harden the explicit Group/Branch access context so every scope-only caller
-- revalidates the auth profile and the current business target state.
...
begin
  ...
  if v_identity.entity_type = 'branch' then
    if not exists (
      ...
      join public.partner_branch_profiles branch
        on branch.partner_id = partner.id
      where partner.id = v_identity.entity_id
  ...
  with member_partners as (
    ...
    join public.partners member
      on member.id = membership.partner_id
     and member.partner_status = 'active_partner'
    where membership.group_id = v_identity.entity_id
      and membership.effective_to is null
  ), scoped as (
    select member.id
    from member_partners member
    union
    select branch.id
    from public.partners branch
    join member_partners member
      on member.id = branch.parent_partner_id
    where branch.partner_status = 'active_partner'
      and exists (
        select 1
        from public.partner_branch_profiles branch_profile
        where branch_profile.partner_id = branch.id
      )
  )
  select
    'group'::text,
...
-- Restore the pre-portal-access Partner/Branch compatibility semantics in the
-- legacy fallback while retaining explicit Group/Branch scopes at the top.
...
begin
  ...
  if found then
    ...
    if v_access_context.account_type = 'group' then
      ...
      from public.partners partner
      where partner.id = v_access_context.entity_id;
  ...
  -- The explicit mapping did not resolve. Delegate to the preserved legacy
  -- Partner/Employee identity implementation so Partner credentials stay intact.
