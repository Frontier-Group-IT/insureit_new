...
bill_agg as (
  select policy_id,
         ...
         case when bool_or(status='Billed') then 'Billed' when bool_or(status='Billing details incomplete') then 'Billing details incomplete' else 'Unbilled' end as billing_status
  from public.policy_payin_bills group by policy_id
),
payout_agg as (
  select policy_id,coalesce(sum(gross_payout),0)::numeric as gross_payout,coalesce(sum(retention_amount),0)::numeric as retention_amount,
         case when bool_or(lower(coalesce(status,''))='paid') then 'Paid' when bool_or(lower(coalesce(status,''))='approved') then 'Approved' else coalesce(max(status),'Pending') end as payout_status,
         max(payout_date) as latest_payout_date
  from public.policy_intermediary_payouts group by policy_id
),
base as (
  select p.id,p.customer_id,p.vehicle_id,p.insurance_company_id,p.policy_no,p.policy_type,
         coalesce(p.issuance_date,p.start_date,p.created_at::date) as business_date,p.rm_name,p.intermediary_code,
         coalesce(c.company_name,c.contact_name,c.customer_code,'—') as customer_name,c.customer_code,coalesce(v.vehicle_no,'—') as vehicle_no,
         coalesce(ic.name,'Unassigned insurer') as insurer_name,coalesce(ppd.gross_premium,p.premium_amount,0)::numeric as gross_premium,
         coalesce(pid.total_projected_payin,0)::numeric as projected_payin,coalesce(pid.tds_amount,0)::numeric as payin_tds,coalesce(pid.payin_after_tds,0)::numeric as payin_after_tds,
         coalesce(ba.billed_amount,0)::numeric as billed_amount,coalesce(ba.billing_status,'Unbilled') as billing_status,ba.latest_bill_date,
         coalesce(pa.gross_payout,0)::numeric as gross_payout,coalesce(pa.retention_amount,0)::numeric as retention_amount,coalesce(pa.payout_status,'Pending') as payout_status,pa.latest_payout_date
  from public.policies p join public.customers c on c.id=p.customer_id
  left join public.vehicles v on v.id=p.vehicle_id left join public.insurance_companies ic on ic.id=p.insurance_company_id
  left join public.policy_premium_details ppd on ppd.policy_id=p.id left join public.policy_payin_details pid on pid.policy_id=p.id
  left join bill_agg ba on ba.policy_id=p.id left join payout_agg pa on pa.policy_id=p.id
...
rms as (select coalesce(jsonb_agg(to_jsonb(x) order by x.projected_payin desc),'[]'::jsonb) arr from (
  ...
  coalesce(sum(gross_payout),0)::numeric gross_payout,coalesce(sum(retention_amount),0)::numeric retention_amount from filtered group by coalesce(rm_name,'Unassigned')) x),
billing as (select coalesce(jsonb_agg(to_jsonb(x) order by x.sort_order),'[]'::jsonb) arr from (
  ...
  case billing_status when 'Unbilled' then 1 when 'Billing details incomplete' then 2 when 'Billed' then 3 else 9 end sort_order from filtered group by billing_status) x),
filter_options as (select jsonb_build_object(
  ...
  'billing_statuses',coalesce((select jsonb_agg(status order by status) from(select distinct billing_status status from base) z),'[]'::jsonb)) obj),
...
register_rows as(select coalesce(jsonb_agg(to_jsonb(x) order by x.business_date desc nulls last,x.policy_no),'[]'::jsonb) arr from(
  select f.* from filtered f order by business_date desc nulls last,policy_no limit least(greatest(coalesce(p_page_size,25),1),100)
  offset((greatest(coalesce(p_page,1),1)-1)*least(greatest(coalesce(p_page_size,25),1),100))) x)
select jsonb_build_object('summary',(select obj from summary),'insurers',(select arr from insurers),'rms',(select arr from rms),'billing',(select arr from billing),'filters',(select obj from filter_options),
