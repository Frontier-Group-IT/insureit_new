...
-- Customer Activity "Via" describes the workflow that produced each action,
-- independently from the customer's immutable creation source.
...
begin
  ...
  if old_business is distinct from new_business then
    activity_channel := case new.partner_type
      when 'corporate' then 'corporate_profile'
      when 'dealership' then 'dealership_profile'
      when 'group' then 'group_profile'
    ...
    values (
      ...
      new.id,
      old_business,
      new_business || jsonb_build_object('activity_channel', activity_channel)
    );
  end if;
  return new;
end;
  ...
  if tg_op = 'INSERT' and new.is_active and new.status = 'active' then
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'activity_channel', 'customer_relationships'
      )
    );
  elsif tg_op = 'UPDATE'
    and old.is_active
