...
create or replace function public.partner_app_insert_claim_stage_detail(
  p_claim_id uuid,
  p_stage public.claim_status,
  p_details jsonb
)
returns uuid
language plpgsql
...
begin
  ...
  values (p_claim_id, p_stage, coalesce(p_details, '{}'::jsonb), v_actor)
...
revoke all on function public.partner_app_insert_claim_stage_detail(uuid,public.claim_status,jsonb) from public, anon;
grant execute on function public.partner_app_insert_claim_stage_detail(uuid,public.claim_status,jsonb) to authenticated, service_role;
...
declare
  ...
  v_operations_actor boolean := false;
  v_partner_actor boolean := false;
begin
  ...
  v_operations_actor := public.is_operations_role() and public.can_access_claim(auth.uid(), new.claim_id);
  v_partner_actor := public.partner_app_claim_in_scope(new.claim_id);

  if not v_operations_actor and not v_partner_actor then
