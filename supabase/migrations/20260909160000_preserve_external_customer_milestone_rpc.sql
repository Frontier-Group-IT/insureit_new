-- Preserve Customer milestone processing for externally sourced claims after
-- Operations enters the same claim into the broker-managed canonical workflow.
--
-- This is intentionally additive: previously applied migrations are not rewritten.
-- Internal/SIBL broker-managed claims remain blocked from the Customer self-managed RPC.
...
begin
  ...
  -- External-policy identity is durable even after Operations changes ownership to
  -- broker_managed and/or accepts assistance. Preserve the Customer External Claim
  -- milestone journey for that source only. For all non-External claims, retain the
  -- original self-managed + assistance-not-accepted restriction.
  if not (
    (
      v_claim.policy_service_source = 'external'::public.policy_service_source
      and v_claim.external_policy_id is not null
    )
    or (
      v_claim.claim_service_mode = 'self_managed'::public.claim_service_mode
      and coalesce(v_claim.assistance_status::text, 'not_requested') <> 'accepted'
    )
  ) then
    raise exception 'Only customer-processable External or self-managed claims can be updated here.';
  end if;
