-- Canonical policy-business report scope.
-- Policy visibility is owned by policies.rm_employee_id, not by customer visibility.
-- This prevents a shared customer from widening one RM's policy/report scope.

create or replace function public.get_policy_business_report_v4(
...
filter_rms as (
  select coalesce(
    jsonb_agg(
      jsonb_build_object('id',x.rm_employee_id,'name',coalesce(x.rm_name,'Relationship manager'))
      order by coalesce(x.rm_name,'Relationship manager'),x.rm_employee_id
    ),
    '[]'::jsonb
  ) rows
  from (
    select distinct rm_employee_id,rm_name
    from policy_base
    where rm_employee_id is not null
  ) x
),
...
revoke all on function public.get_policy_business_report_v4(uuid[],date,date,uuid,uuid,text,text,text,integer,integer) from public;
