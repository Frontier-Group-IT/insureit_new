...
set requested_type = coalesce(i.requested_type, case when p.partner_type = 'misp' then 'misp' else 'posp' end),
    ...
    onboarding_id = coalesce(nullif(trim(i.onboarding_id), ''), i.intermediary_code),
    account_status = coalesce(nullif(trim(i.account_status), ''), 'active'),
    registration_status = coalesce(nullif(trim(i.registration_status), ''), 'partner_active'),
    portal_access_status = coalesce(nullif(trim(i.portal_access_status), ''), 'not_created'),
    iib_status = coalesce(nullif(trim(i.iib_status), ''), 'not_applicable'),
    compliance_status = coalesce(nullif(trim(i.compliance_status), ''), 'active'),
    updated_at = now()
from public.posp_misp_onboarding_profiles p
where p.application_id = i.application_id
  and i.intermediary_type = 'partner'
  and (
    i.requested_type is null
    or i.display_name is null
    or trim(i.display_name) = ''
    or i.onboarding_id is null
    or trim(i.onboarding_id) = ''
    or i.account_status is null
    or i.registration_status is null
    or i.portal_access_status is null
    or i.iib_status is null
    or i.compliance_status is null
  );
