...
alter table if exists public.intermediary_onboarding_applications
  drop constraint if exists intermediary_onboarding_applications_partner_category_check;
alter table if exists public.intermediary_onboarding_applications
  add constraint intermediary_onboarding_applications_partner_category_check
  check (partner_category is null or partner_category in ('individual','business'));

alter table if exists public.posp_misp_onboarding_profiles
  drop constraint if exists posp_misp_onboarding_profiles_partner_category_check;
alter table if exists public.posp_misp_onboarding_profiles
  add constraint posp_misp_onboarding_profiles_partner_category_check
  check (partner_category is null or partner_category in ('individual','business'));
...
begin
  ...
  select application_id into v_existing
  from public.posp_misp_onboarding_profiles
  where coalesce(aadhaar_hash, dp_aadhaar_hash) = v_aadhaar_hash
    and application_id <> p_application_id
    and partner_id is not null
  limit 1;
