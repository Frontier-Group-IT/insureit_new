...
-- Forward-only corrective migration for the Group/Branch portal access feature.
-- Historical migrations remain immutable. Existing Partner credentials and
-- business hierarchy rows are not modified.
...
begin
  ...
  with member_partners as (
    ...
    select mp.partner_id as id
    from member_partners mp
    union
    select p.id
    from public.partners p
    join public.partner_branch_profiles b on b.partner_id = p.id
...
revoke all on function public.partner_portal_scoped_partner_ids() from public, anon;
grant execute on function public.partner_portal_scoped_partner_ids() to authenticated, service_role;
...
begin
  ...
  -- Exact legacy Partner/Employee boundary, including Branch -> parent Partner
  -- Group inheritance and the business-Group employee isolation rule.
