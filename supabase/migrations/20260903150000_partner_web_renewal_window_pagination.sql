...
begin
  if v_mode not in ('due','expired') then
    raise exception 'Invalid renewal mode';
  end if;

  if v_window not in ('all','0_7','8_15','16_30') then
    raise exception 'Invalid renewal window';
  end if;

  if v_mode = 'expired' then
    v_window := 'all';
  end if;
  ...
  filtered as (
    ...
    where
      (
        ...
        (
          v_mode='due'
          and b.end_date between current_date and current_date+30
          and (
            v_window='all'
            or (v_window='0_7' and b.end_date between current_date and current_date+7)
            or (v_window='8_15' and b.end_date between current_date+8 and current_date+15)
            or (v_window='16_30' and b.end_date between current_date+16 and current_date+30)
          )
        )
      )
      and (
  ...
  from filtered f
  order by
    case when v_mode='expired' then f.end_date end desc,
    case when v_mode='due' then f.end_date end asc,
    f.created_at desc
  limit v_limit offset v_offset;
end;
$$;

revoke all on function public.partner_app_list_renewals(integer,integer,text,text,text) from public, anon;
grant execute on function public.partner_app_list_renewals(integer,integer,text,text,text) to authenticated, service_role;
