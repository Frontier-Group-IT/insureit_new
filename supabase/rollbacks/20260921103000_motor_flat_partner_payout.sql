...
drop function if exists public.apply_motor_flat_partner_payout_v1(uuid,jsonb);
...
begin
  select commercial_status,commercial_reviewed_at,commercial_reviewed_by,projected_od_percent,projected_tp_percent,insurer_scheme_amount,total_projected_payin
  into v_old_payin_status,v_old_payin_reviewed_at,v_old_payin_reviewed_by,v_old_payin_od,v_old_payin_tp,v_old_payin_scheme,v_old_payin_total
  from public.policy_payin_details where policy_id = p_policy_id;

  select id,commercial_status,commercial_reviewed_at,commercial_reviewed_by,od_payout_percent,tp_payout_percent,gross_payout
  into v_old_payout_id,v_old_payout_status,v_old_payout_reviewed_at,v_old_payout_reviewed_by,v_old_payout_od,v_old_payout_tp,v_old_payout_total
  from public.policy_intermediary_payouts where policy_id = p_policy_id order by created_at desc limit 1;

  v_result := public.update_motor_policy(p_policy_id, p_payload);

  select projected_od_percent,projected_tp_percent,insurer_scheme_amount,total_projected_payin
  into v_new_payin_od,v_new_payin_tp,v_new_payin_scheme,v_new_payin_total
  from public.policy_payin_details where policy_id = p_policy_id;

  select id,od_payout_percent,tp_payout_percent,gross_payout
  into v_new_payout_id,v_new_payout_od,v_new_payout_tp,v_new_payout_total
  from public.policy_intermediary_payouts where policy_id = p_policy_id order by created_at desc limit 1;
