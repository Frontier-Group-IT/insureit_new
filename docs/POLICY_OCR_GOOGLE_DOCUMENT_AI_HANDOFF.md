...
**MERGED / NOT DEPLOYED:** PR #2088 merged as `686f04cd440b0dd7f27ba017e85ae01607a41f28` after canonical Verify web portal run `35377504937` (run #4186) passed. Base `parsePolicyDocument` now has first-class `icici_lombard` family detection using current-policy ICICI legal-name/header evidence plus Product Code 3003 + GCV layout evidence, preventing the document from being preprocessed as New India before the dedicated ICICI refiner. Production upload/review/apply for the supplied policy remains **UNVERIFIED** until an explicitly approved production deployment and authenticated replay.
