...
The server-only notification uses `RESEND_API_KEY` and `RESEND_FROM_EMAIL`. It contains no attachment, raw OCR, policy/vehicle/customer identifiers or PII. A blind copy is sent to `it@insureit.in` for operational oversight while the assigned reviewer remains the primary recipient. Missing Resend configuration records a failed notification attempt and never marks it sent. The workflow code and migration are merged; production notification behavior remains subject to the deployed commit and an actual assigned-task send.

## 17. Bounded insurer/layout orchestrator — 2026-09-07

**IMPLEMENTED / NOT APPLIED / NOT DEPLOYED:** the scheduled endpoint is now fail-closed unless `POLICY_OCR_ORCHESTRATOR_ENABLED=true` and a configured running `POLICY_OCR_ORCHESTRATOR_ID` exists. Migration `20260907170000_policy_ocr_training_orchestrator.sql` persists iteration budgets, top-ten insurer-volume sampling, product/layout strata, deterministic holdout/fresh-sibling assignments, leases/retries, field-level reviewer feedback, sanitized candidate proposals and satisfaction tasks. A run is eligible for satisfaction only after the server-side 95% field-accuracy, zero-wrong-financial-values, holdout and three-fresh-sibling gates. Candidate proposals remain reviewable data; no parser source is self-modified, merged or deployed. Required migration application and environment configuration remain a deployment blocker.

## 20. ICICI Lombard GCV structured training — 2026-09-18

**MERGED / NOT DEPLOYED:** PR #2087 merged as `f59162a99112382d920f19c420c333af73197cfb` after canonical Verify web portal run `35376638762` passed.

The supplied ICICI Lombard Goods Carrying Vehicles Package policy exposed a repeated structural failure class: generic flattened extraction could misroute the insurer and promote table headings such as Category, Model/RTO labels, or unrelated money candidates into review fields. The dedicated refiner `policy-ocr-icici-lombard-refiner.ts` now uses strong ICICI identity evidence plus Layout Parser table columns for registration, make, model, manufacturing year, chassis, engine and Total IDV. Financial values are accepted from explicit ICICI Total Own Damage, Total Liability and Total Package rows only when OD + TP reconciles to printed net/package premium. Explicit displayed tax and gross totals are preserved; the known whole-rupee printed rounding difference is warned rather than recomputed. Explicit CPA opt-out/CSI zero evidence maps to CPA No / 0.

A privacy-safe regression reproduces the prior contaminated state without storing real customer identity and proves correction of insurer routing, registration state, header-value contamination, IDV and premium totals. The complete OCR suite now includes `policy-ocr:icici-lombard-regression`.

**CONTINUATION / IMPLEMENTED ON FOLLOW-UP BRANCH:** base `parsePolicyDocument` still had no ICICI family and could score the generic “Goods Carrying Vehicle Package Policy” structure as New India before the approved-layout ICICI refiner repaired it. Branch `ocr/icici-lombard-routing-hardening` adds first-class `icici_lombard` family detection using current-policy ICICI legal-name/header evidence and Product Code 3003 + GCV layout evidence, with regression coverage. This follow-up must pass canonical CI and be merged before deployment. Production upload/review/apply for the supplied policy remains **UNVERIFIED** until an explicitly approved production deployment and authenticated replay.
