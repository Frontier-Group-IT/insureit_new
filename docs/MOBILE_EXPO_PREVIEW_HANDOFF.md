...
**IMPLEMENTED / NOT YET DEPLOYED:** branch `ui/partner-business-reference-layout-v3` rebases the approved Business redesign onto the latest `main`. It reuses the Home hero background asset and Home typography scale, adds the full-width branded hero with logo/actions and floating search/filter surface, and enlarges/refines the Business Overview, trend, product, insurer placeholder, Quick Actions and Network surfaces. No backend, RPC, schema, RLS, authorization, native dependency, runtime-version or APK/AAB change is included. Top Insurers remains evidence-safe because the current Business payload does not supply insurer-wise totals.
...
- The Policies banner repository object is a real binary JPEG, not base64 text. A UTF-8 read fails on byte `0xFF`, while the GitHub binary fetch exposes a JPEG header.
- The earlier runtime-0.1 OTA repair incorrectly attempted `base64 --decode` on that binary file, causing the compatibility workflow to stop with `base64: invalid input`.
- The previous direct copy reached Expo/Metro but the supplied progressive JPEG produced `unsupported file type: undefined` in the legacy runtime-0.1 export path.
- The workflow now copies the binary banner directly, verifies it as JPEG, then uses ffmpeg to normalize only the temporary compatibility copy to a baseline MJPEG/JPEG before Expo export.
- The committed user-supplied banner itself is not replaced or visually altered; only the OTA staging copy is normalized for legacy bundler compatibility.
