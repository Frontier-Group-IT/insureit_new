...
**DEPLOYED dashboard coverage summary polish OTA:** Commit `716f1cfaaa1b5471ddbb454dad84b2084fbaa843` removes the protected/unprotected copy from under the vehicle count, moves that status into the bottom fleet ticker with `All vehicles are covered` when applicable, keeps the ticker chevron at the far right, and replaces the dotted protection indicator with an animated continuous red/green SVG coverage ring. Published to Expo `preview` runtime `0.2.0`; update group `95a0dcbd-88c4-4509-b299-85663af17574`; Android update `01a0091c-3540-7af3-a48f-9bb9954a5de4`; iOS update `01a0091c-3540-761a-91b7-df3e6b059a6b`. Message: `Polish dashboard coverage summary 2026-08-16`.

**DEPLOYED final dashboard polish OTA:** Commit `e6de727470b172fe33dbbe4dedca46fe20f76286` centers the fleet vehicle count within its own column, adds the `Vehicles` label below the count, switches the coverage ring fill from green to InsureIT blue, changes the fleet bottom ticker to show only uncovered vehicles or the all-protected check state, adds an animated attention icon when a vehicle lacks active policy, matches the Quick Actions heading style to the fleet heading, animates only the pending-renewal badge, gives each Claims KPI a distinct status background, and flattens the Claims Desk Call/WhatsApp/Ticket controls while updating the tagline. Published to Expo `preview` runtime `0.2.0`; update group `d4697442-c16c-4d21-9f39-be36d14a7f6d`; Android update `01a00938-841b-7d99-aba4-c8fa24dd82a0`; iOS update `01a00938-841b-7ec2-bc67-6be60c61abc1`. Message: `Finalize customer dashboard polish 2026-08-16`.

**DEPLOYED TWP bike artwork OTA:** Commit `464cbbf2376cbac684431a3862029dd871a21a16` replaces the TWP vehicle placeholder with the user-supplied bike artwork in `apps/mobile-app/assets/vehicles/twp-bike.png`, applies the TWP image on both My Vehicles cards and Vehicle Detail, and broadens two-wheeler matching for `TWP`, `Two Wheeler`, `Two-Wheel`, `2 Wheel`, `Motorcycle`, `Motor Cycle`, `Bike`, and `Scooter`. Published to Expo `preview` runtime `0.2.0`; update group `038bba5b-801d-429e-9105-edc30110887d`; Android update `01a00a05-e4c4-7bc4-8794-92a1a8c67510`; iOS update `01a00a05-e4c4-73f1-a250-5daa45d258d9`. Message: `Show TWP bike artwork 2026-08-16`.

**DEPLOYED vehicle empty-policy CTA OTA:** Commit `274c898eb786ecb6cd61ca320b6c50072a1311cb` refines the no-policy state on My Vehicles cards. Instead of compressed `Add`/`Add now` text, cards now show a compact `Protection needed` panel with `Get quote` as the primary CTA and `Add policy` as the secondary action for attaching an existing policy. `Get quote` passes the selected vehicle number into `/customer/insurance-quote` so the quote form starts prefilled. Published to Expo `preview` runtime `0.2.0`; update group `ac4b4a18-0034-4b95-ab3b-38fee733d440`; Android update `01a00a0f-7c1e-71e9-ac31-a4b8a4de693e`; iOS update `01a00a0f-7c1e-7d50-b826-f18cc5861a62`. Message: `Refine vehicle empty policy CTA 2026-08-16`.
