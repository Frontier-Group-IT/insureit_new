...
- **USER-CONFIGURED / FIX PENDING DEPLOYMENT:** Vercel Preview uses the OpenRouter OpenAI-compatible endpoint with a user-managed Preview-only key and restored `google/gemma-4-26b-a4b-it:free`. Ling returned HTTP 400 for the required tools plus structured-output request. After restoration, Gemma reached the provider but exceeded the former 15-second timeout and returned 504; the branch now uses a 30-second provider timeout, a 45-second route ceiling, and metadata-only timeout diagnostics. Never store or repeat the key.
