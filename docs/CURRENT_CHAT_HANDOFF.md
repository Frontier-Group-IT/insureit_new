...
**IMPLEMENTED / NOT DEPLOYED (2026-09-09):** OCR training rerun diagnostics now retain only bounded metadata in the existing `extraction_method` field (`layout_tables=present|zero_or_unusable|not_requested` plus a capped table count), while the queue displays parser version and the trigger-maintained `updated_at` timestamp. Successful manual reruns now call `router.refresh()` after `revalidatePath`, preventing stale queue rows from being mistaken for an unchanged rerun. No migration, raw OCR, table cells, policy/vehicle identifiers, provider payloads or credentials were added. Code commit `32706924`; targeted OCR/training regressions, typecheck, lint, CI-env production build and diff check passed. Live production rerun remains unverified and no deployment was performed.
