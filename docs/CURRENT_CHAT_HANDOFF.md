...
**IMPLEMENTED:** added an admin-only `/policies/ocr-training` page and `policy_ocr_training_labels` migration for human-verified Section 03 ground truth linked to each uploaded `policy_documents` row. The workflow exposes private-copy review, label status, evidence notes, and approved insurer/product/policy/date/IDV/premium fields while keeping raw documents and OCR text out of fixtures and logs. The page now safely handles Supabase returning the optional label relation as an object, array, or null.

**APPLIED:** the migration was executed and verified in Supabase by workflow run `32480011119`; only migration `202608210001` was recorded because remote migration history is ahead of this checkout and must not be replayed or repaired speculatively. Automated OCR of every stored copy is intentionally deferred; labels must be human-verified before creating insurer-specific parser regressions.

**DEPLOYMENT TRIGGERED:** protected workflow run `32481825236` passed the compulsory verification gate and accepted the Vercel production deploy hook for main commit `e316903de156485f44afd947b5117fccaab58613`. The canonical route currently returns the expected unauthenticated redirect; authenticated training-page and label-save verification remain **UNVERIFIED**.
