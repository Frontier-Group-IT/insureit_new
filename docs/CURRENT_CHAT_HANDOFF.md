...
Section 02 vehicle extraction has deliberately not been added. The exact current form-to-payload-to-database mapping is in `docs/POLICY_OCR_SECTION_02_FIELD_MAP.md`. It establishes policy snapshot precedence, class-aware CC/GVW/seating handling, identity masking, the current server requirement for chassis/engine in both registration modes, and the unresolved `NEW-` versus `PENDING-` unregistered-vehicle migration hazard. Resolve and test those contracts before expanding the parser.
