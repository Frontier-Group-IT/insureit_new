## 2026-09-25 — Customer Profile remove Documents & KYC count badge (rebuilt from latest main)

- Branch: `ui/customer-profile-remove-document-count-badge-v2`.
- PR #2415 passed both canonical checks but became unmergeable because newer `main` introduced merge conflicts; this branch rebuilds the same UI-only change from the latest `main`.
- Removed the blue numeric badge from the right side of **Documents & KYC** on Customer App → Profile.
- The subtitle document count, expand/collapse chevron, upload control, and document list remain unchanged.
...
- **IMPLEMENTED; replacement PR/CI/merge/OTA pending.**
...
- PR: #2407 (replacement for #2405 after rebasing onto latest main).
...
- PR #2407 passed canonical `Verify web portal` workflow #4658, including regressions, typecheck, lint, and production build.\n- Merge commit: `4957d91520dd4e59d00bc1a485f83fa6972adb89`.\n- **MERGED; production deployment not verified in this step.**
...
- **IMPLEMENTED; final PR/CI/merge/migration application/deployment pending.**
...
- The extra child-page context row is removed.
- Period/horizon shortcuts are compact dropdowns; existing query/filter behavior is retained.
...
- Scope: Operations web claim detail only for claims where `policy_service_source='external'` and `claim_service_mode='broker_managed'`.
- All 9 Operations journey stages are now always clickable for these managed External Claims; lock icons are not used for future stages.
- Stage fields remain editable before/after the official stage and after `Claim Complete`, `Settled`, or `Closed`.
- Safety rule: a non-current stage saves details only. It cannot advance, reopen, regress, or otherwise change `claims.current_status`.
- The actual current stage retains the normal progression behavior and may advance through the existing workflow contract.
- Claim Intimation's dedicated workspace follows the same rule: save details from any stage position, but only advance when Claim Intimation is the actual active stage.
- Server validation now allows save-only future-stage detail writes for managed External Claims without requiring the corresponding customer milestone to already be completed.
- Internal/SIBL claims retain sequential stage locking. Self-managed External Claims remain customer-controlled and still enter Operations through the existing assistance/takeover flow.
- No database schema, migration, RLS, permission, mobile app, Partner app, APK/AAB, or native-runtime change.
- **IMPLEMENTED; no PR created; not merged; not deployed.**
...
**DEPLOYED / VERIFIED:** The operations claim detail now includes a web-only nine-stage journey panel aligned to the existing mobile external-claim field contract. It preserves historical claim statuses, renames the final-document presentation to Claim Intimation, stores operations-entered stage values in `claim_stage_details`, updates insurer claim number on `claims`, enforces mobile-matching mandatory fields, and leaves customer document upload/replacement behavior unchanged. No mobile files, mobile workflow, migration, or production configuration changed. PR #1221 merged as `3da966bb011050501fcaa9373b332803c086f162`; canonical verification run `33839297746` passed for the feature commit `082baccc13bb23eaa9aeff729805c69921af861a`; protected production workflow `33842743911` completed successfully and Vercel accepted the deploy hook with HTTP 201. Final Vercel Ready status and live end-to-end claim testing remain unverified.

**IMPLEMENTED, NOT DEPLOYED:** Spot Intimation now contains editable Accident Date/Time and Spot Intimation Date/Time fields while retaining the original claim overview above the stage menu. Saved accident time updates `claims.accident_at`, saved Spot Intimation time is read from `claim_stage_details`, and the unexplained raw bottom stage-detail panel was removed. The existing Spot Intimation Document Verification layout was not changed.
...
### Operator review workflow hardening — 2026-09-07

**IMPLEMENTED LOCALLY / NOT MERGED OR DEPLOYED:** an authorized IT Super User can now open an existing review task, start it, submit field-level PDF-grounded decisions and enqueue the sanitized refinement job without being blocked by the retired separate-reviewer workflow. The selected queue row renders the checklist even when the assignment panel is present, and assignment instructions are generic rather than encoding sample-specific IFFCO values. The refinement worker detects stale evidence labels and rebuilds the candidate through the protected sanitizer/RPC before returning it. Focused and complete OCR regressions, typecheck, lint and whitespace validation pass. The production build compiled but could not prerender `/auth/callback` because local Supabase public environment variables were unavailable; CI must provide the configured build environment.
...
### Self-healing refinement candidate automation — 2026-09-09
