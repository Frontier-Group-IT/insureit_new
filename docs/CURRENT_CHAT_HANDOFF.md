...
**IMPLEMENTED:** added an admin-only `/policies/ocr-training` page and `policy_ocr_training_labels` migration for human-verified Section 03 ground truth linked to each uploaded `policy_documents` row. The workflow exposes private-copy review, label status, evidence notes, and approved insurer/product/policy/date/IDV/premium fields while keeping raw documents and OCR text out of fixtures and logs.

The migration is committed but **NOT APPLIED** until run in the target Supabase environment. Automated OCR of every stored copy is intentionally deferred; labels must be human-verified before creating insurer-specific parser regressions. Local typecheck and lint passed. The web build reached compilation but was blocked during prerender by missing `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` environment variables.
