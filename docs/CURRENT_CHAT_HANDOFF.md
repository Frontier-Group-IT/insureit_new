...
The `/policies/ocr-training` queue shows proposal versus reviewer correction, bounded evidence labels, confidence and warnings. Reviewer dates use `DD/MM/YYYY` and save as ISO dates. `review_policy_ocr_training` and `approve_policy_ocr_training` are separate capabilities; database and server checks prohibit self-approval. Owner approval creates a sanitized candidate with a synthetic policy number and no raw OCR/PII, and never edits parser code.

Migration `202608210001_policy_ocr_training_labels.sql` remains **APPLIED** from the earlier release. New migration `20260821153000_premium_ocr_training_workflow.sql`, worker secret/cron configuration, authenticated two-person review, and live backlog processing remain **UNVERIFIED** until applied/deployed and directly checked.
