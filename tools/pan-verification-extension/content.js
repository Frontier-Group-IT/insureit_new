(() => {
  ...
  async function init() {
    await sendRuntimeMessage({ type: "CONTENT_READY" });
  ...
  function ensureAutomation() {
    if (loopStarted || !contextAlive) return;
    loopStarted = true;
  ...
  async function automationLoop() {
    while (contextAlive) {
      const stateResponse = await sendRuntimeMessage({ type: "GET_ALL_STATE" });
      if (!stateResponse?.ok || !contextAlive) return;
  ...
  async function nextJob() {
    let runtime = await sendRuntimeMessage({ type: "GET_RUNTIME" });
    if (!runtime) return null;
    let queue = Array.isArray(runtime.queue) ? runtime.queue : [];
    if (!queue.length) {
      const claim = await sendRuntimeMessage({ type: "CLAIM_MORE" });
      if (!claim?.ok) { await updateStatus(claim?.error || "Could not claim PAN jobs."); return null; }
      runtime = await sendRuntimeMessage({ type: "GET_RUNTIME" });
      if (!runtime) return null;
  ...
  async function processJob(job) {
    ...
    const runtime = await sendRuntimeMessage({ type: "GET_RUNTIME" });
    if (!runtime) return;
    ...
    for (let attempt = previousAttempt + 1; attempt <= MAX_JOB_ATTEMPTS && contextAlive; attempt += 1) {
      ...
      if (result === "session") {
        await sendRuntimeMessage({ type: "UPDATE_RUNTIME", patch: { state: "RECOVERING_SESSION", status: "IIB session expired. Sign in again; the same PAN will retry.", currentPan: masked, currentJobId: job.id, currentAttempt: attempt } });
        location.href = LOGIN_URL;
        return;
      }
  ...
  async function waitForResult(timeoutMs) {
    ...
    while (contextAlive && Date.now() - started < timeoutMs) {
      const pageType = classifyPage();
  ...
  async function updateStatus(status, currentPan) {
    ...
    await sendRuntimeMessage({ type: "UPDATE_RUNTIME", patch });
  }
  ...
  function waitFor(predicate, timeoutMs, intervalMs) { return new Promise((resolve) => { const started = Date.now(); const timer = setInterval(() => { if (!contextAlive) { clearInterval(timer); resolve(false); return; } let matched = false; try { matched = Boolean(predicate()); } catch (_) {} if (matched || Date.now() - started >= timeoutMs) { clearInterval(timer); resolve(matched); } }, intervalMs); }); }
